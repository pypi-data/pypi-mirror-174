...
class DatosCROM(TablasVC):
    ...
    def __procesar_d10s(self,mensajes_shallow=True,mensajes_deep=True):
        ...
        self.__procesar_d10s_dv(mensajes=mensajes_deep)
    ...
    def __procesar_d10s_dv(self,mensajes=True):
    ...
    def __iec61400_crear_columnas_pev(self,index,mensajes=False):
        ...
        n = len(index)
        nans   = np.full(n,fill_value=np.nan)
        ...
        s_bool_t = pd.Series(data=trues,index=index,dtype=bool)
        s_bool_f = pd.Series(data=falses,index=index,dtype=bool)
        s_f64 = pd.Series(data=zeros,index=index,dtype='float64')
        s_i64 = pd.Series(data=zeros,index=index,dtype='Int64')
        s_str = pd.Series(data=nans,index=index,dtype=str)

        #Tengo que aperturarlas una por una, porque en pandas 1.5
        cols_dtypes =  {
                'ID':s_i64.copy(deep=True), 
                'SolverAgent':s_str.copy(deep=True), 
                'Reason':s_str.copy(deep=True), 
                'Origin':s_str.copy(deep=True), 
                'Priority_IEC':s_i64.copy(deep=True),   
                'Pnom':None,
                'PotDis':s_f64.copy(deep=True), 
                'FullPerf':s_bool_t.copy(deep=True), 
                'Lim':s_bool_f.copy(deep=True), 
                'LimExt':s_bool_f.copy(deep=True), 
                'LimInt':s_bool_f.copy(deep=True), 
                'Ind':s_bool_f.copy(deep=True), 
                'IndExt':s_bool_f.copy(deep=True), 
                'IndInt':s_bool_f.copy(deep=True), 
                'ENS':s_f64.copy(deep=True), 
                'ENS_Lim':s_f64.copy(deep=True), 
                'ENS_LimExt':s_f64.copy(deep=True), 
                'ENS_LimInt':s_f64.copy(deep=True), 
                'ENS_Ind':s_f64.copy(deep=True), 
                'ENS_IndExt':s_f64.copy(deep=True), 
                'ENS_IndInt':s_f64.copy(deep=True), 
        }
        ...
        for p in parques_con_datos:
            for e in get_equipos_parque(p):
                for v,t in cols_dtypes.items():
                    if v == 'Pnom':
                        pot = np.full(n, fill_value = get_pot_pe(p,e)) 
                        columnas[(p,e,v)] = pd.Series(data=pot,index=index,dtype='float64').copy(deep=True)
                    else:
                        columnas[(p,e,v)] = t   
        return columnas
    ...
    def elaborar_curvas_de_potencia(self,
        ...
        if proceder:
            ...
            if n >1:
                ...
                dict_dfs_count = {k:v for k,v in dict_dfs_val.items() if not v is None}
        ...
        with pd.ExcelWriter(archivo) as w:
            ...
            if exp_rpt_consolidado: 
                ...
                if isinstance(self.consolidado,pd.DataFrame):
                    self.consolidado.to_excel(w,sheet_name='Consolidado')
                else:
                ...
                if isinstance(self.consolidado_redux,pd.DataFrame):
                    self.consolidado_redux.to_excel(w,sheet_name='Consolidado_Redux')
                else:
                    for g,df in self.consolidado_redux.items():
                        df.to_excel(w,sheet_name=f'Consolidado_Redux_{g}')
            
            if exp_rpt_curvas_de_potencia: 
                if isinstance(self.curvas_de_potencia,pd.DataFrame):
                    self.__rpt_curvas_de_potencia.to_excel(w,sheet_name='Curvas')
                else:
