...
class DatosCROM(TablasVC):
    ...
    def __procesar_d10s_dc(self,mensajes=True):
        ...
        index_teorico = pd.date_range(start=dia_datos_i,end=dia_datos_f,freq='10s',name='t_stamp')
        faltantes = [i for i in index_teorico if not i in index_real]
        if mensajes: print(f"\tRegistros faltantes: {faltantes}")
        
        #Registrar columnas y sus datatypes
        if mensajes: print(f"\tCreando DataFrame vacío por cada registro faltante")
        cols = self.datos_seg.columns
        df_vacio = pd.DataFrame(columns=cols,index=faltantes)
        ...
        dtypes = self.datos_seg.dtypes.to_dict()

        #Cambiar dtypes del dataframe vacío
        if mensajes: print(f"\tCompatibilizando datatypes de las columnas")
        for col,dtype in dtypes.items():
            try:
                df_vacio.loc[:,col] = df_vacio.loc[:,col].astype(dtype=dtype,errors='raise')
            except:
                print(f"\tError al intentar cambiar el dtype de {col} a {dtype}")
        
        # Con el DF vacío listo, se puede proceder a concatenar todo y ordenar
        self.datos_seg = pd.concat([self.datos_seg,df_vacio]).sort_index()
        self.datos_seg.index.name = 't_stamp'
    ...
    def __iec61400_crear_columnas_pev(self,index,mensajes=False):
        ...
        for p in parques_con_datos:
            for e in get_equipos_parque(p):
                #p = Parque, e = equipo
                columnas[(p,e,'ID')] = pd.Series(data=np.zeros(n),index=index,dtype='Int64')
                columnas[(p,e,'SolverAgent')] = pd.Series(data=np.full(n,fill_value=np.nan),index=index,dtype=str)
                columnas[(p,e,'Reason')] = pd.Series(data=np.full(n,fill_value=np.nan),index=index,dtype=str)
                columnas[(p,e,'Origin')] = pd.Series(data=np.full(n,fill_value=np.nan),index=index,dtype=str)
                columnas[(p,e,'Priority_IEC')] = pd.Series(data=np.full(n,fill_value=np.nan),index=index,dtype=str)
                columnas[(p,e,'PotDis')] = pd.Series(data=np.zeros(n),index=index,dtype='float64')
                columnas[(p,e,'FullPerf')] = pd.Series(data=np.full(n,fill_value=True),index=index,dtype=bool)
                columnas[(p,e,'Lim')] = pd.Series(data=np.full(n,fill_value=False),index=index,dtype=bool)
                columnas[(p,e,'LimExt')] = pd.Series(data=np.full(n,fill_value=False),index=index,dtype=bool)
                columnas[(p,e,'LimInt')] = pd.Series(data=np.full(n,fill_value=False),index=index,dtype=bool)
                columnas[(p,e,'Ind')] = pd.Series(data=np.full(n,fill_value=False),index=index,dtype=bool)
                columnas[(p,e,'IndExt')] = pd.Series(data=np.full(n,fill_value=False),index=index,dtype=bool)
                columnas[(p,e,'IndInt')] = pd.Series(data=np.full(n,fill_value=False),index=index,dtype=bool)
                columnas[(p,e,'ENS')] = pd.Series(data=np.zeros(n),index=index,dtype='float64')
                columnas[(p,e,'ENS_Lim')] = pd.Series(data=np.zeros(n),index=index,dtype='float64')
                columnas[(p,e,'ENS_LimExt')] = pd.Series(data=np.zeros(n),index=index,dtype='float64')
                columnas[(p,e,'ENS_LimInt')] = pd.Series(data=np.zeros(n),index=index,dtype='float64')
                columnas[(p,e,'ENS_Ind')] = pd.Series(data=np.zeros(n),index=index,dtype='float64')
                columnas[(p,e,'ENS_IndExt')] = pd.Series(data=np.zeros(n),index=index,dtype='float64')
                columnas[(p,e,'ENS_IndInt')] = pd.Series(data=np.zeros(n),index=index,dtype='float64')
                columnas[(p,e,'Pnom')] = pd.Series(data=np.full(n, fill_value = get_pot_pe(p,e)),index=index,dtype='float64')
        return columnas
    ...
    def __iec61400_explotar_minutalmente_incidencias(self,jerarquia_equipos='WTG',mensajes=False):
        ...
        self.__rpt_iec61400_minutal = self.__super_deep_copy(df)
    ...
    def __iec61400_colapsar_nuevamente_a_incidencias(self,mensajes=False):
        ...
        fecha_i = copy.deepcopy(self.fecha_i).replace(hour=0,minute=0,second=0)
        fecha_f = copy.deepcopy(self.fecha_f).replace(hour=23,minute=59,second=59)
        ...
        for p in parques_con_datos:
            ...
            for e in equipos:
                ...
                fechas_ini = df.index[flt_fechas_ini].copy(deep=True).array
                fechas_fin = df.index[flt_fechas_fin].copy(deep=True).array

                # A partir de las fechas obtenidas, rellenar el resto de los valores de las incidencias
                n = len(fechas_ini)
                if n > 0:
                    pnom = get_pot_pe(p,e)
                    ids = df.loc[fechas_ini,(p,e,v)].copy(deep=True).array
                    origenes = df.loc[fechas_ini,(p,e,'Origin')].copy(deep=True).array
                    razones = df.loc[fechas_ini,(p,e,'Reason')].copy(deep=True).array
        
                    data = {
                        'Nemo':pd.Series(np.full(n,dtype=object,fill_value=p)).copy(deep=True),
                        'Equipo':pd.Series(np.full(n,dtype=object,fill_value=e)).copy(deep=True),
                        'ID':pd.Series(ids,).copy(deep=True),
                        'Start':pd.Series(fechas_ini).copy(deep=True),
                        'End':pd.Series(fechas_fin).copy(deep=True),
                        'Hours':pd.Series(np.zeros(n)).copy(deep=True),
                        'ENS':pd.Series(np.zeros(n)).copy(deep=True),
                        'Reason':pd.Series(razones).copy(deep=True),
                        'Origin':pd.Series(origenes).copy(deep=True),
                        'Pnom':pd.Series(np.full(n,fill_value=pnom)).copy(deep=True)
                    }

                    #Crear un dataframe temporal con los resultados
                    df_tmp = pd.DataFrame(data)
                    df_tmp['Hours'] = (df_tmp['End'] - df_tmp['Start']).dt.total_seconds()/3600
                    lista_dfs.append(self.__super_deep_copy(df_tmp))
    ...
    #
    def _identificar_indisponibilidades(self,tolerancia_en_segundos=30):
        ...
        for p in parques_con_datos:
            equipos = self.consultar_equipos_no_agrupamientos(nemo_parque=p)
            for e in equipos:
                pgen_e0 = self.datos_seg.loc[:,(p,e,'Pgen')] == 0
                ppos_e0 = self.datos_seg.loc[:,(p,e,'Ppos')] > 0
                distinto_nan = self.datos_seg.loc[:,(p,e,['Pgen','Ppos'])].notna().all(axis=1)
                considerable = distinto_nan & ppos_e0 & pgen_e0

                distinto_anterior = (considerable != considerable.shift(1)).fillna(False)
                distinto_siguiente = (considerable != considerable.shift(-1)).fillna(False)
                igual_anterior = (considerable == considerable.shift(1)).fillna(True)
                igual_siguiente = (considerable == considerable.shift(-1)).fillna(True)

                serie_pri = distinto_anterior & igual_siguiente & considerable
                serie_ult = igual_anterior & distinto_siguiente & considerable
                unicos = distinto_anterior & distinto_siguiente & considerable

                #Creamos dos series, una para detectar las fechas de inicio de las nuevas incidencias y otra para las fechas de fin
                flt_fechas_ini = serie_pri | unicos
                flt_fechas_fin = serie_ult | unicos

                # Obtener fechas exactas de inicios y fines de incidencia
                fechas_ini = self.datos_seg.index[flt_fechas_ini].array
                fechas_fin = self.datos_seg.index[flt_fechas_fin].array
        ...
        df_tmp = pd.concat(lista_dfs,ignore_index=True).query(f'Hours >= {tolerancia_en_segundos}')
        df_tmp['Hours'] = df_tmp['Hours'] / 3600
        df_tmp['Motivo'] = 'Pgen == 0 & Ppos > 0'
    ...
    def elaborar_curvas_de_potencia(self,
        ...
        if proceder:
            ...
            dict_dfs_val = {v:self.__curvas_de_potencia(df,vm,v,vf,f) for v in vc}        # Crea un diccionario del tipo 'VariableCategórica':df_curvas_de_potencia
            dict_dfs_val = {k:v for k,v in dict_dfs_val.items() if not v is None}           # En caso de que haya algún resultado sin curvas de potencia, descartar

            if n >1:
                # Crea un diccionario del tipo 'VariableCategórica':df_curvas_de_potencia
                dict_dfs_val = {v:self.__curvas_de_potencia(df,vm,v,vf,f) for v in vc}          
                dict_dfs_count = {v:self.__curvas_de_potencia(df,vm,v,vf,'count') for v in vc}
