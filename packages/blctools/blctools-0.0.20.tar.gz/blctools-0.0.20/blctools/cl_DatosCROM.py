...
class DatosCROM(TablasVC):
    ...
    #
    def _identificar_indisponibilidades(self,duracion=30,fc=0.00,ws=2,ppos_u=0.01,ws_pl=None,ppos_u_pl=None):
        ...
        for p in parques_con_datos:
            ...
            for e,pnom in equipos.items():
                flt_pgen = self.datos_seg.loc[:,(p,e,'FC')] <= fc
                flt_ppos = self.datos_seg.loc[:,(p,e,'Ppos_U')] >= ppos_u
                flt_ws = self.datos_seg.loc[:,(p,e,'Wind')] >= ws
                distinto_nan = self.datos_seg.loc[:,(p,e,['Pgen','Ppos'])].notna().all(axis=1)
                considerable = distinto_nan & flt_pgen & flt_ppos & flt_ws
                mensaje = f"Duración >= {duracion}s, Pot <= {pnom*fc:.2f}kW, Ppos >= {ppos_u*pnom:.2f}kW, Wind >= {ws}m/s"
                
                if ws_pl: 
                    flt_ws_pl = self.datos_seg.loc[:,(p,'PLANT','Wind')] >= ws_pl
                    considerable = considerable & flt_ws_pl
                    mensaje += f"Wind (Plant avg) >= {ws_pl}m/s"
                
                if ppos_u_pl: 
                    flt_ppos_u_pl = self.datos_seg.loc[:,(p,'PLANT','Ppos_U')] >= ppos_u_pl
                    considerable = considerable & flt_ppos_u_pl
                ...
                if n > 0:
                    ...
                    df_tmp['Motivo'] = mensaje
                    lista_dfs.append(df_tmp)

        df_tmp = pd.concat(lista_dfs,ignore_index=True).query(f'Hours >= {duracion}')
