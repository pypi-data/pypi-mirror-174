...
def generate_unpaved(
    ...
    lu_counts = zonal_stats(
        catchments, lu_rast, affine=lu_affine, categorical=True, all_touched=all_touched
    )

    rast, affine = read_raster(soiltype, static=True)
    soiltypes = zonal_stats(
        catchments, soiltype, affine=affine, stats="majority", all_touched=all_touched
    )
...
def generate_ernst(
    catchments, depths, resistance, infiltration_resistance, runoff_resistance
):
    """
    ...
    ernst_drr.set_data(
        pd.DataFrame(
            ...
            columns=["code", "reslist", "lvs", "cvi", "cvs"],
            dtype="str",
        ),
        index_col="code",
    )
...
def generate_paved(
    ...
    mean_elev = zonal_stats(
        catchments, sl_rast, affine=sl_affine, stats="median", all_touched=all_touched
    )

    if isinstance(street_storage, str):
        strs_rast, strs_affine = read_raster(street_storage, static=True)
        str_stors = zonal_stats(
            catchments, strs_rast, affine=strs_affine, stats="mean", all_touched=True
        )
    ...
    if isinstance(sewer_storage, str):
        ...
        sew_stors = zonal_stats(
            catchments, sews_rast, affine=sews_affine, stats="mean", all_touched=True
        )
    ...
    if isinstance(pump_capacity, str):
        ...
        pump_caps = zonal_stats(
            catchments, pump_rast, affine=pump_affine, stats="mean", all_touched=True
        )
    ...
    for num, cat in enumerate(catchments.itertuples()):
        ...
        if sewer_areas is not None:
            ...
            pav_area = (
                str(lu_counts[num][14.0] * px_area) if 14.0 in lu_counts[num] else "0"
            )
...
def generate_openwater(catchments, landuse, meteo_areas, zonalstats_alltouched=None):
    ...
    ow_drr.set_data(
        pd.DataFrame(
            ...
            columns=["code", "area", "meteostat", "px", "py", "boundary"],
            dtype="str",
        ),
        index_col="code",
    )
...
def generate_boundary(boundary_nodes, catchments, drrmodel, overflows=None):
    ...
    bnd_drr.set_data(
        pd.DataFrame(np.zeros((numlats, 3)), columns=["code", "px", "py"], dtype="str"),
        index_col="code",
    )
    if overflows is not None:
...
def generate_precip(areas, precip_folder=None, precip_file=None):
    ...
    for ifile, file in tqdm(
        enumerate(file_list), total=len(file_list), desc="Reading precipitation files"
    ):
...
def generate_evap(areas, evap_folder=None, evap_file=None):
    ...
    for ifile, file in tqdm(
        enumerate(file_list), total=len(file_list), desc="Reading evaporation files"
    ):
