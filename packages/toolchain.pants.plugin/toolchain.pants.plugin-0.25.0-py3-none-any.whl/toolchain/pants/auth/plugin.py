...
# For backward compatiblity since plugin_name is only supported in pants 2.14 and above.
# we can refactor this when the plugin removes support for pants 2.13 and lower.
_EXTRA_RESULT_KWARGS = (
    {"plugin_name": "Toolchain Remote Cache Auth Plugin"} if hasattr(AuthPluginResult, "plugin_name") else {}
)
