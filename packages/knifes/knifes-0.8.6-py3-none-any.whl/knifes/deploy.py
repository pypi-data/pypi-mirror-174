...
def main_script(chdir, logdir, app_name):
    ...
    if not args:
        ...
        autostart_at_boot(chdir, app_name)
    else:
        print_help(chdir)
...
def autostart_at_boot(project_dir, app_name):
    """设置开机自启动"""
    ...
    gunicorn_conf_filepath = f'{project_dir}gunicorn_{app_name}_conf.py'
    cmd = f'{project_dir}venv/bin/gunicorn -c {gunicorn_conf_filepath}'
    ...
    with open(systemd_conf_filepath, 'w') as f:
        f.write(systemd_template.replace('{Description}', app_name).replace('{ConditionPathExists}', gunicorn_conf_filepath).replace('{ExecStart}', cmd))
