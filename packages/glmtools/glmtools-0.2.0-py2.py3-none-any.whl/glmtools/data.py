...
class AbstractGLMData(AbstractAnam):
    ...
    def drop(self, inds, inplace=False):
        ...
        for key in ret.info.keys():
            if isinstance(ret.info[key], (list, np.ndarray)) and len(ret.info[key]) == ret.data.shape[0]:
                ret.info[key] = np.delete(np.array(ret.info[key]), inds)

        ret.data = np.delete(ret.data, inds, axis=0)
        ret.info['num_observations'] = ret.data.shape[0]
...
class GLMData:
    ...
    def __init__(self, **kwargs):
        ...
        # Arbitrary information stored in meta
        self.meta = {}
        if 'dim_labels' in kwargs.keys():
            # Set default dimension labels if not defined
            self.meta['dim_labels'] = kwargs['dim_labels']
        elif self.data is not None and 'dim_labels' not in self.info:
            self.meta['dim_labels'] = list(('Observations', *['dim'+chr(65+ii) for ii in range(self.data.ndim-1)]))

        if 'meta' in kwargs:
            for key, item in kwargs['meta'].items():
                self.meta[key] = item
        self.meta['num_observations'] = self.data.shape[0]

        # Covariates MUST be list of same length of data
        self.covariates = {}
        for key in kwargs.keys():
            if key not in ('data', 'meta', 'dim_labels'):
                if len(kwargs[key]) == self.data.shape[0]:
                    self.covariates[key] = list(kwargs[key])
