...
def forecast_to_excel(Filename, Sheet_name, output_name):
    ...
    for transaction_type in transactions:
        if transaction_type == 'Debet':
            for i in datac['Account'].unique():
                try:
                    forecast_data = forecasting_bzdata(data, i, 'Debet', 12)
            ...
            for i in datac['Account'].unique():
                try:
                    forecast_data = forecasting_bzdata(data, i, 'Credit', 12)
                    for k, l in enumerate(forecast_data['future_prediction'][1:]):
