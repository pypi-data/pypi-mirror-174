...
def deploy_truss(
    ...
    if model_info is not None and is_development:
        version_info = try_patch_development_truss(model_name, b10_truss)
    ...
    if is_development:
        model_id, model_version_id = deploy_development_truss(
...
def deploy_development_truss(
        model_name,
        s3_key,
        config,
        is_trusted=False,
):
    model_version_json = api.deploy_development_truss(
        model_name=model_name,
        s3_key=s3_key,
        config=config,
...
def try_patch_development_truss(
    model_name: str,
    truss_handle: TrussHandle,
) -> Optional[dict]:
    dev_model_version_info = _get_development_model_version_info(model_name)
    if dev_model_version_info is None:
        logger.info(f"Giving up on patching: no development model found for model {model_name}.")
        return
    ...
    resp = api.patch_development_truss(
        model_name=model_name,
        client_version=baseten.__version__,
        patch=patch_request,
    )
    ...
    logger.info(f"Successfully patched development version of model `{model_name}`.")
    return dev_model_version_info


def _get_development_model_version_info(model_name) -> Optional[dict]:
    model = api.get_model(model_name)
    versions = model["model_version"]["oracle"]["versions"]
    for version in versions:
        if version["is_development"] is True:
            return version
...
def build_and_deploy_truss(
    ...
    if isinstance(model, TrussHandle):
        return deploy_truss(model, model_name, version_bump, is_trusted, is_development, external_model_version_id)
    truss = build_truss(model)
    return deploy_truss(truss, model_name, version_bump, is_trusted, is_development, external_model_version_id)
