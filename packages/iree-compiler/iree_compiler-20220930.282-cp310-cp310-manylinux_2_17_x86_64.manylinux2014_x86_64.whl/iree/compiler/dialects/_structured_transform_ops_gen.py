...
@_ods_cext.register_operation(_Dialect)
@_ods_extend_opview_class(_ods_ext_module)
class MapNestedForeachThreadToGpuBlocks(_ods_ir.OpView):
  OPERATION_NAME = "transform.structured.map_nested_foreach_thread_to_gpu_blocks"

  _ODS_REGIONS = (0, True)

  def __init__(self, result, target, *, gridDim=None, generate_gpu_launch=None, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
    operands.append(_get_op_result_or_value(target))
    if gridDim is not None: attributes["gridDim"] = gridDim
...
@_ods_cext.register_operation(_Dialect)
@_ods_extend_opview_class(_ods_ext_module)
class MapNestedForeachThreadToGpuThreads(_ods_ir.OpView):
  OPERATION_NAME = "transform.structured.map_nested_foreach_thread_to_gpu_threads"

  _ODS_REGIONS = (0, True)

  def __init__(self, result, target, *, blockDim=None, syncAfterDistribute=None, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
    operands.append(_get_op_result_or_value(target))
    if blockDim is not None: attributes["blockDim"] = blockDim
    if syncAfterDistribute is not None: attributes["syncAfterDistribute"] = syncAfterDistribute
    results.append(result)
    _ods_successors = None
    super().__init__(self.build_generic(
      attributes=attributes, results=results, operands=operands,
