...
class FeatureDefinitionWrapper:
    ...
    def is_continuous(self) -> bool:
        # TODO(kevjumba): Will add temporal continuous mode here in separate pr
        return (
            self.fv.temporal_aggregate.is_continuous
            if self.is_temporal_aggregate and self.fv.temporal_aggregate.HasField("is_continuous")
            else False
        )
