...
class PGS3CH:
    ...
    def __init__(self, config, s3_config, ch_config, pg_config, **kwargs):
        """
        ...
        self.kwargs = kwargs
        self.by_date = self.kwargs.get("by_date", False)
        ...
        if self.incremental or self.by_date:
    ...
    def extract_updated_to_s3(self):
        ...
        for _ in range(0, parts):
            ...
            sql = SQL_HISTORY.format(
                key_field=self.incremental_field or self.key_field,
                name=self.entity_name,
                fields='"' + '","'.join(self.schema_pg_select_fields.keys()) + '"',
                range_from=range_from,
                range_to=range_to,
