...
import random
from base.SushiGo.env import get_list_action, getActionSize, check_victory, amount_player, numba_main
@njit()
...
def playerRandom1(play_state,file_temp,file_per):
    ...
    if len(file_temp) < 2 :
        ...
        if len(play_state)<55 or 70<len(play_state)<120:# SushiGo #MACHIKORO :
            ...
            file_temp.append(np.random.random((100,getActionSize())))
            ...
            file_temp.append(np.random.random((50,getActionSize())))
        elif 120 <len(play_state)  <170:  # Splendor , Splendor_view_only
            ...
            file_temp.append(np.random.random((200,getActionSize())))
        else :   # CENTURY
            file_temp.append((np.random.random((len(play_state),300)))*2 -0.6)
            file_temp.append(np.random.random((300, 150)))
            file_temp.append(np.random.random((150,getActionSize())))
...
def playerScore1(play_state,file_temp,file_per):
    ...
    for act in a:
        if matran2[act] > max:
            max = matran2[act]
            action_max = act
    check_vic = getReward(play_state)
    if check_vic != -1:
