...
class EnginePageBase(ConsolePage, Console):
    ...
    def GetPageMenu(self):
        ...
        if self.debug is True:
            ...
            dbg_item.SetItemLabel('Disable Debugger')
        else:
            dbg_item.SetBitmap(disabled_bmp)
            dbg_item.SetItemLabel('Enable debugger')
        menu.Append(dbg_item)
        ...
        if self.profile is True:
            ...
            pro_item.SetItemLabel('Disable profiler')
        else:
            pro_item.SetBitmap(disabled_bmp)
            pro_item.SetItemLabel('Enable profiler')
        menu.Append(pro_item)
...
class EngPageSTC(EnginePageBase, stc.StyledTextCtrl):
    ...
    def OnKeyDown(self, event):
        ...
        # backspace
            ...
            if (currentpos>(self._promptpos)) and (edit is True):
                event.Skip()
        ...
        #in uneditable region move to end and skip event to allow printable chars
        elif (edit is False) and ( chr(key) in string.printable):
