...
class ConsoleFrame(aui_addons.AUIFrame):
    ...
    def Show(self, show=True):
        """
        Overload show to make sure auimanger works correctly
        BUG in agw auimanger unbinding when window hiden?
        """
        self.auimgr.SetManagedWindow(self)
        self.book.GetAuiManager().SetManagedWindow(self.book)
        wx.Frame.Show(self, show)
        self.book.GetAuiManager().Update()
