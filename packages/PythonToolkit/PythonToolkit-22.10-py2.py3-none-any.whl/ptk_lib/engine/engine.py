...
from . import eng_tasks                        #engine task utils  
...
class Engine(MBClient):
    ...
    def __init__(self, englabel='New engine', userdict={}, timeout=10):
        ...
        if '_engine' in __main__.__dict__:
        ...
        elif '__builtins__' not in  userdict:
            #add builtins if not already present
            userdict['__builtins__'] = builtins
        ...
        #-----------------------------------------------------------------------
        #the builtins module
        #exec('import builtins',self._userdict)
    ...
    def run_task(self,taskname, args=(), kwargs={}):
        ...
        #run the task
        logging.captureWarnings(True) # supress warnings to the log
        try:
            result=task(globals, locals, *args, **kwargs)
        except:
            log.exception('run_task failed :'+task.__name__)
            raise
        logging.captureWarnings(False) # allow warnings again
        return result
    ...
    def register_task(self,task):
        ...
        """
        log.debug('Registering task '+task.__name__)
        if task.__name__ in self._tasks:
            result = False
        else:
            self._tasks[task.__name__]=task
            result=True
        return result
    ...
    def get_task(self, taskname):
        ...
        """
        #get the task
        if taskname in self._tasks is False:
    ...
    def msg_register_task(self, msg):
        ...
        s = msg.data[0]
        c = marshal.loads(s)
        #task = types.FunctionType(c, globals(), None)
        task = types.FunctionType(c, {'__builtins__':builtins}, None)
    ...
    def msg_add_builtin(self, msg):
        ...
        #give builtin command access to user dict.
        cmd = types.FunctionType(code,self._userdict,None)
        builtins.__dict__[name] = cmd
        return True
