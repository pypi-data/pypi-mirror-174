...
class DebuggerInterface():
    ...
    def clear_breakpoints(self, ids):
        """
        Clear the debugger breakpoint with the ids given.
        If ids is None all breakpoints will be cleared.
        """
        if self.console.is_interactive is False:
            raise Exception('Managed engine is not active')

        res = self.console.send_msg( self.console.engine, 
                    eng_messages.ENG_DEBUG_CLEARBP, tuple(ids), True )
        return res
