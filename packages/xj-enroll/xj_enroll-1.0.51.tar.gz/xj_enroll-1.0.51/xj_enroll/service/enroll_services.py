...
from xj_enroll.serializers import EnrollRecordListSerializer
...
class EnrollServices:
    ...
    def enroll_undertake_list(params, need_pagination=True):
        ...
        enroll_obj = EnrollRecord.objects.annotate(thread_id=F("enroll__thread_id")).filter(**params).values()
        if not need_pagination:
