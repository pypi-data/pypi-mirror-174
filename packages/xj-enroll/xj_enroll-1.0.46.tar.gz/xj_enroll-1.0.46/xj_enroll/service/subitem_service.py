...
class SubitemService:
    ...
    def detail(pk=None):
        ...
        subitem_dict = subitem_obj.to_json()
        subitem_list = output_convert([subitem_dict])
        return subitem_list[0], None
        ...
        try:
            ...
            if not is_pagination:
                ...
                for item in result_list:
                    record = SubitemService.check_has_enroll(item.get("id"))
                    item["is_enroll"] = 1 if record else 0
                    item["record"] = record.to_json() if record else {}
                    result.append(item)
                return result, None
    ...
    @staticmethod
    def check_has_enroll(subitem_id):
        return EnrollSubitemRecord.objects.filter(enroll_subitem_id=subitem_id).exclude(enroll_subitem_status_code=124).first()
