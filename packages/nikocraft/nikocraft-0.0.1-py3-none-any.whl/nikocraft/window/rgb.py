...
class RGB(tuple):
    ...
    def dict(self, keys: Tuple[AnyStr, AnyStr, AnyStr] = ("red", "green", "blue")) -> Dict[AnyStr, int]:
    ...
    def __add__(self, other: Self) -> Self:
        return RGB(self[0] + other[0], self[1] + other[1], self[2] + other[2]).clamp()

    def __sub__(self, other: Self) -> Self:
        return RGB(self[0] - other[0], self[1] - other[1], self[2] - other[2]).clamp()

    def __mul__(self, factor: Union[int, float]) -> Self:
        return RGB(self[0] * factor, self[1] * factor, self[2] * factor).clamp()

    def __truediv__(self, divisor: Union[int, float]) -> Self:
        return RGB(self[0] / divisor, self[1] / divisor, self[2] / divisor).clamp()

    def __floordiv__(self, divisor: Union[int, float]) -> Self:
        return RGB(self[0] // divisor, self[1] // divisor, self[2] // divisor).clamp()

    def __pow__(self, power: float, modulo: float = None):
        return RGB(pow(self[0], power, modulo), pow(self[1], power, modulo), pow(self[2], power, modulo)).clamp()
