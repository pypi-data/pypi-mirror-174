...
def implement_corrections(final_ss_df,df_l3m_sales):
    ...
    df['fcst_zero_w_sales'] = np.where(((df['fcst']==0)&(df['l3m_sales_qty']>0)),1, 0)
    df['order_upto_point'] = np.where(df['fcst_zero_w_sales']==1,np.round(df['l3m_sales_qty']/3), df['order_upto_point'])
    df['reorder_point'] = np.where(df['fcst_zero_w_sales']==1,np.floor(df['order_upto_point'] / 2), df['reorder_point'])
    df['safety_stock'] = np.where(df['fcst_zero_w_sales']==1,np.floor(df['order_upto_point'] / 4), df['safety_stock'])
    df = df[cols]
    return df
