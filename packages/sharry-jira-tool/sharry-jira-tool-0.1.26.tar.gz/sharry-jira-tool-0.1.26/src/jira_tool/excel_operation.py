...
def read_excel_file(
    ...
    # The count of column is taking from the defination file to avoid too many columns inside the excel file.
    # Also, need to avoid exceed the range of the actual count.
    if excel_defination.column_count > sheet.max_column:
        column_count = sheet.max_column
    else:
        column_count = excel_defination.column_count
...
def process_excel_file(
    ...
    if excel_defination_config is None:
        ...
        print("Using default excel defination...")
    else:
        excel_defination.load_file(excel_defination_config)
        print("Using custom excel defination...")

    validation_result = excel_defination.validate()
    ...
    for sort_strategy in sort_strategy_priorities:
        if sort_strategy.lower() in "InlineWeights".lower():
            ...
            sort_stories_by_property_and_order(stories, excel_defination)
