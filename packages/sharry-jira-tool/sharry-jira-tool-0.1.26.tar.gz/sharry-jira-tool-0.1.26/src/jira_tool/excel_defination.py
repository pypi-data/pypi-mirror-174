    ...
    def validate(self) -> "list":
        ...
        for (
            ...
            if column_name is None or len(column_name) == 0:
                invalid_definations.append(
                    f"Column name cannot be empty. Index: {column_index}"
                )
                continue
            # Check Missing/Duplicate Index
            if column_index is None:
                invalid_definations.append(f"Missing Index. Column: {column_name}")
            if column_index in exist_indexes:
                invalid_definations.append(f"Duplicate Index. Column: {column_name}")
            ...
            if column_type not in (
                ...
                invalid_definations.append(f"Invalid Type. Column: {column_name}")
