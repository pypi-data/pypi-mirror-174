...
class XiuxianDateManage:
  ...
  "fabao_weapon" integer DEFAULT 0
    ...
    def update_back(self, user_id, goods_id, num=None):
        """使用物品"""
        check_sql = f"select goods_num from back where user_id=? and goods_id=?"
        cur = self.conn.cursor()
        cur.execute(check_sql, (user_id, goods_id))
        ...
        if result and result[0] > 0:
            sql = f"UPDATE back SET goods_num = ? where user_id = ? and goods_id=?"
            cur.execute(sql, (id, user_id,))
            self.conn.commit()

    def get_back_goods(self, user_id, goods_id):
        """根据USER_ID，goods_id获取用户数据"""
        check_sql = f"select goods_num from back where user_id=? and goods_id=?"
        cur = self.conn.cursor()
        cur.execute(check_sql, (user_id, goods_id))
        result = cur.fetchone()
