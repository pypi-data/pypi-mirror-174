...
except ModuleNotFoundError : system("pip install webbrowser"); import webbrowser
...
class Connect() :
    def post(data: dict) -> dict :
        for _ in range(3):
            try :
                connection: HTTPSConnection = HTTPSConnection(Robot._getURL(), 80)
                connection.request('POST', Robot._getURL(), body=data)
...
class Metas :
    def checker(text: str) -> list :
        ...
        if '**' in text : Bold: list = findall(r'\*\*(.*?)\*\*', text); boldFromIndex: list = [text.index(i) for i in Bold]; [(result.append({'from_index': from_index-2, 'length': len(length), 'type': 'Bold'})) for from_index , length in zip(boldFromIndex, Bold)]
        if '__' in text : Italic: list = findall(r'\_\_(.*?)\_\_', text); ItalicFromIndex: list = [text.index(i) for i in Italic]; [(result.append({'from_index': from_index-2, 'from_index': len(length), 'type': 'Italic'})) for from_index , length in zip(ItalicFromIndex, Italic)]
        if '``' in text : Mono: list = findall(r'\`\`(.*?)\`\`', text); monoFromIndex: list = [text.index(i) for i in Mono]; [text.index(i) for i in Mono]; [(result.append({'from_index': from_index-2, 'length': len(length), 'type': 'Mono'})) for from_index , length in zip(monoFromIndex, Mono)]
        return [result, texts]

class Tags :
    def checker(text: str, guids=None, types=None) -> list : result, texts = [], text.replace('@', ''); Tags: list = findall(r'\@(.*?)\@', text); tagFromIndex: list = [text.index(i) for i in Tags]; [(result.append({'type': 'MentionText', 'mention_text_object_guid': guid, 'from_index': from_index-1, 'length': len(length), 'mention_text_object_type': mode})) for from_index, length, guid, mode in zip(tagFromIndex, Tags, guids, types)]; return [result, texts]
...
class Robot :
    def __init__(self, auth: str = None, app='rubx', phone_number: str = None, device: dict = defaultDevice, proxy: dict = {'http':'http://127.0.0.1:9050'}, your_name: str = 'rubx', city: str = 'tehran', banner: bool = True, creator_channel_open: bool = True) -> None :
    ...
    def sendMessage(self, text: str, chat_id: str = None, username: str = None, link: str = None, metadata: list =[], parse_mode: str = None, button_id: str = None, message_id: str = None) -> dict:
