...
class Editor(tkinter.Toplevel):
	...
	def __init__(self):
		...
		if pkg_resources.resource_exists(__name__, ICONPATH):
			...
			realpath = pkg_resources.resource_filename(__name__, '__init__.py')
			idx = realpath.rindex('/') + 1
			realpath = realpath[:idx] + ICONPATH

			print('Can not find icon-file %s' % (realpath))
		...
		if pkg_resources.resource_exists(__name__, HELPPATH):
			...
			realpath = pkg_resources.resource_filename(__name__, '__init__.py')
			idx = realpath.rindex('/') + 1
			realpath = realpath[:idx] + HELPPATH

			print('Can not find help-file %s' % (realpath))
			self.helptxt = 'Could not load help-file. Press ESC to return.'
		...
		self.contents.grid(row=1, column=0, columnspan=3, sticky='nswe')
...
########## Configuration Related Begin
	...
	def save_config(self, event=None):
		...
		if not pkg_resources.resource_exists(__name__, CONFPATH):
			...
			realpath = pkg_resources.resource_filename(__name__, '__init__.py')
			idx = realpath.rindex('/') + 1
			realpath = realpath[:idx] + CONFPATH
