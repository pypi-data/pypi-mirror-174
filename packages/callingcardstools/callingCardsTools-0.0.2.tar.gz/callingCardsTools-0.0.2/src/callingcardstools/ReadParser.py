...
class ReadParser(BarcodeParser):
    ...
    def __init__(self, barcode_details_json:str = "", r1:str = "", r2:str = "", output_id_field:str = "") -> None:
        """_summary_
    ...
    def fastq_path_parser(self,fq_path:str) -> bool:
        ...
        if not os.path.exists(fq_path):
            ...
            if os.path.splitext(fq_path)[1] == ".gz":
                if not os.path.splitext(os.path.splitext(fq_path)[0])[1] in fq_extensions:
    ...
    def next(self) -> None:
        ...
        Raises:
            ...
            StopIteration: If the end of file is reached in either R1 or R2 (if it is set)
            ValueError: If after advancing the read ids for R1 and r2 (if paired end)
            do not match
        """
        r1_stop = False
        r2_stop = False
    ...
    def parse(self,add_bc_to_id:bool = True, set_name_to_empty:bool = True) -> dict:
        """Using the barcode details, process a given read

        Args:
            add_bc_to_id (bool, optional): Set to False to skip adding the 
            barcode to the read id (UMITools style, <read_id>_<barcode>). 
            Defaults to True.
            set_name_to_empty (bool, optional): Set the name attribute to empty. 
            SeqIO sets to ID if a name DNE. Defaults to True.

        Returns:
            dict: A dictionary with the r1 and r2 SeqRecord objects, the barcode 
            and the unadulterated read ID (if add_bc_to_id is True, then the 
            r1 and r2 SeqRecord ids will have the barcode appended)
        """
        ...
        # instantiate a dict to hold the sequences corresponding to barcode components
        components_dict = {}
        ...
        for end,read in read_dict.items():
            if read:
                for k,v in self.barcode_dict[end].items():
                    components_dict["_".join([end,k])] = str(read.seq[v['index'][0]:v['index'][1]])
