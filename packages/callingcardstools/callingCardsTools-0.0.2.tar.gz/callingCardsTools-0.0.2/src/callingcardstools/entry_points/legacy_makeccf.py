...
def sort_ccf_file(ccffilename):
    sorter = ["chrI","chrII","chrIII","chrIV","chrV","chrVI","chrVII","chrVIII","chrIX","chrX","chrXI","chrXII","chrXIII","chrXIV","chrXV","chrXVI","chrM"]
...
def make_ccffile(sbamFilename,out_dir):
    ...
    with open(qc_output,'w') as qc_handle:
        qc_handle.write("%s\t%s\t%s\n" %(remove_suffix(os.path.basename(ccf_file),'.ccf'),qc_list[0], qc_list[1] ))
