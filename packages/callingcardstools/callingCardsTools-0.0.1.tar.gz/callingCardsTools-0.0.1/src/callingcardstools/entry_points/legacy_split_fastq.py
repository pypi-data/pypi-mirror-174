...
def filter_reads(read1fn,read2fn,barcodefn,output,hammp,hammt):
    ...
    for key in barcode_dict.keys():
        r1_filename = output+barcode_dict[key]+"_"+key[0]+"_"+key[1]+"_R1.fastq"
        #print the filename minus the _R1.fasta suffix 
        #This makes the next step easier
        print(output+barcode_dict[key]+"_"+key[0]+"_"+key[1],file=filelist_filehandle)
        r1_bcp_filehandle_dict[key] = open(r1_filename,'w')
        if read2fn:
            r2_filename = output+barcode_dict[key]+"_"+key[0]+"_"+key[1]+"_R2.fastq"
    ...
    for expt in barcode_dict.values():
        expt_dict[expt]=0
        rest_enzyme_dict[expt] = {"TaqAI":0,"HinP1I":0,"HpaII":0,"Undet":0}
    ...
    for read1_record in SeqIO.parse(r1Handle,"fastq"):
        ...
        if FILTER_SEQUENCE in read1_record.seq[0:7+len(FILTER_SEQUENCE)]:
            ...
            if (primerbc,transbc) in barcode_dict:
                ...
                print(read1_record.format("fastq")[0:len(read1_record.format("fastq"))-1], file=r1_bcp_filehandle_dict[(primerbc,transbc)])
                if read2fn:
                    print(read2_record.format("fastq")[0:len(read2_record.format("fastq"))-1],file=r2_bcp_filehandle_dict[(primerbc,transbc)])
            else: #if there is no match, print reads to undetermined file
                print(read1_record.format("fastq")[0:len(read1_record.format("fastq"))-1],file= r1Undet_filehandle)
                print(read2_record.format("fastq")[0:len(read2_record.format("fastq"))-1],file=r2Undet_filehandle)
        else: #if there is no match, print reads to undetermined file
            print(read1_record.format("fastq")[0:len(read1_record.format("fastq"))-1],file=r1Undet_filehandle)
            print(read2_record.format("fastq")[0:len(read2_record.format("fastq"))-1],file=r2Undet_filehandle)
    #print QC values to file
    print("There were "+str(total_reads)+" total reads",file=qc_filehandle)
    print(str(reads_with_transposon_seq/float(total_reads))+" of the reads had a transposon sequence.",file=qc_filehandle)
    print(str(matched_reads/float(total_reads))+" of the total reads also had matched barcodes.",file=qc_filehandle)
    print("\n",file=qc_filehandle)
...
def parse_args(args=None):
    ...
    parser.add_argument('-r2','--read2',help='Read2 filename (full path)',required=True)
    parser.add_argument('-b','--barcodefile',help='barcode filename (full path)'\
            ,required=False,default='../scripts/barcodes.txt')
    parser.add_argument('-o','--output',help='path to output directory',\
            required=False,default='../raw/')
...
def main(args = None):
    ...
    if not args.output[-1] == "/":
        args.output = args.output+"/"
    filter_reads(args.read1,args.read2,args.barcodefile,args.output,int(args.hammp),int(args.hammt))
