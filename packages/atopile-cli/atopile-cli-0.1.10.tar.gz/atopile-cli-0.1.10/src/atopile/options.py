...
def get_container_project_dir(val):
    # if it's not set, assume we're not in a docker container already and the normal logic to get
    # the project dir will work
    if val == None:
        return project_dir.value
    else:
        # we can't use get_project_dir here because this option is used when we're already in a 
        # docker container, where our paths are already different and this resolution won't work. 
        # We need to trust the user in this case.
        return val
