...
class Stage:
    ...
    async def run(self, docker: aiodocker.Docker, inputs: Dict[str, Union[StageIO, List[StageIO]]]) -> bool:
        run_log = stages_log.getChild(self.name)
    ...
    def from_dict(cls, config_data: dict, name: str, data: dict):
        ...
        if stage_def_path:
            try:
                stage_def_resource = resources.interpret_ref(stage_def_path, config_data)
            ...
            inputs, outputs = cls.ios_from_stage_def(name, stage_def, data)

        # "simple" stages
        if image:
            command = data.get('command')
            if not command:
                stage_log.error(f'No command provided for {name}')
                raise utils.AtopileError
