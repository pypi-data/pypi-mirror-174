...
class Block:
    ...
    def _check_step(self, context, block_context):
        if self.executor.step_by_step:
            if self.executor.selected:
                if self.block_id == self.executor.selected:
                    raise StepForward(self.block_id, context, block_context, self.executor.workspace_name)
            elif self.executor.next_step:
                raise StepForward(self.block_id, context, block_context, self.executor.workspace_name)
            elif self.block_id == self.executor.step and self.executor.step_workspace == self.executor.workspace_name:
                if self.executor.next_step is False:  # встаем на этом шаге
                    raise StepForward(self.block_id, context, block_context, self.executor.workspace_name)
                else:
                    self.executor.next_step = True  # встаем на следующем шаге

    def _set_next_step(self):
        if self.executor.step_by_step:
            if self.block_id == self.executor.step and self.executor.step_workspace == self.executor.workspace_name:
                self.executor.next_step = True

    def _set_step(self):
        if self.executor.step_by_step:
            # if self.executor.step != self.block_id:
            self.executor.next_step = False
            self.executor.step = self.block_id
    ...
    def get_variable(cls, context, name):
        try:
            return context.debug['__thread_vars'][name]
        except KeyError:
