...
class KeyPoint2DParse(BaseParse):
    ...
    def parse_class_domain(self):
        ...
        for task in tqdm(self._data_info["tasks"], desc="parsing class domain..."):
            ...
            for category_info in catalog:
                ...
                num_points = category_info.get("num_points", 0)
                if num_points == 0:
                    continue
                domain_name = FileIO.clean(f"KeyPoint_{category_name}_ClassDom")
                points_names = category_info.get("point_names", None)
                skeleton = category_info.get("skeleton", None)
                ...
                if points_names:
                    ...
                    for point_ind in range(1, num_points + 1):
                        this_point_label = Label(f"point_{point_ind}")
                        this_point_label.add_supercategory(this_label)
                        this_point_domain.add_label(this_point_label, ori_id=point_ind)
    ...
    def parse_ann_info(self):
        ...
        if len(object_struct.fields):  # 如果有 object field的话
            ...
            return (object_struct, sample_struct), [
                self._parent_class_domain] + parent_class_domains + class_domains, sample_container
