...
def _rc_verify_current_software_version_task(
    task: Task, desired_version: str, verbose: bool = False
) -> Tuple[str, bool]:
    """
    TBD
    """
    ...
    if response.status_code == 200:
        ...
        if desired_version in current_version:
            ...
            custom_result = result_verbose if verbose else result_summary

            # Return the custom result string and the boolian to idicate if the task failed
    ...
    # If the RESTCONF call was not successful
    # Return the custom result string and the boolian to idicate if the task failed
    return custom_result, True
...
def rc_verify_current_software_version_task(task: Task, verbose=False) -> Result:
    ...
    # Try to verify the current software version with RESTCONF
    try:
        custom_result, failed = _rc_verify_current_software_version_task(
            task=task,
            desired_version=desired_version,
            verbose=verbose,
        )

        # Return the custom Nornir result
        return Result(host=task.host, custom_result=custom_result, failed=failed)

    except Exception as error:  # pylint: disable=broad-except
        custom_result = "Exception raised for: _rc_verify_current_software_version_task\n\n" + error
