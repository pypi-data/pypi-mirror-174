...
class DataSource:
    ...
    * Method for training a model against a datasource.
    ...
    Here's an example of using the DataSource class's methods to create a new datasource instance and display its first few records.
    ...
    Example:
        ...
        # When the progress is finished, run the following command to display the
        # head of the data to ensure everything is fine.
        print(ds.head())
        ```
    """
    ...
    def dtypes(self) -> pd.DataFrame:
        ...
        Here's an example to get the dtypes of a datasource:
        ...
        Example:
            ...
            # Optional Step: Create a new datasource
            ...
            # To get the dtypes of an existing datasource, create a new instance of the
            # DataSource and pass the existing datasource's uuid.
            existing_ds = DataSource(uuid=ds.uuid)
            print(existing_ds.dtypes)
            ```
        """
    ...
    def ls(
        ...
        Here's an example of displaying the details of datasources created by the currently logged-in user:
        ...
        Example:
            ...
            # Get the list of all DataSource instances created by the currently logged-in user
            dsx = DataSource.ls()
            print(dsx)
...
def is_ready(
    ...
    Here's an example of checking the processing status of a datasource:
    ...
    Example:
        ...
        # Check the ready status of the datasource
        print(ds.is_ready())
        ...
        # Check the ready status of the datasource
        # If the processing is successful, True will be returned
        print(ds.is_ready())
        ```
    """
...
def wait(self: DataSource, sleep_for: Union[int, float] = 1, timeout: int = 0):
    ...
    Here's an example of preventing further calls from being executed until the datasource processing is complete:
    ...
    Example:
        ...
        # Further calls to the API will be blocked until the datasource
...
def delete(self: DataSource) -> pd.DataFrame:
    ...
    Here's an example to delete a datasource from the server:
    ...
    Example:
        ...
        # Print the uuid of the newly created datasource
        print(ds.uuid)

        # Delete an existing datasource from the server
        # In this case, a new instance of the DataSource is created on purpose
        # to demonstrate that you can use uuid to delete any existing datasource.
        ds_to_delete = DataSource(ds.uuid)
        ds_to_delete.delete()

        # Check whether the datasource has been deleted
        # An exception is raised if the details method is called on a deleted
        # datasource instance.
        try:
            ds = DataSource(ds.uuid)
            ds.details()
...
def details(self: DataSource) -> pd.DataFrame:
    ...
    Here's an example to get details of a datasource from the server:
    ...
    Example:
        ...
        # Create a datasource
        # In this example, the datasource will be stored in an Azure Blob Storage.
        # The region is set to westeurope (default), feel free to change the cloud
        # provider and the region to suit your needs.
        db = DataBlob.from_s3(
            uri="{fill in uri}",
        ...
        # Print the details of the newly created datasource
        # In this case, a new instance of the DataSource is created on purpose
        # to demonstrate that you can use uuid to get details of any
        # existing datasource.
        ds_details = DataSource(ds.uuid)
        print(ds_details.details())

        # To display the details of all datasource created by the user, use
        # the method below.
...
def tag(self: DataSource, name: str) -> pd.DataFrame:
    ...
    Here's an example to tag a datasource:
    ...
    Example:
        ...
        # Print the tag set to the newly created datasource.
        print(ds.details()["tags"][0])

        # Update the tag name for the datasource
        # In this case, a new instance of the DataSource is created on purpose
        # to demonstrate that you can use uuid to tag any existing datasource.
        ds_to_tag = DataSource(ds.uuid)
        ds_to_tag.tag(name="{fill in tag_name}")

        # Print the tag set to the newly created datasource.
        print(ds_to_tag.details()["tags"][0])
        ```
    """
...
def head(self: DataSource) -> pd.DataFrame:
    ...
    Here's an example to display the first few records of the datasource:
    ...
    Example:
        ...
        # Print the head of the datasource
        # In this case, a new instance of the DataSource is created on purpose to
        # demonstrate that you can use uuid to get head of any existing datasource.
        ds_head = DataSource(ds.uuid)
        print(ds_head.head())
        ```
    """
...
def train(
    ...
    *,
    ...
    Example:
        ...
        # Train a model against the datasource.
        # In this case, a new instance of the DataSource is created on purpose to
        # demonstrate that you can use uuid to train a model against any existing
        # datasource. This example predicts which users will perform a purchase
        # event ("*purchase") three hours before they actually do it.
        ds_to_train = DataSource(ds.uuid)
        model = ds_to_train.train(
