...
class Mysql():
    ...
    def __init__(self, configuraion_file=configuration_file, section="mysql"):
        ...
        try:
            ...
            self.default_dabase_name = database_str
            charset = config.get(section, "charset")
    ...
    def is_legal_value(self,table_name,column_name,val):
        ...
        if "." not in table_name:
            table_name  = "{}.{}".format(self.default_dabase_name,table_name)
        col_name = "{}.{}".format(table_name,column_name)
        if self.columns_dict is None:
            return True
        row = self.columns_dict.get(col_name)
        data_type = row.get("DATA_TYPE")
        if data_type in ['int','tinyint','bigint'] and not str(val).isnumeric():
            logger.warning("{} {} is not a valid int".format(column_name,val))
            return False
        return True
