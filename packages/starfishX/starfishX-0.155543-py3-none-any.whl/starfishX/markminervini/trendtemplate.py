def trendMA_YMF(symbol,start,plot=True):
  df = obj.getPrice(symbol,start=start)
...
def trendMA_Year(symbol,start,month=3,plot=True):
...
def check52WeekHighLow(symbol,start,plot=True):
    df = obj.getPrice(symbol,start=start)
...
def ROC_Basket(basket,start):
  pct_change = []
  close_price = []
  close_start = []
  for i in basket:
    df = obj.getPrice(i,start)
