...
def plot_posterior_distribution(proc_saver,
    ...
    framewidth: width of the frame line -> float, default = 1.2
    barwidth: the width of bars in the bar plot -> float, default = 0.8
    ...
    if reverse:
        ...
        ax1.bar(barx, bary,width=barwidth,color=color)
    else:
        ax1.plot(sorted_sample_val[:, variable_id], cum_sample[:, variable_id],color=color)
        ax2.bar(barx, bary,width=barwidth,color=subcolor)
...
def plot_3d_posterior_bars(proc_results,
                           variable_id,
                           obj_path="posterior_bar3d.pdf",
                           figsize=(8, 8),
                           dpi=400,
                           alpha=0.3,
                           title=" ",
    ...
    r_list = [res1, res2]
    plot.plot_3d_posterior_bars(r_list,variable_id=0,obj_path="bars3d.jpg",y_ticklabels=["SSA","GWO"])
    """
    ...
    for c,z,id in zip(cs[0:num_res],zs[0:num_res],idx):
        ...
        ax.bar(xs ,ys,zs=z,zdir="y",color=bar_c,alpha=alpha)
