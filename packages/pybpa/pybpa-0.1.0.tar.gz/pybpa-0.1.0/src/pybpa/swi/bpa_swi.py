...
class SWI:
    ...
    def ref_to_coi(self, swx):
        ...
        g = np.array([gs[i][ANGLE] for i in range(len(self.linked_dat.gen_name))])
        """g: angle: time * gen"""
    ...
    def get_label(self, swx):
        ...
        if g:
            ...
            '''
            temp_g = pd.concat([g[i][ANGLE] for i in range(len(self.linked_dat.gen_name))], axis=1, ignore_index=True)
            temp_g.columns = [i for i in range(len(self.linked_dat.gen_name))]
...
if __name__ == '__main__':
    ...
    folder_path = r'E:/39bpa'
    # folder_path = r'D:\OneDrive\桌面\总调项目\20191112100152_save_1'
    # temp_dat = DAT.build_from_folder(folder_path + op)
    swi = SWI.build_from_folder(r'E:\Data\transient_stability\39rd_gen\rd_gen2')
    # p = [i for i in os.listdir(folder_path + op) if '.swx' in i.lower()]
    # for f in p:
    #     print(f)
    #     swx, pfs, label  = swi.read_swx(folder_path + op + '/' + f, temp_dat)
    swx, pfs, label = swi.read_swx(r'E:\Data\transient_stability\39rd_gen\rd_gen2\2_115_0_31_0/0(1-0).SWX')
    #         break
    # swx, pfs, label  = swi.read_swx(r'D:\OneDrive\桌面\PsdEdit\a\0_105_0\0(6-266).SWX', temp_dat)
    # pfo_path = r'D:\OneDrive\桌面\PsdEdit\a\0_105_0\0_105_0.pfo'
    # pfo = temp_dat.read_pfo(pfo_path)
