...
def run_kaks(sp_list,step1out,args,config,step4out,step5out,gene_pairs_idmap):
    ...
    trimal=config.get("software", "trimal")
    ...
    if not args.only_script:
        ...
        for pair in gene_pairs_idmap:
            tmp=(pair,pepmap,cdsmap,KaKs_Calculator,Kaks_aligncmd,Epal2nal,trimal,op)
            arg_list.append(tmp)
        sh_pool=Pool(args.t)
...
def run_ks(args):
    sub_sh(args[0],args[1],args[2],args[3],args[4],args[5],args[6],args[7])
...
def sub_sh(pair,pepmap,cdsmap,KaKs_Calculator,Kaks_aligncmd,Epal2nal,trimal,op):
    out=[]
