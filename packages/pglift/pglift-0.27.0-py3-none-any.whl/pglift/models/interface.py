...
class BaseRole(Manifest):
    ...
    state: PresenceState = Field(
        const=True, default=PresenceState.absent, description=("Role state.")
    )
    ...
    pgpass: bool = Field(
        const=True,
        default=False,
        description="Add an entry in password file for this role.",
    )
...
class Role(_RoleExisting, RoleDropped):
    ...
    @validator("drop_owned", "reassign_owned")
    def __validate_drop_reassign_owned(
        cls, value: Union[bool, str], values: Dict[str, Any], field: fields.ModelField
    ) -> Union[bool, str]:
        """Make sure drop_owned or reassign_owned are set only for roles
        with absent state.
...
class Instance(BaseInstance):
    ...
    def composite(cls: Type[_T], pm: "PluginManager") -> Type[_T]:
        ...
        for m in pm.hook.interface_model():
            sname = m.__service__
            sdesc = m.__doc__ or f"Configuration for the {sname} service"
            if sname in fields:
            ...
            for f in m.__fields__.values():
                ...
                if f.required:
                    f = Field(required=True, description=sdesc)
                    break
            else:
