...
def upgrade(
    ...
    *,
    ...
    apply(ctx, new_manifest, _creating=True, _upgrading=True)
...
def apply(
    ...
    *,
    ...
    with configure_context(
        ctx, instance, _creating=_creating, _upgrading=_upgrading
    ) as changes:
        if _creating:
...
def _get(ctx: "BaseContext", instance: system.Instance) -> interface.Instance:
    ...
    if instance.standby:
        try:
            ...
            kw: Dict[str, Any] = {
                "primary_conninfo": instance.standby.primary_conninfo,
    ...
    if instance_is_running:
        ...
        with db.connect(ctx, instance, dbname="postgres") as cnx:
            extensions += [
                e for e in db.installed_extensions(cnx) if e not in extensions
            ]
    else:
        try:
