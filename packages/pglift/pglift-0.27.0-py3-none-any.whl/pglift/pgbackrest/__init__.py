...
def site_configure_install(settings: "Settings") -> None:
    ...
    s.configpath.mkdir(mode=0o750, exist_ok=True, parents=True)
...
def system_lookup(
    ...
    try:
        stanza = impl.stanza(instance, settings)
    except exceptions.ConfigurationError as exc:
        logger.debug(
            "failed to read pgbackrest configuration for %s: %s", instance, exc
        )
        return None
    else:
        return models.Service(stanza=stanza)
...
def instance_configure(
    ...
    stanza = service_manifest.stanza or instance.qualname
    if creating and not upgrading and impl.enabled(stanza, settings):
        if not ctx.confirm(
        ...
        impl.revert_init(ctx, instance, stanza, settings)
        impl.revert_setup(ctx, stanza, settings, config, instance.datadir)

    impl.setup(ctx, stanza, settings, config, instance.datadir)

    info = impl.backup_info(ctx, stanza, settings)
    password = None
    ...
    if not info or info[0]["status"]["code"] == 1:
        impl.init(ctx, instance, stanza, settings, password)

    if upgrading:
        impl.upgrade(ctx, instance, stanza, settings, password)
...
def instance_drop(ctx: "BaseContext", instance: system.Instance) -> None:
    ...
    try:
        ...
        return
    assert impl.enabled(stanza, settings)

    nb_backups = len(impl.backup_info(ctx, stanza, settings)[0]["backup"])
    ...
    if not nb_backups or ctx.confirm(
        ...
        impl.revert_init(ctx, instance, stanza, settings)
        impl.revert_setup(ctx, stanza, settings, instance.config(), instance.datadir)
