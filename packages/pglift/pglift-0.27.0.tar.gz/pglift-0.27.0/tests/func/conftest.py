...
@pytest.fixture(scope="package")
def p_settings(
    settings: Settings, patroni_execpath: Optional[pathlib.Path]
) -> Optional[Settings]:
    """Settings with 'patroni' component enabled, if available."""
    if patroni_execpath is None:
        return None
    s = settings.dict()
    s.update({"patroni": {"execpath": patroni_execpath, "loop_wait": 1}})
    return Settings.parse_obj(s)


@pytest.fixture(scope="package")
def any_settings(settings: Settings, p_settings: Settings) -> Settings:
    return p_settings or settings
...
def _installed(
    ...
    if systemd_requested:
        assert any_settings.service_manager == "systemd"

    custom_settings = tmp_path / "settings.json"
    custom_settings.write_text(any_settings.json())
    _install.do(
        any_settings,
