...
@pytest.fixture
def patroni_settings(p_settings: Settings) -> PatroniSettings:
    return patroni.get_settings(p_settings)


@pytest.fixture
def ctx(p_settings: Settings) -> Context:
    return Context(settings=p_settings)
...
@pytest.fixture(scope="module")
def instance1(
    p_settings: Settings, instance1_manifest: interface.Instance
) -> Iterator[system.Instance]:
    yield from _make_instance(p_settings, instance1_manifest)
...
@pytest.fixture(scope="module")
def instance2(
    p_settings: Settings, instance2_manifest: interface.Instance
) -> Iterator[system.Instance]:
    yield from _make_instance(p_settings, instance2_manifest)
...
def test_start_stop(
    ...
    use_systemd = p_settings.service_manager == "systemd"
