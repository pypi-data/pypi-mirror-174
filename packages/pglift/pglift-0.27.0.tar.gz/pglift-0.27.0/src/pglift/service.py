...
def start(ctx: "BaseContext", service: "Runnable", *, foreground: bool) -> None:
    """Start a service.
    ...
    """
    if foreground:
        cmd.execute_program(service.args(), env=service.env(), logger=logger)
        return
    ...
    if cmd.status_program(pidfile) == cmd.Status.running:
        ...
        return
    cmd.Program(service.args(), pidfile, env=service.env(), logger=logger)


def stop(ctx: "BaseContext", service: "Runnable") -> None:
    """Stop a service.
