...
@pytest.mark.usefixtures(
    "clear_notebooks", "clear_notebook_directory"
)
class TestSaveAs:
    @pytest.mark.asyncio
    ...
    async def test_rename_different_directory(
        ...
        assert os.path.exists(f"{settings.ROOT_DIR}/simple.ipynb")
        assert not os.path.exists(f"{other_notebook_directory}/awholenewname.ipynb")
        ...
        async with client.websocket_connect(
            ...
            metadata = data["metadata"]
            assert (
                resolve(metadata["jupyter_d1"]["path"])
                == resolve(f"{other_notebook_directory}/awholenewname.ipynb")
            )
            assert metadata["jupyter_d1"]["name"] == "awholenewname"

            assert os.path.exists(f"{other_notebook_directory}/awholenewname.ipynb")
            assert not os.path.exists(f"{settings.ROOT_DIR}/simple.ipynb")
