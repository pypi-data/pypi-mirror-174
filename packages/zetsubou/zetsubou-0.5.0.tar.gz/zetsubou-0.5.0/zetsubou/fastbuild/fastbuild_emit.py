...
def get_min_max_msvc_version(proj : ProjectTemplate) -> Tuple[int, int]:
    vmin = None
    vmax = None
    for tool in proj.toolchains:
        if vmin is None or tool.version < vmin:
            vmin = tool.version
        if vmax is None or tool.version > vmax:
            vmax = tool.version
    return (vmin, vmax)
...
class FastbuildEmitter(Emitter):
    ...
    def fill_base_project(self, context : EmitContext, fbuild: target_kinds.LinkableTarget, target: ResolvedTarget, target_variant: TargetVariant, toolchain : Toolchain):
        ...
        fbuild.LinkerOptions += list_to_string(libs)
        fbuild.LinkerOptions += list_to_string( [ f"/LIBPATH:\"{l}\"" for l in lib_paths ])
    ...
    def fill_object(self, context : EmitContext, fbuild: target_kinds.ObjTarget, target: ResolvedTarget, target_variant: TargetVariant, root_dir : str, toolchain : Toolchain, source_path : str, includes : List[str]):
        ...
        # Append defines as compiler flags with /D prefix
        defines = join_unique([
            target_variant.defines.public,
            target_variant.defines.private
        ])

        fbuild.CompilerOptions += list_to_string( [ f"/D{d}" for d in defines ] )

        # Append includes as compiler flags with /I prefix
        fbuild.CompilerOptions += list_to_string([ f"/I\"{i}\"" for i in includes ])

        # out_path = fs.path.join( fs.path.dirname(target.target.get_loaded_from_file()), source_path )
    ...
    def gather_includes(self, context : EmitContext, target : ResolvedTarget, target_variant : TargetVariant, toolchain : Toolchain) -> List[str]:
    ...
    def emit_static_library(self, context: EmitContext, target: ResolvedTarget, target_name : str, project_configs_list : str) -> List[str]:
        ...
        for config_variant in context.config_matrix.variants:
            ...
            with create_bff_writer() as writer:
                ...
                self.emit_buildable_target_variant(context, writer, static_lib, obj_list, target_variant, project_configs_list, config_includes)
    ...
    def emit_linkable_target(self, context: EmitContext, target: ResolvedTarget, target_name : str, project_configs_list : str, ctor : Callable[[], target_kinds.LinkableTarget]) -> List[str]:
        ...
        for config_variant in context.config_matrix.variants:
            ...
            with create_bff_writer() as writer:
                ...
                self.emit_buildable_target_variant(context, writer, lnk_target, obj_list, target_variant, project_configs_list, config_includes)
    ...
    def emit_toolchain(self, context: EmitContext, toolchain: Toolchain):
        ...
        with bff_struct(writer) as struct:
            struct.write_object(toolchain, hidden_fields=['name', 'Compiler', 'CompilerFamily'])
