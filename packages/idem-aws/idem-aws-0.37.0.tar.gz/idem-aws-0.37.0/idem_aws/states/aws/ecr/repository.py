...
async def search(
    ...
    """Use an un-managed AWS ECR repository as a data-source.
    ...
    """
    hub.log.warning(
        f"aws.ecr.repository.search '{name}' state has been deprecated. Please use exec.run with aws.ecr.repository.get instead."
    )
    result = dict(comment=(), old_state=None, new_state=None, name=name, result=True)
