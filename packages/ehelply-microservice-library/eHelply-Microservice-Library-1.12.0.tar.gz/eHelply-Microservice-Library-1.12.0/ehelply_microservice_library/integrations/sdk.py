...
class SDK(Integration):
    ...
    @staticmethod
    def make_sam_client(include_default_headers: bool = True) -> ApiClient:
        config = Configuration(
            host=get_fact_endpoint("ehelply-sam")
        )
        return make_api_client(config, include_default_headers)

    @staticmethod
    def make_experimental_sam_client(include_default_headers: bool = True) -> ExperimentalApiClient:
        config = ExperimentalConfiguration(
            host=get_fact_endpoint("ehelply-sam")
        )
    ...
    @staticmethod
    def make_products_client(include_default_headers: bool = True) -> ApiClient:
        config = Configuration(
            host=get_fact_endpoint("ehelply-products")
        )
        return make_api_client(config, include_default_headers)

    @staticmethod
    def make_experimental_products_client(include_default_headers: bool = True) -> ExperimentalApiClient:
        config = ExperimentalConfiguration(
            host=get_fact_endpoint("ehelply-products")
        )
        return make_experimental_api_client(config, include_default_headers)
