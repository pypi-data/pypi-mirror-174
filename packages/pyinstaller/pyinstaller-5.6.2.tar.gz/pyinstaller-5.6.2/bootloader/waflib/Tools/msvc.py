...
all_msvc_platforms = [('x64', 'amd64'), ('x86', 'x86'), ('ia64', 'ia64'), ('x86_amd64', 'amd64'), ('x86_ia64', 'ia64'),
                      ('x86_arm', 'arm'), ('x86_arm64', 'arm64'), ('amd64_x86', 'x86'), ('amd64_arm', 'arm'),
                      ('amd64_arm64', 'arm64')]
all_wince_platforms = [('armv4', 'arm'), ('armv4i', 'arm'), ('mipsii', 'mips'), ('mipsii_fp', 'mips'),
                       ('mipsiv', 'mips'), ('mipsiv_fp', 'mips'), ('sh4', 'sh'), ('x86', 'cex86')]
...
def setup_msvc(conf, versiondict):
    ...
    if platforms == ['']:
        platforms = Utils.to_list(conf.env.MSVC_TARGETS
                                  ) or [i for i, j in all_msvc_platforms + all_icl_platforms + all_wince_platforms]
...
def gather_wince_supported_platforms():
    ...
    try:
        ce_sdk = Utils.winreg.OpenKey(
            Utils.winreg.HKEY_LOCAL_MACHINE, 'SOFTWARE\\Wow6432node\\Microsoft\\Windows CE Tools\\SDKs'
        )
    except OSError:
        try:
            ce_sdk = Utils.winreg.OpenKey(
                Utils.winreg.HKEY_LOCAL_MACHINE, 'SOFTWARE\\Microsoft\\Windows CE Tools\\SDKs'
            )
        except OSError:
            ce_sdk = ''
    if not ce_sdk:
    ...
    while 1:
        ...
        for arch, compiler in all_wince_platforms:
            if os.path.isdir(os.path.join(path, device, 'Lib', arch)):
                platforms.append((
                    arch, compiler, os.path.join(path, device, 'Include',
                                                 arch), os.path.join(path, device, 'Lib', arch)
                ))
        if platforms:
        ...
        if path and os.path.isfile(os.path.join(path, 'bin', 'SetEnv.cmd')):
            ...
            for target, arch in all_msvc_platforms:
                targets[target] = target_compiler(
                    conf, 'wsdk', arch, version, '/' + target, os.path.join(path, 'bin', 'SetEnv.cmd')
                )
...
def gather_msvc_targets(conf, versions, version, vc_path):
    ...
    if os.path.isfile(os.path.join(vc_path, 'VC', 'Auxiliary', 'Build', 'vcvarsall.bat')):
        for target, realtarget in all_msvc_platforms[::-1]:
            targets[target] = target_compiler(
                conf, 'msvc', realtarget, version, target,
                os.path.join(vc_path, 'VC', 'Auxiliary', 'Build', 'vcvarsall.bat')
            )
    elif os.path.isfile(os.path.join(vc_path, 'vcvarsall.bat')):
        for target, realtarget in all_msvc_platforms[::-1]:
            targets[target] = target_compiler(
                conf, 'msvc', realtarget, version, target, os.path.join(vc_path, 'vcvarsall.bat')
            )
    elif os.path.isfile(os.path.join(vc_path, 'Common7', 'Tools', 'vsvars32.bat')):
        targets['x86'] = target_compiler(
            conf, 'msvc', 'x86', version, 'x86', os.path.join(vc_path, 'Common7', 'Tools', 'vsvars32.bat')
        )
...
def gather_msvc_versions(conf, versions):
    ...
    for (v, version, reg) in gather_msvc_detected_versions():
        try:
            ...
            try:
                msvc_version = Utils.winreg.OpenKey(
                    Utils.winreg.HKEY_LOCAL_MACHINE, "SOFTWARE\\Wow6432node\\Microsoft\\VisualStudio\\SxS\\VS7"
                )
                    ...
                    for vscomntool in ('VS110COMNTOOLS', 'VS100COMNTOOLS'):
                        if vscomntool in os.environ:
                            ...
                            if (
                                ...
                                and not os.path.exists(vs_express_path) and not os.path.exists(dev_env_path)
                            ):
                                Logs.warn((
...
def find_msvc(conf):
    ...
    compiler_name, linker_name, lib_name = _get_prog_names(conf, compiler)
    v.MSVC_MANIFEST = (compiler == 'msvc'
                       and version >= 8) or (compiler == 'wsdk'
                                             and version >= 6) or (compiler == 'intel' and version >= 11)
