...
class Configurations(Constants):
    ...
    def _help(self):
        print("""
        Refer Below for Supported Commands:

        - create : Creates a DB, fails if DB already exists.

        - delete : Deletes the Current DB, Use with Caution.

        - recreate : A helper that deletes then create the DB.

        - help : Show Supported DB Operations.
        """)

    def _recreate(self):
        self._config_database_manager('delete')
        self._config_database_manager('create')

    def _delete(self):
        if exists(self.DATABASE_PATH):
            remove(self.DATABASE_PATH)
            print(self.DATABASE_PATH, " Deleted!")
        else:
