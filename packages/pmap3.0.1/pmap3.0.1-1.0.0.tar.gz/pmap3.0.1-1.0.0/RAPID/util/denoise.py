...
class UNet(nn.Module):
    ...
    Args:
        inchans (int): Number of neurons in the input layer of the network.
        outchans (int): Number of neurones  in the final layer of the network.
    """
    ...
    def __init__(self, inchannels=1, outchannels=2):
        ...
        self.block00 = UNetBlock(inchannels, ncount[0], ncount[0])
        ...
        self.final = nn.Conv2d(ncount[0], outchannels, kernel_size=1)
...
def predict_mask(model, image, bs):
    ...
    features = club_patches(result, modimage)[0:image.shape[0], 0:image.shape[1]]
    return features
