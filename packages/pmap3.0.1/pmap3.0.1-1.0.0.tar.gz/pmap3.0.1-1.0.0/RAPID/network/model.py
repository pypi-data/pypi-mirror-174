...
class Conv1DLayer(nn.Module):
    ...
    def forward(self, X):
        return self.bnconv1d(F.relu(self.conv1d(X)))
...
class RAPIDMixNet(nn.Module):
    ...
    def forward(self, xx):
        ...
        # rr.transpose(0,1).contiguous().view((3,-1)).transpose(0,1)
        inputdata = torch.cat((torch.from_numpy(dataTrain), torch.from_numpy(newdata)), 0)
