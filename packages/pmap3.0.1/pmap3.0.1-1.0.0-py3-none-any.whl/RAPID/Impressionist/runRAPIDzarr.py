...
def test(args, model, device, datapath, outpath, batchsize=100000):
    ...
    # print("GPU::::",os.system("find_my_gpus.pl |cut -c28 |sed -z 's/\\n/lll/g'"))
    # device_ids = [os.system("find_my_gpus.pl |cut -c28 |sed -z 's/\n/,/g'").split(',')]
    # print("GPUs used :",device_ids)
    device_ids = [3, 5, 6, 7]  # @@@@@@@@
    ...
    markermaxval = hf["minmax"][:]
    count = 0
    # run prediction
    with torch.no_grad():

        # define empty array for identified cluster quantification
        samplenames = []

        # generate random colormap
        # col_list = generate_colormap(args.ncluster * 2)
        ### TODO: Uncomment this
        #col_list = generate_colormap(args.ncluster+1)[:-1]
        col_list = generate_colormap(args.ncluster)
        # randomcolor = np.random.choice(len(col_list), int(args.ncluster * 2), replace=False)
        # col_list = col_list[randomcolor, :]
        if (args.predict) and args.color != None:
            col_list = np.load(args.color + "/color.npy")
        totalpixels = hf["data_normalized"].attrs['totalpixels']
        ...
        hf = zarr.open(datapath, 'r+')
        image_list = hf['data_normalized'].attrs['imageslist']
        dataraw = hf["data"]
        Slices = np.zeros((len(imageshapelist), args.ncluster,
                           len(hf["data_normalized"].attrs['selmarkernames']) + 3))
        hftab = hf.create_dataset('tab',
                                  shape=(args.ncluster * len(imageshapelist), hf["data_normalized"].shape[1] + 3),
                                  dtype='f4')
        ...
        with progress(imageshapelist, desc='Image', total=0 if len(imageshapelist) == 1 else None, ) as pbr:
            ...
            for shape in pbr:
                ...
                image_name = os.path.split(image_list[r])[-1]  # [0:-4]
                ...
                data = ndimage.gaussian_filter(data.reshape((imageshapelist[r, 0], imageshapelist[r, 1], hf["data_normalized"][0:10, :].shape[1])), filtersizeG)
                data = data.reshape((int(imageshapelist[r, 0] * imageshapelist[r, 1]), hf["data_normalized"][0:10, :].shape[1]))
                print(data.shape)
                ...
                # for BSTART in progress(pixelinds,desc='Segmenting',total=0 if len(pixelinds) == 1 else None,):
                for BSTART in range(0, TESTPATCHPRED.shape[0], batchsize):
                    # x = torch.from_numpy(TESTPATCHPRED[BSTART:BSTART + (batchsize), :].astype(np.float32)).float().cuda(device_ids[0])
                ...
                backgroundclass = TESTPATCHPREDO[0]
                print("counting unique clusters")
                # unique, counts = np.unique(TESTPATCHPREDO, return_counts=True)

                # geenrate the mean marker intensity table for each cluster
                unique, counts = da.unique(da.from_array(TESTPATCHPREDO, chunks=10000000), return_counts=True)
                unique = unique.compute()
                counts = counts.compute()

                TESTPATCHPRED = TESTPATCHPREDO.reshape((imageshapelist[r, 0], imageshapelist[r, 1]))

                ### TODO: This does it for every image, not necessarily same cluster in all images. Should be cluster with lowest quantified values.
                backgroundclass = TESTPATCHPRED[5, 5]

                uncertainpixelmask = copy.deepcopy(((TESTPATCHPREDO_Prob < 0.1)))
                TESTPATCHPREDO_Prob = np.abs(TESTPATCHPREDO_Prob - 1)
                TESTPATCHPRED_Prob = TESTPATCHPREDO_Prob.reshape((imageshapelist[r, 0], imageshapelist[r, 1])) * 255
                VolImage = np.zeros((imageshapelist[r, 0], imageshapelist[r, 1], 3))
                VolImage1 = np.zeros((imageshapelist[r, 0], imageshapelist[r, 1], 3))
                # TESTPATCHPRED[uncertainpixelmask.reshape(TESTPATCHPRED.shape)]=1000
                TESTPATCHPRED[-args.ncluster:, -1] = np.arange(args.ncluster)

                COUNTR = 0
                cv.imwrite(outpath + "/Pred_" + image_name + "_Gray.png", TESTPATCHPRED.astype(np.uint16))
                # cv.imwrite(Param + str(EPOC) + "_" + Name + "_Prob.png", TESTPATCHPRED_Prob.astype(np.uint8))
                for i in range(len(col_list)):
                    mask = TESTPATCHPRED == i
                    if i == backgroundclass:
                        col_list[COUNTR, :] = (0, 0, 0)
                    VolImage[mask] = col_list[COUNTR, :]
                    VolImage1[mask] = col_list[COUNTR, :]
                    COUNTR += 1
                if not args.GUI:
                    cv.imwrite(outpath + "/Pred_" + image_name + "_Pred.png",
                               cv.cvtColor(VolImage1.astype(np.uint8), cv.COLOR_BGR2RGB))
                tab = vaex.from_pandas(pd.DataFrame(np.hstack([np.expand_dims(TESTPATCHPRED.reshape(-1), 1), dataraw[
                                                                                                             count:count + (
                                                                                                                     imageshapelist[
                                                                                                                         r][
                                                                                                                         0] *
        ...
        np.savetxt(outpath + "/Pred__" + image_name + "_data.txt", X=np.nan_to_num((np.vstack(Slices))))
        my_data = pd.DataFrame(np.nan_to_num((np.vstack(Slices))))
        markers = hf["data_normalized"].attrs['selmarkernames']
        print(markers)
        markers = [marker for marker in markers]
        my_data.columns = np.hstack([["Sample", "Cluster", "Pixels"], markers])
        my_data.to_csv(outpath + "/RAPID_cluster_table.csv")
    tabledata, my_data_scaled, DistMatrix, uniqueClusters = prep_for_mst(clustertable=my_data, minnumpixels=1000,
                                                                         outfolder=outpath, includedmarkers=markers)
    ...
    mst = generate_mst(distancematrix=DistMatrix, normalizeddf=my_data_scaled[final], colors=col_list, randomseed=0,
                       outfolder=outpath, clusterheatmap=True, displaymarkers=hf["data_normalized"].attrs['selmarkernames'],
                       uniqueclusters=uniqueClusters, samplenames=samplenames, displaysingle=False)
    grey = np.array(grey)
...
def train_rapid(args, device, datapath, outputfolder="", patcharray=""):
    ...
    if args.reassume | args.predict:
        if args.predmodpath != None:
            model = load_checkpoint(args.predmodpath, model, optimizer)
        else:
            model = load_checkpoint(outputfolder + '/checkpoint.pth', model, optimizer)
        model.to(device)
    acc = None

    # torch.save(checkpoint, args.rfold + '/checkpoint.pth')
    torch.save(checkpoint, outputfolder + '/checkpoint.pth')
    ...
    for epoch in range(0, args.epoch):
        ...
        if not args.predict:
            if epoch > 10:
                ...
                rescale = False
            if args.SCANloss:
                train_scan(args, model, datapath, optimizer, epoch, outputfolder, args.rescalefactor,
                           checkpoint, noneigh, logger, patcharray)
            else:
                train(args, model, datapath, optimizer, epoch, outputfolder, rescale, args.rescalefactor, checkpoint,
                      noneigh, logger, patcharray)

            if (epoch + 1) % args.testinterval == 0:
                TESTPATCHPREDO, prob, my_data, COL_LIST, samplenames = test(args, model, device, datapath, outputfolder,
                                                                            args.testbs)
        if args.predict:
...
def run_rapid_analysis(args, mst=True, spatial=True):
    ...
    *Spatial*, which helps us to understand the biology behind spatial phenotypic organisation of tissues by
    ...
    my_data = pd.read_csv(rfold + "/RAPID_cluster_table.csv", sep=",", header=0, index_col=0)
    ...
    tabledata, my_data_scaled, DistMatrix, uniqueClusters = prep_for_mst(clustertable=my_data,
                                                                         minnumpixels=10000,
                                                                         includedmarkers=include_names, outfolder=rfold,
    ...
    if mst:
        ...
        print(samplenames.shape)
        mst = generate_mst(distancematrix=DistMatrix, normalizeddf=my_data_scaled[final], colors=color_list,
                           randomseed=0, outfolder=args.rfold, clusterheatmap=True, displaymarkers=displayMarkeOnMst,
...
def get_parameters():
    ...
    parser.add_argument('--normalize', action='store_true', default=False,
                        help="Standadize data (default: %(default)s)")
    parser.add_argument('--normalizeall', action='store_true', default=False,
                        help="Standadize data across sample (default: %(default)s)")
    parser.add_argument('--normtype', type=str, default="none", help="Standadize data (default: %(default)s)")
...
def run_rapid():
    """
    This function in the entry point for the RAPID algorithm. This module/function is called when user run RAPID from the terminal or GUI.
    """
    ...
    if args.train | args.predict:
        ...
        if os.path.exists(zarrpath):
            ...
            file_list, image_shape_list, args = rio.tiff_to_zarr(file_list, zarrpath, gaussianblur=args.gaussianblur,
                                                                 gaussianblurstd=args.gbstd, medianblur=args.medianblur,
                                                                 normalize=args.normalize, normtype=args.normtype,
                                                                 ndims=args.nchannels, deletechannel=delete_channel,
                                                                 normalizeacross=args.normalizeall,
                                                                 markernames=markernames,
                                                                 args=args, pretrained=args.vmodels,
    ...
    if not args.runanalysis:
        ...
        print("Markers are set to " + str(hf['data_normalized'].attrs['selmarkernames']) + ".")
...
def train(args, model, datapath, optimizer, epoch, outpath, rescale, rescalefactor, checkpoint, noneighbors, logger,
    ...
    # empty_array,empty_array_gb, IND = cropandpaste_zarr(datapath,args.patchsize,args.npatches,rescale,rescalefactor,gaussianblurstd=args.gbmi)

    empty_array, empty_array_gb, IND = cropandpaste_zarrSelected(datapath, args.patchsize, args.npatches, rescale,
                                                                 rescalefactor, gaussianblurstd=args.gbmi,
                                                                 patcharray=patcharray, percent_ext=args.percentile_e)
...
def train_scan(args, model, datapath, optimizer, epoch, outpath, rescalefactor, checkpoint, noneighbors, logger,
    ...
    # generate random crops from multiple images from multiple input images stored in zarr file
    empty_array, empty_array_gb, IND = cropandpaste_zarr(datapath, args.patchsize, args.npatches, False, rescalefactor,
                                                         gaussianblurstd=args.gbmi, patcharray=patcharray)
