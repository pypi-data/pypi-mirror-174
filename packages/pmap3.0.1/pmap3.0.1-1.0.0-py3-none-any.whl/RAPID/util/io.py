...
def tiff_to_zarr(file_list, zarrpath, gaussianblur=False, gaussianblurstd=None, medianblur=False, normalize=False,
    ...
    >>> from RAPID.util.io import tiff_to_zarr
    >>> module_path = dirname(__file__)
    >>> tifffile = module_path+"/../Data/MouseLN/"
    >>> tiff_to_zarr(tifffile,"/tmp/zarr/", gaussianblur=False,gaussianblurstd=None,normalize=False,ndims=None,deletechannel="",normalizeacross=False,markernames=None)
    >>> hf = zarr.open("/tmp/zarr/", mode='r')
    >>> data = np.array(hf["data_normalized"])
    >>> print(data.shape)
    """
    ...
    # totalpixel = getdatashape(file_list)
    totalpixel = 0
    for file in file_list:
        isize = tifffile.imread(file).shape
        totalpixel += int(np.prod(isize) / isize[1])
    ...
    # unnormalized = np.zeros((totalpixel, args.nchannels), dtype='uint8')
    ...
    for file in range(len(file_list)):
        ...
        imgind += 1
        # imglist.append(os.path.split(file_list[file])[-1].split(".")[0])
        ...
        if ndim > 3:
            ...
            for zch in range(readslice.shape[0]):
                ...
                fhd[lastindex: lastindex + readslice_z.shape[0] * readslice_z.shape[1], :] = readslice_z.reshape(
                ...
                if (not normalizeacross) & (normalize):
                    fhd[lastindex: lastindex + readslice.shape[0] * readslice_z.shape[1], :] = readslice_z.reshape(
                        (-1, readslice_z.shape[2]))
            ...
            fhd[lastindex: lastindex + readslice.shape[0] * readslice.shape[1], :] = readslice.reshape(
                (-1, readslice.shape[2]))
            fhdr[lastindex: lastindex + readslice.shape[0] * readslice.shape[1], :] = readslice.reshape(
                (-1, readslice.shape[2]))
            readslice = preprocess(zarrpath, medianblur=medianblur, gaussianblur=gaussianblur,
                                   gaussianblurstd=gaussianblurstd, data_array=readslice, normtype=normtype,
                                   normalize_first=False, submean=args.submean, mbk=3)
            if (not normalizeacross) & (normalize):
                fhd[lastindex: lastindex + readslice.shape[0] * readslice.shape[1], :] = readslice.reshape(
                    (-1, readslice.shape[2]))
    ...
    os.mkdir(os.path.join(zarrpath, "ImageData"))
    hf = zarr.open(os.path.join(zarrpath, "ImageData"), mode='w')
    currentmarkers = np.zeros(
        (len(imglist), int(np.max(imgshapes[:, 0])), int(np.max(imgshapes[:, 1])), len(markernames[0])),
        dtype=dt)
    for i in range(len(markernames[0])):
        currentmarker = np.zeros(
            (len(imglist), int(np.max(imgshapes[:, 0])), int(np.max(imgshapes[:, 1]))), dtype=dt)
        for j in range(len(imglist)):
            currentmarker[j, : int(imgshapes[j, 0]), : int(imgshapes[j, 1])] = imgslist[j][:int(
                imgshapes[j, 0]), :int(imgshapes[j, 1]), i]
        currentmarkers[:, :, :, i] = currentmarker
        hf.create_dataset(str(i + 1) + "_" + markernames[0][i], data=currentmarker, dtype='i4')
    ...
    if normalizeacross:
        ...
        if args.pca:
            ...
            try:
                ...
                print(ex)
            fhd = hf.create_dataset('data_normalized', shape=(
                totalpixel, np.asarray(df_trans[PCA__cols][0:10, :len(trainmarkernums)]).shape[1]), dtype='f8')
            ...
            # fhd[:,:] = np.asarray(df_trans[PCA__cols])[:,:len(trainmarkernums)]
        else:
            hf = zarr.open(zarrpath, 'r+')
            try:
                del hf['data_normalized']
            ...
            scaled__cols = [col for col in scalar_trans.column_names if 'scaled_' in col]
            fhd = hf.create_dataset('data_normalized', shape=(
                totalpixel, np.asarray(scalar_trans[scaled__cols][0:10, :len(trainmarkernums)]).shape[1]), dtype='f8')
            for btch in range(0, np.asarray(scalar_trans[scaled__cols][0:10, :len(trainmarkernums)]).shape[1],
                ...
                btchsize
                fhd[btch:btch + btchsize, :] = np.asarray(tmpdata)
            # scaler.fit(da.from_array(fhd[:lastindex, :]))

    # imgshapes = np.vstack(imgshapes)
    fh.attrs['flipimg'] = flipimg
    # fh.create_dataset('imageshapelist', data=imgshapes, dtype='i4')
    fhd.attrs['totalpixels'] = lastindex
    fhd.attrs['percentile'] = percentlist

    ### TODO: DO we use this?
    fhd.attrs['numMarkers'] = ndims
    if not deletechannel.any() == 1000:
        fhd.attrs['selmarkernames'] = np.delete(markernames, deletechannel).tolist()
    else:
        fhd.attrs['selmarkernames'] = markernames
    # fhd.attrs['markers'] = markernames
    fhd.attrs['imageslist'] = imglist
