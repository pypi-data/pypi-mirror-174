...
def cropandpaste_zarrSelected(zarrpath, patchsize, numberofpatches, rescale, rescalefactor, gaussianblurstd=0.5,
    ...
    normdata = hf["data_normalized"]
    rawdata = hf["data"]
    shapelist = hf["imageshapelist"][:, :]
    print("Shapelist ...", shapelist)

    if percent_ext == None:
        percentarray = hf["data_normalized"].attrs['percentile']
    else:
        percentarray = [float(V) for V in percent_ext.split(",")]

    print("percentarray ...", percentarray)
    emptyarray = np.zeros((numberofpatches * len(shapelist), patchsize, patchsize, normdata.shape[1]))
    patchcount = 0
    pixelscount = 0
    ...
    if patcharray == "":
        for i in range(len(shapelist)):
            normimg = normdata[pixelscount:int(pixelscount + (shapelist[i, 0] * shapelist[i, 1])), ].copy()
    ...
    EMPTY_ARRAY = np.zeros((patchsize, patchsize * patchcount, normdata.shape[1]))
    EMPTY_ARRAY_G = np.zeros((patchsize, patchsize * patchcount, normdata.shape[1]))
    if rescale:
        EMPTY_ARRAY = np.zeros(
            (int(patchsize * rescalefactor), int(patchsize * rescalefactor) * patchcount, normdata.shape[1]))
...
def cropandpaste_zarr(zarrpath, patchsize, numpatches, rescale, rescalefactor, gaussianblurstd=0.5, patcharray=""):
    ...
    normdata = hf["data_normalized"]
    shapelist = hf["imageshapelist"][:, :]
    emptyarray = np.zeros((numpatches * len(shapelist), patchsize, patchsize, normdata.shape[1]))
    patchcount = 0
    pixelscount = 0
    if patcharray == "":
        for i in range(len(shapelist)):
            TMP = normdata[pixelscount:int(pixelscount + (shapelist[i, 0] * shapelist[i, 1])), ].copy()
        ...
        for i in range(len(shapelist)):
            ...
            for patN in range(len(patcharray[i])):
                startx = patcharray[i][patN][0]
                starty = patcharray[i][patN][1]
                emptyarray[patchcount:patchcount + 1, :, :, :] = TMP[startx:startx + 64, starty:starty + 64, :]
                patchcount += 1

    emptyarray = emptyarray[0:patchcount, :, :, :]
    patcharray = np.zeros((patchsize, patchsize * patchcount, normdata.shape[1]))
    blurredpatcharray = np.zeros((patchsize, patchsize * patchcount, normdata.shape[1]))

    if rescale:
        patcharray = np.zeros(
            (int(patchsize * rescalefactor), int(patchsize * rescalefactor) * patchcount, normdata.shape[1]))
        blurredpatcharray = np.zeros(
            (int(patchsize * rescalefactor), int(patchsize * rescalefactor) * patchcount, normdata.shape[1]))
    j = 0
    for i in range(emptyarray.shape[0]):
        tmpvar = emptyarray[i, :, :, :]
...
def smoothimg(img, medianblur=False, gaussianblur=False, gaussianblurstd=1, submean=True, medianblursize=3):
    ...
    if medianblur:
        smoothedimg = ndimage.median_filter(img, filtersizeM)
        # datar = data.reshape((-1,data.shape[-1]))
        # data = (datar - np.mean(datar, axis=0)).reshape(data.shape)
        # data[data<0] =0
        # img = data
    if gaussianblur:
        smoothedimg = ndimage.gaussian_filter(img, filtersizeG)
        # if(submean):
        #    datar = data.reshape((-1,data.shape[-1]))
        #    data = (img.reshape((-1,data.shape[-1])) - np.mean(datar, axis=0)).reshape(data.shape)
        #    data[data<0] =0
    return smoothedimg
