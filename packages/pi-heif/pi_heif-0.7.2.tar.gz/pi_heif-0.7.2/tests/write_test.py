...
def test_heif_no_encoder():
    with mock.patch("pi_heif.heif.have_encoder_for_format") as mock_func:
...
def test_heif_save_zero(size: tuple):
    out_heif = BytesIO()
    im = pi_heif.from_pillow(Image.new("RGB", size))
    with pytest.raises(ValueError):
        im.save(out_heif, format="HEIF")
...
def test_hdr_save(im_path, save_format):
    ...
    heif_file_to_add = pi_heif.HeifFile().add_from_heif(heif_file)
...
def test_I_color_modes_to_10_12_bit(enc_bits, save_format):  # noqa
    try:
        pi_heif.options().save_to_12bit = True if enc_bits == 12 else False
        ...
        for mode in ("I", "I;16", "I;16L"):
            ...
            assert pi_heif.open_heif(out_heic, convert_hdr_to_8bit=False).bit_depth == enc_bits
            heic_pillow = Image.open(out_heic)
            helpers.compare_hashes([src_pillow, heic_pillow], hash_size=8)
    finally:
        pi_heif.options().reset()
...
def test_pillow_append_images():
    ...
    heif_file2 = pi_heif.open_heif(helpers.create_heif((81, 79), thumb_boxes=[32], n_images=2))
    heif_file3 = pi_heif.open_heif(helpers.create_heif((72, 68), thumb_boxes=[32, 16], n_images=2))
    out_buf = BytesIO()
...
def test_add_from():
    ...
    heif_file1 = pi_heif.open_heif(heif_file1_buf)
    heif_file2_buf = helpers.create_heif((210, 128), thumb_boxes=[48, 32], n_images=2)
    heif_file2 = pi_heif.open_heif(heif_file2_buf)
