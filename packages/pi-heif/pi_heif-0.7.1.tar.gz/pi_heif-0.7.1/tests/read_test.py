...
def test_heif_corrupted_open(img_path):
    for input_type in [img_path.read_bytes(), BytesIO(img_path.read_bytes()), img_path, builtins.open(img_path, "rb")]:
        try:
            pi_heif.open_heif(input_type).load()
            assert False
        except pi_heif.HeifError as exception:
            assert exception.code == pi_heif.HeifErrorCode.INVALID_INPUT
...
def test_heif_inputs(img_path):
    def perform_test_input():
        with builtins.open(img_path, "rb") as fh:
            ...
            for fp in [bytes_io, fh, img_path, img_path.as_posix(), b]:
                ...
                collect()
                assert getattr(heif_file[0], "_heif_ctx") is not None
                if not pi_heif.options().ctx_in_memory:
                ...
                heif_file_from = pi_heif.HeifFile().add_from_heif(heif_file)
                collect()
    ...
    perform_test_input()
    try:
        pi_heif.options().ctx_in_memory = False
        perform_test_input()
    finally:
        pi_heif.options().ctx_in_memory = True
...
def test_pillow_inputs(img_path):
    with builtins.open(img_path, "rb") as fh:
        ...
        for fp in [bytes_io, fh, img_path, img_path.as_posix()]:
            ...
            assert len(pi_heif.from_pillow(pillow_image, load_one=True)) == 1
...
def test_heif_from_heif(img_path):
    def heif_from_heif(hdr_to_8bit=True):
        ...
        heif_file_from_from = pi_heif.HeifFile().add_from_heif(heif_file_from)
        for _ in heif_file:
            _.unload()
        collect()
...
def test_heif_read_images(image_path):
    def test_read_image(convert_hdr_to_8bit: bool) -> bool:
        ...
        for image in heif_file:
            assert isinstance(pi_heif.getxmp(image.info["xmp"]), dict)
            ...
            assert isinstance(image.load(), pi_heif.HeifImage)
            for thumbnail in image.thumbnails:
                ...
                assert isinstance(thumbnail.load(), pi_heif.HeifThumbnail)
...
def test_pillow_truncated_fail(img_path):
    ...
    with pytest.raises(pi_heif.HeifError):
        truncated_heif.load()
...
@mock.patch("pi_heif.misc.ElementTree", None)
def test_no_defusedxml(monkeypatch):
    with pytest.warns(UserWarning):
        pi_heif.getxmp(b"xmp_data")
