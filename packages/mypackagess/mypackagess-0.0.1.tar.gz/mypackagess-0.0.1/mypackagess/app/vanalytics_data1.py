...
class CommonFeatures:
    ...
    sensor_store = {}   
    #   will store data for processing {sensor_id + "_" + topic: [list(raw_data), power, nwindowsize]}
    vs_sensor_rawdata = {}  
    machine_sensor_rawdata = {}
    bearing_sensor_rawdata = {}
    gear_sensor_rawdata = {}
    misalignment_sensor_rawdata = {}
    looseness_sensor_rawdata = {}
    # stores the rawdata for unbalance algorithm
    unbalance_sensor_rawdata = {}
    main_config = {}
...
class VsensStreamProcessor:
    ...
    def process_burst(self):
        ...
        try:
            ...
            # velocity severity
            severity = algo_severity.velocity_severity(self.TempStore["power"], avg_velocityX, self.EquipmentID, self.SensorID)
            ...
            unbalance_output = []
            try:
                unbalance_obj = algo_unbalance.Unbalance_Analysis(
                    self.SamplingFrequency, self.NoOfSamples, self.WindowSize, operating_rpm_X, self.SensorID,
            ...
            final_output = bearing_output + gear_output + misalignment_output + looseness_output + unbalance_output
            data = [self.EquipmentID, self.SensorID, "FO", json.dumps(final_output) + "$T$" + str(time.time())]
