...
class FileOrder(FileSystemEventHandler):
    ...
    def process_file(self, path, force_flag=False):
        if not path.lower().endswith(self.file_ext.lower()): return
        self.logger.info("process file: {0}".format(path))
        try:
            b = open(path).read()
            if force_flag or b != self.cache.get(path):
                self.cache[path] = b
                self.process_order(path)
    ...
    def scan_files(self):
        ...
        for f in os.listdir(self.file_path):
            ...
            self.process_file(path, force_flag=True)

    def process_order(self, path):
        try:
            df: pd.DataFrame = pd.read_csv(path)
            df = df.dropna(axis=0)
            df = df[[df.columns[0], df.columns[1], df.columns[-1]]]
            self.logger.info('\n' + str(df))
