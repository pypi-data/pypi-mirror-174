...
def methods_inherited(base_class, p=0, table=0):
    import pandas as pd
    class Base:
        pass
    ...
    df = pd.DataFrame()
    returned_set = tuple(set(dir(A)) - set(dir(Base)))
    df['returned_set'] = returned_set
    try:
        abstract_methods = A.__abstractmethods__
        df['__abstractmethods__'] = df['returned_set'].isin(abstract_methods)
        df = df.set_index('__abstractmethods__', append=True)
        df = df.unstack(0).stack().sort_index(ascending=False)
    except:
    ...
    if p == 1:
        ...
        if abstract_methods:
            pprint({'__abstractmethods__': abstract_methods}, compact=True, width=120)
    else:
        if abstract_methods:
            return returned_set, {'__abstractmethods__': abstract_methods}
        else:
            return returned_set
