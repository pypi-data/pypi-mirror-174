...
#         DataFrame.__setattr__(self,key,values)
    ...
    def shape(self):
        if self.__ncol_original__ == 0 and self.__nrow_original__ == 0:
            ...
            command = "awk 'BEGIN{FS=\"" + self.delimiter + "\"}{if (NR>1){exit}}END{print NF}' " + self.path
            result = os.popen(command).read()
            values = result.replace("\n","").strip()
            ncol = int(values)
...
#                 self.nrow_modified_to_unknown_value = False
    ...
    def __shape_current__(self,has_header = True):
        command = "awk -v has_header=has_header_variable " + """'
        BEGIN {
            FS = \"""" + self.delimiter + """\"
            number_rows = 0
        }
...
#         print(types)
    ...
    def __get_rows_from_to__(self,min_row,max_row,has_header = True,output_header = True):
    ...
    def get_rows(self,rows,has_header = True,output_header = True):
        
        
        
        if type(rows) == range:
            return DataFrame.__get_rows_from_to__(self,min(rows),max(rows),has_header = has_header,output_header = output_header)
        else:
...
# return __ddf__
    ...
    def to_csv(self,path_output,append=False,clear_all_commands = False,set_as_new_path = False):
        if not append:
