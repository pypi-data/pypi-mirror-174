...
class VersionParts:
    ...
    def combine(self):
        if self.prerelease and self.buildmetadata:
            self.version = f"{self.major}.{self.minor}.{self.patch}-rc.{self.prerelease_num}+build.{self.buildmetadata_num}"
        ...
        if not self.prerelease and self.buildmetadata:
            # 2.0.0-rc.1+build.123
            self.version = f"{self.major}.{self.minor}.{self.patch}+build.{self.buildmetadata_num}"
