...
def pep_440_strategy(version_str: str):
    """
    in PEP 440 there are couple of differences to cover
    we will convert them to semantic first
    1. => 2.0.1rc2+
    when we have something like this first lets convert it to
        2.0.1-rc2
    """
    dot_parts = version_str.split('.')

    if len(dot_parts) > 3:
        """correction for 0.0.0.1 version => 0.0.0 """
        version_str = ".".join(dot_parts[0:3])

    version_str = version_str.replace("a", "rc").replace("b", "rc")
    version_str = version_str.replace('rc', '-pre').replace('.dev', '+build').replace('.post', '+build')
    # version_str = ".".join(first_parts)
    # print("returning PEP====> ", version_str)
    return version_str
...
def valid_check(version_str: str):
    ...
    version_str = pep_440_strategy(version_str)
