...
class VersionParts:
    ...
    def combine(self):
        if self.prerelease_num and self.buildmetadata_num:
            ...
            self.version = f"{self.major}.{self.minor}.{self.patch}-pre.{self.prerelease_num}+build.{self.buildmetadata_num}"

        if self.prerelease_num and not self.buildmetadata_num:
            self.version = f"{self.major}.{self.minor}.{self.patch}-pre.{self.prerelease_num}"
