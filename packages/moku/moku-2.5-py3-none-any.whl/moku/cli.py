...
import requests
from moku import __version__, MOKU_DATA_PATH
...
def get_moku_data_url(fw=None):
    return f'''http://updates.liquidinstruments.com/static/
                mokudata-{fw or compat_fw}.tar.gz'''


def list_mokus(args):
    results = Finder().find_all(timeout=args.wait,
                                filter=lambda x: x.hwver > 2.0)
    spacing = "{: <20} {: <7} {: <6} {: <20}"

    header = spacing.format('Name', 'Serial', 'FW', 'IP')
    print(header)
    print("-" * len(header))
    ...
    for m in results:
        print(spacing.format(m.name, m.serial, m.fwver, m.ipv4_addr))
...
parser_list.add_argument('--wait', '-w',
                         type=float,
                         help="Browse for n seconds before "
                              "concluding results",
                         default=5.0)
