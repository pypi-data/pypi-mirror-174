...
class ModelLocalCacheMixin(Protocol):
    @classmethod
    def _add_to_local_cache(cls, instances: List[ModelProtocol]):
        ctx = get_context()
    ...
    @classmethod
    def _get_from_local_cache(cls, indices: List[Any]) -> List[Optional[ModelProtocol]]:
        ctx = get_context()
    ...
    @classmethod
    def _drop_from_local_cache(cls, indices: List[Any] = None) -> List[bool]:
        ctx = get_context()
    ...
    @classmethod
    def _has_in_local_cache(cls, indices: List[Any]) -> List[bool]:
        ctx = get_context()
