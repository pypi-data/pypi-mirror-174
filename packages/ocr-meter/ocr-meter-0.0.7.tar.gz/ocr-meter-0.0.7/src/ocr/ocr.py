        ...
        if meter_type == "water" or meter_type == "electricity":
            ...
            for i in range(len(result)):
                ...
                if len(ocr_res) == num_digit - 1 and ocr_res.isdigit():
                    ...
                    if meter_type == "electricity" and "1" in ocr_res:
                        length = int(
                            ((box[1][0] - box[0][0]) / len(ocr_res)) * (lack_digits + 1)
                        )
                    else:
                        length = int(
                            ((box[1][0] - box[0][0]) / len(ocr_res)) * (lack_digits)
                        )
                    x1, y1 = box[1]
                    x2, y2 = box[2]
                    box[1], box[2] = self.cal_expand_point(x1, y1, x2, y2, length)
                    img_crop = self.crop_rect(img, box)
            ...
            for i in range(len(result)):
                ...
                if len(ocr_res) == num_digit and ocr_res.isdigit():
                    ...
                    break
                if (
                    len(ocr_res) >= num_digit - 3
                    and len(ocr_res) <= num_digit - 1
                    and ocr_res.isdigit()
                ):
                    for j in range(len(result)):
                        if i == j:
