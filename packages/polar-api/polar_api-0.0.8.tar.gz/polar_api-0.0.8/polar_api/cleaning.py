...
def extract_players(players_and_staff):
    ...
    map_positions = {"Målmand": "Goal Keeper", 
                    "Forsvar":"Centre Back", 
...
def get_interval_sessions_name_id_dict(sessions_data):
    ...
    df['record_start_time'] = df.record_start_time.apply(lambda x: ", ".join(x.split("T"))[:-3])
    df['record_end_time'] = df.record_end_time.apply(lambda x: x.split("T")[1][:-3])
    df['name'] = df['name'] + " (" + df['record_start_time'] + "-" + df['record_end_time'] + ")"
...
def preprocess(data, set_date, team_players, session_name, account):
    ...
    if account == "M":
        ...
        data.rename(columns={'Time in HR zone 1 (50 - 59 %)':'Time in HR zone 1 (50 - 59 %).1',
                            ...
                            'Time in HR zone 5 (90 - 100 %)':'Time in HR zone 5 (90 - 100 %).1'}, inplace=True)
        ...
        data = pd.concat([data, pd.Series(data['Duration'].astype(str).apply(hms_to_m), name='Duration_min')], axis=1)
        data = pd.concat([data, pd.Series(data['Time in HR zone 1 (50 - 59 %).1'].apply(hms_to_m), name='Time in HR zone 1 (50 - 59 %)')], axis=1)
        data = pd.concat([data, pd.Series(data['Time in HR zone 2 (60 - 69 %).1'].apply(hms_to_m), name='Time in HR zone 2 (60 - 69 %)')], axis=1)
        data = pd.concat([data, pd.Series(data['Time in HR zone 3 (70 - 79 %).1'].apply(hms_to_m), name='Time in HR zone 3 (70 - 79 %)')], axis=1)
        data = pd.concat([data, pd.Series(data['Time in HR zone 4 (80 - 89 %).1'].apply(hms_to_m), name='Time in HR zone 4 (80 - 89 %)')], axis=1)
        data = pd.concat([data, pd.Series(data['Time in HR zone 5 (90 - 100 %).1'].apply(hms_to_m), name='Time in HR zone 5 (90 - 100 %)')], axis=1)
        data['Maximum speed [km/h]'] = data["Maximum speed [km/h]"].round(decimals = 1)
        data['Average speed [km/h]'] = data["Average speed [km/h]"].round(decimals = 1)
