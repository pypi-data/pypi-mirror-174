...
def ccg_jitter(times1,times2,jitter_window,binrange=(-.25,.25)):
    ms1 = np.zeros(int(np.ceil(np.max(times1))*1000))
    ind1 = times1*1000
    ms1[ind1.astype(int)]=1
    ms2 = np.zeros(int(np.ceil(np.max(times2))*1000))
    ind2 = times2*1000
    ms2[ind2.astype(int)]=1
    ccg_1 = xcorrfft(ms1,ms2,
                     int(nextpow2(ms1.shape[0]))) / np.sqrt(np.mean(times1.shape[0])**2+np.mean(times2.shape[0])**2) * 100.
    #jitter and recalculate
    b=jitter_1d(ms1.reshape(1000,-1),jitter_window)
    c=jitter_1d(ms2.reshape(1000,-1),jitter_window)
    ccg_2 = xcorrfft(b.flatten(),c.flatten(),int(nextpow2(b.flatten().shape[0]))) / np.sqrt(np.mean(times1.shape[0])**2+np.mean(times2.shape[0])**2) * 100.
    ccg_c = ccg_1-ccg_2
    return ccg_2[int(ccg_2.shape[0]/2 + int(binrange[0]*1000)):int(ccg_2.shape[0]/2 + int(binrange[1]*1000))]

################################################################################################
######    from xioaxuan jia, allen institute and tsinghua university    ########################
################################################################################################
################################################################################################
...
def jitter_1d(data, l):
    # matlab reshape default is Fortran order, but python default is 'C' order, when converting code, need to specify order
    """Jitter correction adapted from Xioaxuan Jia, Jitter correction adapted from Amin. Used in several neuron papers.
       Jittering multidimensional logical data where
       0 means no spikes in that time bin and 1 indicates
       a spike in that time bin.
       First dimention should be time and second should be the trial number.
       data: time*trial
