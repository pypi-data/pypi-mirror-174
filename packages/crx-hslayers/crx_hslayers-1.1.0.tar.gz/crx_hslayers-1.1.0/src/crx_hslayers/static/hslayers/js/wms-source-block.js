class WmsSourceBlockDefiniton extends window.wagtailStreamField.blocks.StructBlockDefinition {
    ...
    async loadWmsLayers(prefix, loadButton){
        ...
        try {
            let data = await this.getCapabilities(window.location.origin + '/proxy/' + urlInput.value);
    ...
    async getCapabilities(url) {
        ...
        let urlObject = new URL(url.split("?")[0]); //throw away URL params, use following instead
