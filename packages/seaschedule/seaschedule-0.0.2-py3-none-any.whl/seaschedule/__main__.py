...
from seaschedule.config import CFG, RUN_TIME
from seaschedule.utils.email import send_mail
from seaschedule.utils.log import get_logger, format_log_message
from seaschedule.utils.sftp import put_files
    ...
    try:
        ...
        for task_log in task_logs:
            log_summary = log_summary + f'- Step-{task_log["step"]}: {task_log["name"]}' \
                                        f'......... {task_log["status"]} ({task_log["remarks"]})' \
                                        f' ({task_log["seconds"]:.2f}s)<br>'
