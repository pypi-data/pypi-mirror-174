def molar_volume(temperature, pressure):
    ...
    assert 1.8 >= temperature >= 0.05, 'The function Helium4.molar_volume is not defined for T = ' + str(
        temperature) + 'K '
    ...
    sum_v = 0
    for i in range(10):
        sum_v = sum_v + coefficients.V[i] * pressure ** i

    sum_a = (coefficients.A[0] / ((pressure - pc) ** 2) + coefficients.A[1] / (
            (pressure - pc) ** (5 / 3))) * temperature ** 4 / 4

    sum_b = coefficients.B[0] + coefficients.B[1] * pressure

    sum_c = 0

    for i in range(3):
        sum_c = sum_c + coefficients.Delta[i] * pressure ** i

    return sum_v * (1 + sum_a - sum_b * erfc((sum_c / temperature) ** 0.5))
...
def molar_mass():
    ...
    Return the constant molar mass of a single atom of helium-3
