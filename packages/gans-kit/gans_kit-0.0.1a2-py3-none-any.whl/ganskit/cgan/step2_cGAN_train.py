...
def load_my_samples(data_file_path='colormed_minist_data.h5'):
	# load dataset
	import h5py
	import numpy as np
	# (trainX, _), (_, _) = load_data()
	train_dataset = h5py.File(data_file_path, 'r')
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs=100, n_batch=128,save_model_path='cgan_generator-2.h5'):
	...
	g_model.save(save_model_path)

def start_train(data_file_path,save_model_path):
	...
	# load image data
	dataset = load_my_samples(data_file_path)
	# train model
	train(g_model, d_model, gan_model, dataset, latent_dim,save_model_path=save_model_path)