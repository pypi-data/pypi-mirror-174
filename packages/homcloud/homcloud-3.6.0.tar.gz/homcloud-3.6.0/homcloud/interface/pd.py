...
class PDList(object):
    ...
    @staticmethod
    def from_alpha_filtration(pointcloud, weight=False, no_squared=None, *,
                              squared=None, subsets=False, check_acyclicity=False,
        ...
        filtration = alpha_shape.create_filtration(squared, vertex_symbols,
    ...
    def from_rips_filtration(distance_matrix, maxdim, maxvalue=np.inf,
                             ...
                             save_graph=False, save_cocycles=False, save_to=None):
        """
...
class PD(object):
    ...
    @staticmethod
    def from_birth_death(degree, births, deaths, ess_births=np.array([]),
                         sign_flipped=False):
    ...
    def graph_optimal_1_cycle(self, weighted=False, torelance=None):
        if torelance is None:
            return GraphOptimal1Cycle(self, graph_opt1cyc.search(
                self.diagram.pd.graph_adjacent_matrix, self.birth_time(), weighted
            ))
        else:
            return GraphOptimal1Cycle(self, graph_opt1cyc.search_with_cocycle(
                self.diagram.pd.graph_adjacent_matrix, self.representative_cocycle(),
                self.birth_time(), torelance, weighted
            ))
