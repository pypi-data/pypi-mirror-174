...
from homcloud.pdgm_format import (
    PDGMWriter, PDChunk, SimpleChunk, BoundaryMapChunk, RipsInformationChunk, GraphWeightsChunk
)
...
class DistanceMatrix(object):
    ...
    def build_rips_filtration(self, save_graph=False, save_cocycle=False):
        return RipsFiltration(self.matrix.astype(np.float32), self.upper_dim, self.upper_value, save_graph, save_cocycle)
...
class RipsFiltration(object):
    ...
    def compute_pdgm_by_ripser(self, f):
        ...
        if self.save_cocycle:
            self.append_cocycle_chunk(writer,
                                      diagrams["dgms"],
                                      diagrams["cocycles"])
    ...
    def append_cocycle_chunk(self, writer, dgms, cocycles):
        for d in range(len(dgms)):
            writer.append_simple_chunk("cocycles", [
                cocycle.tolist() for (cocycle, pair)
                in zip(cocycles[d], dgms[d])
                if pair[1] != np.inf
            ], d)
