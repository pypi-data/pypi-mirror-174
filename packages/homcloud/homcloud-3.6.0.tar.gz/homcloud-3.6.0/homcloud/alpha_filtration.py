...
class AlphaShape(object):
    ...
    def create_filtration(self, squared=True, symbols=None, save_boundary_map=False,
                          save_phtrees=False):
        simplices = [
        ...
        return AlphaFiltration(self.coordinates, self.weights, squared, self.periodicity, simplices, dict_simplices,
...
class AlphaFiltration(FiltrationWithIndex):
    def __init__(self, points, point_weights, squared, periodicity, simplices, dict_simplices, dim, symbols,
    ...
    def alpha_information_chunk(self):
        return AlphaInformationChunk(self.points.shape[0], self.periodicity,
                                     self.points_weighted, self.squared)
