...
class Site(object):
    ...
    *within* the framework.
    ...
    :setting:`DATABASES` and :setting:`SECRET_KEY` and the :attr:`cache_dir`
    """
    # for internal use:
    _logger = None
    _starting_up = False
        ...
        if self.master_site is None:
            ...
            if cache_root:
                ...
                if not cr.exists():
                    ...
                    raise Exception(msg)
                self.cache_dir = (cr / self.project_name).resolve()
            ...
            self.cache_dir = self.master_site.cache_dir
            ...
            if logdir.is_dir():
                ...
                if self.use_linod:
                    handlers['root'] = {
                        ...
                        'host': str(self.project_dir / 'log_sock'),
                        'port': None,
                        'level': level,
                        # 'filters': ['root']
                    }
    ...
    def get_database_settings(self):
        ...
        if self.cache_dir is None:
            ...
            dbname = self.cache_dir / 'default.db'
            return {
                'default': {
    ...
    def install_settings(self):
        ...
        if not self.django_settings.get('STATIC_ROOT', False):
            ...
            if cache_root:
                self.django_settings.update(
                    STATIC_ROOT=str(Path(cache_root) / 'static_root'))
            else:
                self.django_settings.update(
                    STATIC_ROOT=str(self.cache_dir / 'static_root'))
    ...
    def setup_cache_directory(self):
        ...
        stamp = self.cache_dir / 'lino_cache.txt'
