...
def calc_nodes_colors_by_metric(graph, node_style_metric, darkest_color=100, lightest_color=250) -> Dict[str, str]:
    """
    ...
    for node in nodes:
        if node_style_metric in node.metrics:
            ...
            node_color_int = int(
                lightest_color
                - (lightest_color - darkest_color) * (node_metric_value - min_value) / (max_value - min_value)
            )
...
def calc_edges_widths_by_metric(
        graph, edge_style_metric, min_width=0.1, max_width=5
    ...
    if not graph.contains_edge_metric(edge_style_metric):
        ...
        return {}
    metric_values = [
        edge.metrics[edge_style_metric] for edge in graph.get_edges() if edge_style_metric in edge.metrics
    ]
    if any(isnan(metric_values)):
        print(
...
def _get_hex_string(num):
    ...
    hex_string: str
        Hexadecimal string.
    """
    return str(num) if num < 10 else ["A", "B", "C", "D", "E", "F"][num - 10]


def add_metrics_to_node_label(label: str, metrics: Dict[str, float], newline: Optional[str] = "\\n") -> str:
    """
...
def add_metric_to_node_label(
        label: str, metric_name: str, metric_value: float, newline: Optional[str] = "\\n"
) -> str:
    """
    Adds information about node's metrics to its label.
