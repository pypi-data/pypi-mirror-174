...
from numpy import array, concatenate, diag, inf, mean, ndarray, newaxis, zeros
...
class CorrelationMiner(AbstractMiner):
    ...
    def apply(self) -> None:
        ...
        activities = array(sorted(self._data_holder.get_unique_activities()))
        act_counts_dict = data[act_col].value_counts(sort=False).to_dict()
        act_counts_list = [act_counts_dict[a] for a in activities]
        timestamps = {
            i: data[data[act_col] == act][timestamp_col].astype(int).values / 10**9
    ...
    @staticmethod
    def _create_edges_add_count_metric(graph, activities: ndarray, edges_count_matrix: ndarray):
        """
        ...
        edges_count_matrix: ndarray of int, shape=[activities_num, activities_num]
...
def get_precede_succeed_matrix(activities: ndarray, timestamps: Dict[int, ndarray]) -> ndarray:
    """
    Calculate PS matrix.

    Parameters
    ----------
    activities: ndarray of str
        Names of unique activities.

    timestamps: Dict[int, ndarray]
        Dict of timestamps for each activity. Int number corresponds
        to the index of activity in 'activities' array.

    Returns
    -------
    ps_matrix: ndarray of float, shape=[len(activities), len(activities)]

    """
    ps_matrix = zeros((len(activities), len(activities)), dtype="float")
...
def calc_ps_matrix_value(ts1: ndarray, ts2: ndarray) -> float:
    """
    ...
    follows = 0
    while p < len(ts1):
        while q < len(ts2) and not ts1[p] < ts2[q]:
            q += 1
        follows += len(ts2) - q
        p += 1

    return follows / total_combinations


def get_duration_matrix(activities: ndarray, timestamps: Dict[int, ndarray], greedy: bool, sparse) -> ndarray:
    """
    ...
    timestamps: Dict[int, ndarray of int]
        Dict of timestamps for each activity. Int number corresponds
        to the index of activity in 'activities' array.
    ...
    Returns
    -------
    duration_matrix: ndarray of float, shape=[len(activities), len(activities)]

    """
    duration_matrix = zeros((len(activities), len(activities)), dtype="float")
...
# TODO refactor
def calc_mean_duration_value(ts1: ndarray, ts2: ndarray, greedy: bool, sparse: bool) -> float:
    """
...
def create_sparse_matrix(ts1: ndarray, ts2: ndarray) -> csr_matrix:
    """
    ...
    """
    diff = ts2[newaxis, :] - ts1[:, newaxis]
    diff[diff < 0] = 0
    return csr_matrix(diff)


def create_dense_matrix(ts1: ndarray, ts2: ndarray) -> ndarray:
    """
    Create ndarray matrix of time durations.
    Negative duration values will converted inf.
    ...
    Returns
    -------
    matrix: ndarray, shape=[len(ts1), len(ts2)]
    """
    diff = ts2.astype(float)[newaxis, :] - ts1[:, newaxis]
    diff[diff < 0] = inf
    return diff


def get_c_matrix(ps_matrix: ndarray, duration_matrix: ndarray, act_counts_list: List[int]) -> ndarray:
    """
    ...
    ----------
    ps_matrix: ndarray
    duration_matrix: ndarray
    act_counts_list: list of int

    Returns
    -------
    c_matrix: ndarray, shape=len(activities), len(activities)
    """
    c_matrix = zeros((len(act_counts_list), len(act_counts_list)))
    for i in range(len(act_counts_list)):
        for j in range(len(act_counts_list)):
            value = (
                duration_matrix[i, j] / ps_matrix[i, j] * 1 / (min(act_counts_list[i], act_counts_list[j]))
                if ps_matrix[i, j] != 0
                else 0
            )
            if value == 0:
...
def solve_lp_problem(c_matrix: ndarray, act_counts_list: List[int]) -> Tuple[bool, Optional[ndarray]]:
    """
    ...
    edges_count_matrix: ndarray of int, shape=[len(activities), len(activities)] or None
    ...
    for i in range(act_num):
        m = zeros((act_num, act_num))
        m[i, :] = [1] * act_num
        separate_matrixes.append(m)
    A_eq_outgoing = concatenate(separate_matrixes, axis=1)
    ...
    A_eq_incoming = concatenate([diag([1] * act_num) for _ in range(act_num)], axis=1)

    A_eq = concatenate([A_eq_outgoing, A_eq_incoming], axis=0)
    b_eq = concatenate([act_counts_list, act_counts_list])
    ...
    # a)
    A_ub_min = diag([1.0] * act_num**2)  # each row - one unique edge
    b_ub_min = zeros(act_num**2, dtype="float")

    for i, j in product(range(act_num), range(act_num)):
        edge_ind = i * act_num + j
        b_ub_min[edge_ind] = min([act_counts_list[i], act_counts_list[j]])

    # b)
    Aub_zero = -A_ub_min
    b_ub_zero = zeros(act_num**2, dtype="float")

    A_ub = concatenate([A_ub_min, Aub_zero], axis=0)
    b_ub = concatenate([b_ub_min, b_ub_zero])
    ...
    if status == "optimal":
        edges_count_matrix = array(x).reshape(act_num, act_num).astype("int")
        return True, edges_count_matrix
    else:
        simplefilter("always", RuntimeWarning)
        warn(
            f"Linear program solving failed (finished with status '{status}'). The graph cannot be built.",
            RuntimeWarning,
        )
        return False, None


def clean_ts(ts1: ndarray, ts2: ndarray) -> Tuple[ndarray, ndarray]:
    """
    ...
    new_ts2 = concatenate([new_ts2, ts2[j:]])  # add the ts2[j:] too (can be empty if j is maximum)
    ...
    new_ts1 = concatenate([ts1[: i + 1], new_ts1[::-1]])  # add the ts1[:i + 1] too (can be empty if i==-1)
