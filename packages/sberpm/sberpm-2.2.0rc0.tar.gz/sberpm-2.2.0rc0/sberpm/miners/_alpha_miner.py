...
class AlphaMiner(AbstractMiner):
    ...
    n_jobs: int, default=1
        ...
        if n_jobs < 0 - max number of processes = cpu_count() + 1 + n_jobs;
    ...
    def __init__(self, data_holder, n_jobs=1):
        ...
        max_cores_or_only_one = 1 if n_jobs == 0 else cpu_count() + (n_jobs + 1)  # -n gives (max - (1 - n)) of max
        self._n_jobs = n_jobs if n_jobs > 0 else max_cores_or_only_one
    ...
    def find_places(self, follows_pairs, unique_activities):
        ...
        return self._calculate(causal_pairs, independence_matrix, act2label, label2act, self._n_jobs)
    ...
    def _create_independence_matrix(causal_pairs, parallel_pairs, act2label):
        ...
        """
        independence_matrix = full((len(act2label), len(act2label)), True, dtype=bool)
    ...
    def _calculate(causal_pairs, independence_matrix, act2label, label2act, n_jobs):
        ...
        causal_pairs_encoded = array(causal_pairs_encoded)
    ...
    def _calc_new_pairs(
        ...
        if total_iterations < iterations_per_batch or n_jobs == 1:
            ...
            pool = Pool(min(num_batches, n_jobs))
            result_objects = [
    ...
    def _generate_args(causal_pairs_encoded, list_of_pairs, columns_per_batch, independence_matrix):
        ...
        while n < len(list_of_pairs):
            ...
            yield (
                ...
                array(list_of_pairs[n - columns_per_batch : n]),
                n - columns_per_batch,
                independence_matrix,
            )
    ...
    def _independent(act, set2, independence_matrix):
        ...
        """
        return all_(independence_matrix[act][list(set2)])
