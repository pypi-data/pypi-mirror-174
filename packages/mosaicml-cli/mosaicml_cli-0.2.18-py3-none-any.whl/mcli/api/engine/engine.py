...
class MAPIConnection:
    ...
    def _load_from_environment(self, api_key: Optional[str] = None, endpoint: Optional[str] = None) -> None:
        ...
        if api_key is None:
            api_key = config.MCLIConfig.load_config(safe=True).api_key
...
def _deserialize_response(
    ...
    # Parse response (raises 500 if cannot be parsed, 400s for status codes passed through data)
    try:
        data = response['data']
        query_response: Dict[str, Any] = data[query_function]

        # This supports mapi SuccessResponse AND returning the raw list of items
        # TODO: HEK-1435 - Fully deprecate success response
        if isinstance(query_response, list):
            raw_response = RawSuccessResponse(
