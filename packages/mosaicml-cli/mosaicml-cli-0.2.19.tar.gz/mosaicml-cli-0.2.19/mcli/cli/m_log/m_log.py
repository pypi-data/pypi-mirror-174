...
def get_logs(
    ...
    **kwargs,
    ...
    try:
        ...
        # Set to follow until hek-1245
        if conf.feature_enabled(FeatureFlag.USE_MCLOUD) and not follow:
            logger.warning('get_run_logs is not yet implemented serverside (HEK-1245). '
                           'Setting --follow to True. To stop following, hit Ctrl+c')
            follow = True
            # Sleep briefly to help the user see the message
            time.sleep(1)
