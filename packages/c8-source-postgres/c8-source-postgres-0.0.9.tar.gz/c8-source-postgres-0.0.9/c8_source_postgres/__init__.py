...
from c8connector import C8Connector, ConfigProperty, Sample, Schema, AttributeType, Attribute
...
class PostgresSourceConnector(C8Connector):
    ...
    def samples(self, integration: dict) -> list[Sample]:
        ...
        for stream in streams:
            ...
            for k, v in s_schema['properties'].items():
                t = v['type'][-1]
                s_attribs.append(Attribute(k, self.get_attribute_type(t)))
    ...
    @staticmethod
    def get_attribute_type(source_type: str) -> AttributeType:
    ...
    def config(self) -> list[ConfigProperty]:
        ...
        return [
            ConfigProperty('host', AttributeType.STRING, True, False,
                           description='PostgreSQL host',
                           example='51.25.44.24'),
            ConfigProperty('port', AttributeType.INT, True, False,
                           description='PostgreSQL port',
                           example='5432'),
            ConfigProperty('user', AttributeType.STRING, True, False,
                           description='PostgreSQL user',
                           example='postgres'),
            ConfigProperty('password', AttributeType.STRING, True, False,
                           description='PostgreSQL password',
                           example='password'),
            ConfigProperty('dbname', AttributeType.STRING, True, False,
                           description='PostgreSQL database name',
                           example='postgres'),
            ConfigProperty('filter_schemas', AttributeType.STRING, False, False,
                           description='Scan only the specified comma-separated schemas to improve the '
                                       'performance of data extraction',
                           example='employee,department'),
            ConfigProperty('ssl', AttributeType.STRING, False, False,
            ...
            ConfigProperty('debug_lsn', AttributeType.STRING, False, False,
                           ...
                           example='false'),
            ConfigProperty('tap_id', AttributeType.STRING, False, False,
                           description='ID of the pipeline/tap',
                           example=''),
            ConfigProperty('itersize', AttributeType.INT, False, False,
                           description='Size of PG cursor iterator when doing INCREMENTAL or FULL_TABLE.',
                           example='20000'),
            ConfigProperty('default_replication_method', AttributeType.STRING, False, False,
                           ...
                           example='LOG_BASED'),
            ConfigProperty('use_secondary', AttributeType.BOOLEAN, False, False,
                           description='Use a database replica for INCREMENTAL and FULL_TABLE replication.',
                           example='false'),
            ConfigProperty('secondary_host', AttributeType.STRING, False, False,
                           description='PostgreSQL Replica host (required if use_secondary is True)',
                           example='51.25.44.25'),
            ConfigProperty('secondary_port', AttributeType.INT, False, False,
