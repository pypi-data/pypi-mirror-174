...
def my_pipe():
    ...
    delay = 0.013  # neural delay in secs
    stimulus_waveform = np.sum(
        np.sin(2 * np.pi * ffr_frequencies * np.arange(0, burst_duration, 1 / fs).reshape(-1, 1)),
        axis=1).reshape(-1, 1) * 1.0  # generates ffr artifacts with 1 uV amplitude
    template_waveform = np.pad(stimulus_waveform, ((int(fs * delay), 0), (0, 0)), 'constant', constant_values=(0, 0))
    template_waveform = template_waveform[0:stimulus_waveform.shape[
        0]] * 0.2  # generates ffr components with 0.2 uV amplitude and a delay
    template_waveform[template_waveform < 0] = 0  # rectify simulated brain response
    n_channels = 32
    ...
    reader = GenerateInputData(template_waveform=template_waveform,
                               ...
                               event_code=1.0,
                               figures_subset_folder='ffr_ht2_test',
                               noise_seed=0)
    reader.run()
