...
class GPF:
    ...
    def train(self, n_iterations=500, lr=1e-3):
        ...
        for i in range(n_iterations):
            ...
            # compute validation loss and early stopping
            if i % 5 == 0:
                model.eval()
                likelihood.eval()
                ...
                with torch.no_grad(), gpytorch.settings.fast_pred_var():
                    ...
                    print(
                        "Iter %d/%d - Loss: %.3f, Val_Loss: %.3f"
                        % (i + 1, n_iterations, loss.item(), val_loss.item())
                    )
                if self.early_stopping(self.val_losses, 5):
                    break
                model.train()
                likelihood.train()
            else:
