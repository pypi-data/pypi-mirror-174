...
def capture(
    ...
    # type: (...) -> None
    """
    Capture allows you to capture anything a user does within your system, which you can later use in AnalyticKit to find patterns in usage, work out which features to improve or where people are giving up.
    ...
    For example:
    ```python
    analytickit.capture('distinct id', 'opened app')
    analytickit.capture('distinct id', 'movie played', {'movie_id': '123', 'category': 'romcom'})

    analytickit.capture('distinct id', 'purchase', groups={'company': 'id:5'})
    ```
    """
    _proxy(
        "capture",
...
def identify(
    ...
    # type: (...) -> None
    """
    Identify lets you add metadata on your users so you can more easily identify who they are in AnalyticKit, and even do things like segment users by these properties.
    ...
    For example:
    ```python
    analytickit.identify('distinct id', {
...
def set_once(
    ...
    This will not overwrite previous people property values, unlike `identify`.
     ...
     For example:
     ```python
     analytickit.set_once('distinct id', {
         'referred_by': 'friend',
     })
     ```
    """
...
def group_identify(
    ...
    Set properties on a group
     ...
     For example:
     ```python
     analytickit.group_identify('company', 5, {
         'employees': 11,
     })
     ```
    """
...
def alias(
    ...
    For example:
    ```python
    analytickit.alias('anonymous session id', 'distinct id')
    ```
    """
    _proxy(
        "alias",
...
def feature_enabled(
    ...
    For example:
    ```python
    if analytickit.feature_enabled('beta feature', 'distinct id'):
        # do something
    if analytickit.feature_enabled('groups feature', 'distinct id', groups={"organization": "5"}):
        # do something
    ```

    You can call `analytickit.load_feature_flags()` before to make sure you're not doing unexpected requests.
    """
    return _proxy(
        "feature_enabled",
        key=key,
...
def get_feature_flag(
    ...
    Example:
    ```python
    if analytickit.get_feature_flag('beta-feature', 'distinct_id') == 'test-variant':
        # do test variant code
    if analytickit.get_feature_flag('beta-feature', 'distinct_id') == 'control':
        # do control code
    ```
    ...
    ```python
        group_properties={"organization": {"name": "AnalyticKit", "employees": 11}}
    ```
    """
    return _proxy(
        "get_feature_flag",
...
def get_all_flags(
    ...
    Example:
    ```python
    flags = analytickit.get_all_flags('distinct_id')
    ```
