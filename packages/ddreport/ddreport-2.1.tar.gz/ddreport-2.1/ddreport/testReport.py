        ...
        try:
            ...
            # 断言验证
                ...
                if not isinstance(check, (dict, list)):
                    passive_except = False
                    info.update({"错误详情": f"断言异常，参数必须为json类型\n{check}"})
                ...
                for n, ass in enumerate(check):
                    if ass['type'].upper() == 'JSON':
                        if not isinstance(response, (dict, list)):
                            passive_except = False
                            info.update({"错误详情": f"响应体不是JSON类型"})
                            raise #Exception({"错误详情": f"响应体不是JSON类型"})
            ...
            if passive_except:
                info.update({"错误详情": f"{traceback.format_exc()}"})
            raise Exception(info)
                # return info
