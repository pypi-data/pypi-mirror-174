...
class CampaignClient(BaseSubscriptionObjectClient):
    ...
    def get_template(self):
        campaign_template = self.object and self.object.get("template")
        template_action_data = campaign_template and campaign_template.get("action_data")
        template = template_action_data.get(self.action_client.action['id'])
    ...
    def update_object_status(self, status: str):
        self.filum_client._request(
            endpoint=f"/internal/feedback-campaigns/{self.campaign['id']}/status",
            method="PUT",
            data={
                "status": status
            }
