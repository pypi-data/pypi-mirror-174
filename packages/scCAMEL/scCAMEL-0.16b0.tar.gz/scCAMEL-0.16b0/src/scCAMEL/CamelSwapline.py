...
def CellTypeSimilarityViolinPlot(datax, dataref):
    ...
    fig = pyplot.figure(figsize=(7, len(genename)))
    ...
    for g in range(len(genename)):
        ...
        if len(genename[g]) > 127:
            axleft.set_ylabel("%s\n%s\n%s" % (genename[g][:15], genename[g][15:-11], genename[g][-11:]), fontsize=15,
                              rotation=0, labelpad=90)
        else:
            axleft.set_ylabel("%s\n%s" % (genename[g][:15], genename[g][15:]), fontsize=20, rotation=0, labelpad=90)
            axleft.yaxis.set_label_coords(-0.3, 0)
        plt.xticks(x, name, rotation=90, horizontalalignment='center', verticalalignment='top',
                   position=(0, 0), fontsize=25)
        # plt.yticks([])
...
def CellTypeSimilarity(datax, labelnum=False, RowCluster=True,
