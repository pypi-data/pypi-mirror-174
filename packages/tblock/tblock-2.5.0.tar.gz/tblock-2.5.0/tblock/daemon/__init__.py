    ...
    if os.path.isfile(Path.DAEMON_PID) and not no_pid:
        ...
        with sqlite3.connect(Path.DATABASE) as conn:
            ...
            last_update = cur.execute(
                "SELECT value FROM system WHERE variable=?;",
                ("daemon_update",)
            ).fetchone()
            if last_update:
        ...
        updater(sync_repo=sync_repo, frequency=frequency, do_not_remove_pid=no_pid, anti_hijack=anti_hijack,
                force=force, quiet=quiet, last_update=last_update)
...
def time_left(last_update: int, frequency: int) -> int:
    if last_update and last_update + frequency > int(datetime.datetime.now().strftime("%Y%m%d%H")):
        return (last_update + frequency) - int(datetime.datetime.now().strftime("%Y%m%d%H"))
    else:
        return 0
...
def updater(sync_repo: bool = False, frequency: int = 12, do_not_remove_pid: bool = False,
            anti_hijack: bool = True, last_update: int = 0, force: bool = False, quiet: bool = False) -> None:
    ...
    while not process.stopped:
        ...
        if hours_left:
            if not quiet:
                print(f'{Font.BOLD}==> Update was already done, waiting {hours_left} hour(s)...{Font.DEFAULT}')
            x = 1
            while hours_left:
                hours_left = time_left(last_update=last_update, frequency=frequency)
                if process.stopped:
        ...
        if not quiet:
            print(f'{Font.BOLD}==> Waiting {frequency} hour(s) until next update...{Font.DEFAULT}')
        x = 1
