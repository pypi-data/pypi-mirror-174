...
from typing import Any, Callable, Dict, Generic, List, Optional, Protocol, Type, TypeVar, Union, get_type_hints
...
def parse_function(
    ...
    for i, (arg_name, parameter) in enumerate(sig.parameters.items()):
        ...
        if not dataclasses.is_dataclass(typ):
            raise ValueError(f"State value must be a dataclass. Argument '{arg_name}' has type '{typ.__name__}'")
