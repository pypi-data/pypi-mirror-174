...
from chalk.features.feature import Feature, Features, FeatureSetBase, FeatureWrapper, Filter, TimeDelta, unwrap_feature
from chalk.serialization.codec import FEATURE_CODEC
from chalk.utils.collections import ensure_tuple, get_unique_item

# Conditional on whether to use the pandas df query format
# If None, then pd.query will only be used on large (>=10,000) row dataframes
USE_PD_QUERY: Optional[bool] = None
...
class DataFrame(metaclass=DataFrameMeta):
    ...
    def __init__(
        ...
        namespaces = set(x for x in namespaces if not x.startswith("__chalk__"))
        if len(namespaces) != 1:
            # Allow empty dataframes or dataframes with multiple namespaces
            self.namespace = None
            self.references_feature_set = None
        else:
            self.namespace = get_unique_item(namespaces, f"dataframe column namespaces")
    ...
    @overload
    @classmethod
    def from_list(cls, data: Sequence[Features], /) -> Self:
        ...
    ...
    @classmethod
    def from_list(cls, *data: Union[Features, Sequence[Features]]):
    ...
    @classmethod
    def read_parquet(
        cls, path: Union[str, pathlib.Path], columns: Optional[Union[List[int], List[str]]] = None
    ) -> Self:
        return cls(pl.read_parquet(path, columns))

    @classmethod
    def read_csv(
        cls, path: Union[str, pathlib.Path], has_header: bool, columns: Optional[Union[List[int], List[str]]] = None
    ) -> Self:
        return cls(pl.read_csv(path, has_header, columns))
