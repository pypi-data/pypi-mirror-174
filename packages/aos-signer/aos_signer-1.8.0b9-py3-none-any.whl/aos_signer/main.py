...
def run_validate(args):
    validate_service_config(Path('meta/config.yaml'))


def run_upload_service(args):
    upload_service(Path('meta/config.yaml'))


def run_sign(args):
    sign_service(Path('meta/config.yaml'), 'src', '.')


def run_go(args):
    config_path = Path('meta/config.yaml')
...
def main():
    ...
    try:
        ...
        aos_signer.signer.common.print_message('[red]Process failed with error: ')
        se.print_message()
        sys.exit(1)
