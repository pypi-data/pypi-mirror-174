...
def check_reset_code(string):
    """检查是否全是\033[0m"""
    res = re.findall(FIND_CSI_PATTERN, string)
    for code in res:
        if code != '\033[0m':
            return False
    return True
...
class CsiStringMethods(BasicPrintTool):
    ...
    def _sort_string(self, string):
        ...
        while True:
            ...
            if csi is None:
                prev_csi = '' if check_reset_code(prev_csi) else prev_csi
            ...
            if csi_left != 0:
                ...
                prev_csi += '\033[0m' if search_str[csi_left:csi_right] == '\033[0m' else search_str[csi_left:csi_right]
