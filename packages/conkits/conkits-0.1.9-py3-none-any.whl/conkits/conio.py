    ...
    def get_csistr(self, *args):
        ...
        for arg in args:
            if arg not in FORE_VALUE and arg not in BACK_VALUE and arg not in STYLE_VALUE:
                raise ValueError(ERROR_CSI_STR + '所给参数不在Fore, Back, Style的代码范围内' + RESET_ALL)
            format_str += str(arg) + ';'
        return format_str[:-1] + 'm'

    def packing_str(self, text, *args):
        """
        包装字符串
        ...
        if not isinstance(text, str):
            text = str(text)
        return self.get_csistr(*args) + text + RESET_ALL
