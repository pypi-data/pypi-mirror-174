...
def description_printer(text_list, printer, print_delay=None, step=None, csi_code=None):
    """
    专门用于交互式打印文本
    ...
    csi_code = printer.get_csi_code() if csi_code is None else csi_code
    is_wait = True
    for text in text_list:
        char = printer.print(text, print_delay=print_delay, step=step, csi_code=csi_code)
...
tips_lp = DynamicPrint(step=2, csi_code=Style.ITALICs, is_wait=True)  # 打印小提示
...
simple_description_of_usage = [
    ...
    '如果你了解blessing库，那么要知道\n'
    'conkits不同于blessing库，它只是对csi控制代码的封装\n'
    '而不具备对终端的直接的控制\n',
    ...
    def description():
        def description_of_fore_and_back():
            description_of_fb = [
                ...
                f'{orange_fore}在使用时要注意，属性名只是整数值，属性名加上s的值才是csi字符串{Fore.RESETs}\n',
                f'例如下面的代码\n',
        ...
        simple_description_of_fbs = [
            ...
            f'CsiFore，CsiBack，CsiStyle\n',
            f'这三个类都位于conkits内的csi模块\n'
...
def usage_of_cursor():
    def callable_methods():
        ...
        normal_lp.print(f'\nPS:这部分建议直接{Fore.REDs}ctrl + 左键{Fore.RESETs}去看代码，里面都有注释\n'
                        f'如果你使用的是手机，那就凑合看看吧', step=2)
        wait_anim0()
        normal_lp.print(f'\nCursor所含方法\n{orange_fore}带_s后缀的函数返回csi字符串，不带的直接执行{Fore.RESETs}\n')
        wait_anim0()
        for idx, name in enumerate(method_names1):
            ...
            if name != 'moveto_linehead':
                print('对应csi代码： \\033[' + corresponding_code1[idx])
            else:
        ...
        for idx, name in enumerate(method_names2):
            ...
            print('对应csi代码： \\033[' + corresponding_code2[idx])
            print()
...
def usage_of_colors256():
    def attributes():
        ...
        description_of_attrs = [
            ...
            f"RESET_ALL  对应csi代码  \\033[0m  同Style.RESET_ALLs\n\n"
            f"RESET_FORE  对应csi代码  \\033[39m  同FORE.RESETs\n\n"
            f"RESET_BACK  对应csi代码  \\033[49m  同BACK.RESETs\n",
            f"256色的颜色代码范围为 0 - 255 \n",
            f"设置前景色的csi代码为 \\033[38;5;颜色代码m\n"
            f"对应属性 FORE0 - FORE255\n",
            f"设置背景色的csi代码为 \\033[48;5;颜色代码m\n"
        ...
        description_of_operation_rules_add = [
            ...
            f"这两个类都包含256色的csi控制头和相应的颜色代码\n"
            f"同时，这两个类继承自Csi256ColorsCode类\n",
            f"只要是Csi256ColorsCode类支持的运算，ForeCode，BackCode都支持\n",
            f"Csi256ColorsCode类可以进行的运算操作有\n"
            f"加，减，乘，整除，求模，关系运算\n"
            f"还可以被int()，str()这两个内置函数调用\n",
            f"下面简单介绍一下用法\n",
            f"加  add for Csi256ColorsCode\n",
            ...
            f"这样会进行如下运算操作\n"
            f"结果 = （ForeCode/BackCode对象对应的csi代码（字符串） + 字符串）\n"
            f"返回：拼接后的字符串\n",
            ...
            f"在生成Csi256Colors实例时就分别在这个实例中生成了256个ForeCode和256个BackCode对象\n",
            f"也就是说，Colors256对象包含512个Csi256ColorsCode的子类对象\n",
            f"运算结果如果是ForeCode/BackCode对象\n"
            f"就直接返回Csi256ColorsCode实例中对应的ForeCode/BackCode对象的引用\n",
            f"这算是一个折中的办法，虽然防止了重复生成对象，但大部分ForeCode/BackCode对象不一定会用到\n"
            f"所以非必要不要重复使用调用Csi256Colors生成实例\n",
            f"下面是一些示例代码\n",
        ]
...
""",
            ...
            [
                ...
                '否则会丢失原来的引用\n',
                '下面是一个错误的例子\n',
                '示例2 错误地对Csi256ColorsCode对象使用 += 运算\n',
"""
str_code = str([Colors256.FORE0])[2:-2]
# 由于\\033字符不可见，先用这个方式转成可显示的
print('Colors256.FORE0  对应值' + str_code)
    ...
    def callable_methods():
        ...
        def c256_usage_of_get_fore():
            description_of_c256_get_fore = [
                ...
                '返回值：256色前景色的csi代码和控制样式的csi代码拼接而成的字符串\n\n',
...
C256 = Colors256
csi_head = C256.get_fore(45, Style.BRIGHT, Style.UNDERLINE)
# 等同于代码 csi_head = C256.FORE45 + Style.BRIGHTs, Style.UNDERLINEs
# 这里就用到了Style的整数值属性
# 如果你记得住Style对应的整数代码，你也可以直接填数字
print(csi_head + 'example code 1 of Colors256.get_fore' + Style.RESET_ALLs)
""",
                    '运行结果如下\n'
        ...
        def c256_usage_of_get_back():
            description_of_c256_get_fore = [
                ...
                '参数 num：为256色的颜色代码，为int类型，范围0 - 255 \n\n'
                '参数 *args：任意数量的Style代码，为int类型，该数值必须存在与Style对象内\n\n'
                '返回值：256色背景色的csi代码和控制样式的csi代码拼接而成的字符串\n\n',
...
from conkits import Colors256, Style
C256 = Colors256
csi_head = C256.get_back(56, Style.BRIGHT, Style.UNDERLINE)
# 等同于代码 csi_head = C256.BACK56 + Style.BRIGHTs, Style.UNDERLINEs
# 这里用到了Style的整数值属性
# 如果你记得住Style对应的整数代码，你也可以直接填数字
print(csi_head + 'example code 1 of Colors256.get_back' + Style.RESET_ALLs)
""",
'运行结果如下\n'
        ...
        def c256_usage_of_get_double():
            description_of_c256_get_fore = [
                ...
                '参数 *args：任意数量的Style代码，为int类型，该数值必须存在与Style对象内\n\n'
                '返回值：256色前景色 + 背景色的csi代码和控制样式的csi代码拼接而成的字符串\n\n',
...
from conkits import Colors256, Style
C256 = Colors256
csi_head = C256.get_double(0, 208, Style.BRIGHT)
# 等同于代码 csi_head = C256.FORE0 + C256.BACK208 + Style.BRIGHTs
# 这里就用到了Style的整数值属性
# 如果你记得住Style对应的整数代码，你也可以直接填数字
print(csi_head + 'example code 1 of Colors256.get_double' + Style.RESET_ALLs)
""",
                    '运行结果如下\n'
    ...
    def short_description():
        c256_description = [
            f"Colors256是由位于模块conkits.csi256colors内的Csi256Colors类生成的实例\n",
            ...
            f"而现在大部分终端都支持256色\n"
            f"所以我增加了256色的封装\n",
            f"在后续我还会更新 24bit RGB色彩的csi代码的封装\n"
            f"不过或许会有些终端不支持，具体能不能用还得看你们使用的终端\n",
            f"在256色的封装中\n"
...
"""
        ...
        def conio_usage_of_get_csistr():
            get_csistr_description = [
                "Conio.get_csistr(self, *args)\n"
                "获取csi字符串\n",
            ...
            example_codes_of_get_csistr = [
                "示例代码\n"
"""
settings1 = Conio.get_csistr(Fore.BLUE, Back.YELLOW, Style.BRIGHT)
# 这是一种写法，比较麻烦
# 不如直接用字符串属性相加
settings2 = Conio.get_csistr(34, 43, 1, 4)
# 当你能记住相应的数值时
# 可以用这种较为简便的方法
...
""",
            ...
            print()
            if description_printer(get_csistr_description, interactive_lp):
                return
            if description_printer(example_codes_of_get_csistr, interactive_lp):
                return
            settings1 = Conio.get_csistr(Fore.BLUE, Back.YELLOW, Style.BRIGHT)
            # 这是一种写法，比较麻烦
            # 不如直接用字符串属性相加
...
dp = DynamicPrint(print_delay=0.05,
                  csi_code=Fore.YELLOWs + Colors256.BACK0,
                  step=2,
                  is_wait=True,
                  is_auto_reset=True,
...
dp.set_csi_code(Fore.BLUEs + Colors256.BACK0)
dp.set_step(3)
dp.is_wait=True,
...
# 步长明显大于字符串,
                ...
                [
                    "也是一次性就打印完了\n",
                    "下面看看csi_code对print的影响\n",
                    "这里我们使用的字符串中间的部分，也被设置了csi字符串，为绿色前景色\n"
                    "然后参数csi_code设置为Fore.YELLOWs（黄色）\n",
                    "示例代码如下\n",
                    """
...
dp.print(test_str, csi_code=Fore.YELLOWs)
                    """,
                    "运行结果为\n"
                ],
                [
                    "这里可以发现\n",
                    "没在字符串中设置csi字符串的部分显示为黄色\n",
                    "也就是说不被所给字符串中的csi字符串影响的部分\n"
                    "会被参数csi_code影响\n",
                    "下面再看一个例子\n"
...
dp.print(test_str, csi_code=Fore.YELLOWs)    
                ...
                [
                    ...
                    "后面除开绿色的部分都没设置颜色\n",
                    "为什么会出现这种情况？\n",
                    "这是因为只有被\\033[0m的影响的部分才会被设置为参数csi_code的颜色\n"
                    "（Style.RESET_ALLs即\\033[0m）\n",
                    "而Fore.RESETs是\\033[39m\n",
                    "所以被它影响的部分没有如期被参数csi_code的颜色\n",
                    "如果我们只想要字符串中一部分单独设置csi字符串\n"
                    "其他部分使用参数csi_code的颜色\n",
...
def usage_of_choice():
    def attribute():
        simple_description_of_dp_attributes = [
            ...
            "checked_csi_code  被选中时，选项使用的csi字符串\n",
            "影响选中时选项的显示效果\n"
            "建议此参数不设置前景色\n",
            "unchecked_csi_code  未选中时，选项使用的csi字符串\n",
            "影响未选中时选项的显示效果"
            "建议此参数不设置背景色\n",
            "click_csi_code  确认时，选项使用的csi字符串\n",
            "影响未选中时选项的显示效果\n",
...
Choice.__init__(self,
         ...
         methods=None,
         sep_line=None,
         checked_csi_code=None,
         unchecked_csi_code=None,
         click_csi_code=None,
...
qc = Choice(
    ...
    methods=example_methods,
    unchecked_csi_code='',
    checked_csi_code=Back.CYANs + Style.BRIGHTs,
    click_csi_code=Back.LIGHTREDs + Fore.BLACKs,
)
while True:
    idx = qc.run()
    if idx == len(example_options) - 1:
