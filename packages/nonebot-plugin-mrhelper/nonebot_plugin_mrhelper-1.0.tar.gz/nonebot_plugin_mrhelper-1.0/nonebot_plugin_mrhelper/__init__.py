...
ADMIN = plugin_config.mrhelper_admin
AUTOADDFRIEND = plugin_config.mrhelper_autoaddfriend
SUPERUSERS = plugin_config.superusers
if ADMIN is None:
    ADMIN = SUPERUSERS[0]
    logger.warning(f"没有设置主人的qq号（MRHELPER_ADMIN），自动采用{ADMIN}作为主人！")
COMMAND_START = plugin_config.command_start[0]
MR_URL = plugin_config.mrhelper_mrurl
MRDB = plugin_config.mrhelper_db
if MRDB[-11:] != "mrhelper.db":
    logger.warning("mr-helper数据库地址设置不正确！采用默认地址！")
    MRDB = "mrhelper.db"
if MR_URL is None:
...
search_keyword = on_command("搜片", priority=15)
sub_douban = on_command("订阅", priority=15)
get_site_overview = on_command("站点数据", priority=20)
...
async def _(event: Event):
    ...
    if db.Select("qid", qid) is not None:
        ...
        await login.finish(f"{qid}已登录过", at_sender=True)
    else:
        ...
        if len(msg) == 3:
            ...
            await login.finish(msg, at_sender=True)
        else:
            # 检测到无法识别的格式，返回提示，结束
            logger.warning("格式不正确 无法登陆")
            await login.finish(Message(f"请使用如下格式：\n{COMMAND_START}mr登录[空格]账号[空格]密码"), at_sender=True)
...
async def _(event: Event, msg: Message = Arg("keyword")):
    ...
    try:
        ...
        logger.warning(e)
        await search_keyword.finish(f"请先使用 '{COMMAND_START}登录[空格]账号[空格]密码' 登录账号！\n如确定已经登录，请查看日志定位问题！")
    await search_keyword.send(res_num, at_sender=True)
...
async def build_search_result_msg(qid, keyword):
    ...
    for key in range(lens):
        ...
        msg += MessageSegment.text(search_res['data'][key]['url'] + "\n")
...
async def send_forward_msg(session_id, fwd_msg):
    ...
    if type:
        ...
        try:
            ...
            logger.warning(f"发送群聊合并转发消息错误！请查看go-cqhttp端输出！\n大概率是账号被风控，尝试发送私聊合并转发消息！")
            try:
                await boter.send_private_forward_msg(user_id=qid, messages=fwd_msg)
                at = MessageSegment.at(qid)
                await Bot.send_group_msg(group_id=gid, message=at+"群聊消息被风控了 记得看私信哦~")
            except Exception:
                logger.warning(f"发送私聊合并转发消息错误！请查看go-cqhttp端输出！\n可能是没跟机器人加好友！")
                at = MessageSegment.at(qid)
                await Bot.send_group_msg(group_id=gid, message=at + "账号被风控或你没加我好友 反正就是消息没发成功")
        ...
        try:
            ...
            logger.warning(f"发送私聊合并转发消息错误！原因：{e}")
...
async def _(event: Event):
    ...
    info = db.Select("qid", qid)
    if not info:
        await sub_douban.finish("你忘记先登录了！")
    token = info[4]
    try:
        ...
        await sub_douban.finish("请检查序号是否输入正确！", at_sender=True)
    if num >= 100:
        logger.info(f"用户输入数字可能为豆瓣id而并非序号 直接订阅")
        res = await mr_api.submit(MR_URL, token, num + 1)
        if not res:
            await sub_douban.finish("错误！请查看日志定位问题！")
        await sub_douban.finish(f"豆瓣id：{num}影片已提交订阅\napi返回消息：{res}", at_sender=True)
    if len(search_res) == 0:
        await sub_douban.finish("请先使用 #mr搜片+片名 进行搜索再选取！", at_sender=True)
    ...
    if search_res["data"][num]["sub_id"] is not None:
        ...
        search_res = ""
        await sub_douban.finish(f"{douban_name} ({douban_id})已经在库中！", at_sender=True)
...
async def _(event: Event):
    ...
    if not data:
        await get_site_overview.finish("错误！请查看日志定位问题！")
    elif data["code"] == 1:
        reason = data["message"]
        await get_site_overview.finish(f"api返回消息：{reason}\n大概率是你忘记登录了")
...
async def _(event: Event):
    ...
    if db.Select("admin", 1) is not None:
        ...
        await register.finish("没有登录管理员账号 无法注册新账号！", at_sender=True)
    msg = str(event.get_message())
    msg = msg.split(' ', 2)
    if len(msg) != 3:
    ...
    if not res:
        await register.send(f"mr注册：{username}已经被注册了\n（如确定没有，请自行查看日志定位问题！）", at_sender=True)
    else:
        await register.send(f"mr注册：api返回消息：{res}\n请自行判断是否注册成功", at_sender=True)
    if ENABLE_REGISTEREMBY:
        msg = await register_emby(username)
        await register.finish(msg, at_sender=True)
    else:
...
async def _(event: Event):
    ...
    info = db.Select("qid", qid)
    if not info:
        await search_in_library.finish("你忘记先登录了！")
    token = info[4]
    ...
    if len(res["data"]) == 0:
        ...
        try:
            ...
            await search_in_library.finish("从tmdb下载图片超时！请自行解决")
            logger.warning(e)
    else:
        fwd_msg = []
...
async def build_media_info_msg(media_info):
    ...
    poster_img = MessageSegment.image(media_info.poster_url, timeout=10)
