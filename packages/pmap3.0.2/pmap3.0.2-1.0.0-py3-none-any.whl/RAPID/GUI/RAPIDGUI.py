...
### TODO: Add parameters for all functions so that everything can be done in Jupyter.
...
class RAPIDGUI():
    ...
    def __init__(self):
        ...
        self.currentlowerbounds = []
        self.currentlyselectedcells = []
        self.currentlyselectedobjectclusters = []
        self.currentlyselectedpixelclusters = []
        ### TODO: Reset to empty list when object clusters not displayed in table?
        self.currentobjectclusterorder = []
        ### TODO: Reset to empty list when pixel clusters not displayed in table?
        self.currentpixelclusterorder = []
        ### TODO: Reset to empty list when segmentation not displayed in table?
        self.currenttablecellorder = []
        self.currenttableorderfull = []
        self.currenttabdata = np.array([])
        self.currentupperbounds = []
    ...
    def apply_RAPID_pixel(self, tab, args, colors, outpath=""):
        ...
        if not self.hasaddedtable:
            self.currentlowerbounds = copy.deepcopy(self.minvalspixel[self.analysisindex])
            self.currentupperbounds = copy.deepcopy(self.maxvalspixel[self.analysisindex])
            self.update_table(data[0, :, :], self.currentlowerbounds, self.currentupperbounds, data.shape[1])
        clusterlist = []
        s = self.labeledpixels[-1].shape
        volimage = np.zeros((s[0], s[1], s[2], 3)).astype(np.uint8)
        # color = generate_colormap(args.ncluster + 1)
        for j in range(args.ncluster):
            mask = self.labeledpixels[-1] == j
        ...
        for i in range(len(data)):
            self.pixelclusterdata.append(data[i, :, :])
        self.currentpixelclusterorder = copy.deepcopy(self.currenttableorderfull)
        if outpath != "":
            for i in range(self.numimgs):
                if self.imageisflipped[i]:
        ...
        if selectimagesadded.grey:
            self.viewer.add_image(self.labeledpixels[-1] + 1, name=f"Pixel Cluster IDs {self.pixelclustercount + 1}",
                                  blending="additive", contrast_limits=[0, np.max(self.labeledpixels[-1]) + 1])
        if selectimagesadded.color:
            self.viewer.add_image(volimage, name=f"Pixel Clusters {self.pixelclustercount + 1}", blending="additive")
    ...
    def biaxial_gate(self):
        ...
        numcells = 0
        for i in range(self.numimgs):
            numcells += len(self.segmentationdata[segindex * self.numimgs + i])
        fullquantified = np.zeros((numcells, self.segmentationdata[segindex * self.numimgs].shape[1]))
        count = 0
        cellsperimage = []
        for i in range(self.numimgs):
            cellsincurrimg = []
            ...
            currentimage = self.segmentationdata[segindex * self.numimgs + i]
            for j in range(count, count + len(self.segmentationdata[segindex * self.numimgs + i])):
        ...
        if len(self.segmentationclusteringrounds[segindex]) > 0:
            ...
            if biaxial.colorbyindivclusters or biaxial.colorbycombclusters:
                if len(self.segmentationclusteringrounds[segindex]) > 1:
                    iteration = GUIUtils.ObjectTrainIteration(self.segmentationclusteringrounds[biaxial.imageindex])
                    iteration.exec()
                    if not iteration.OK:
                        return
                    startindex = self.segmentationclusteringrounds[biaxial.imageindex][iteration.iteration]
                    ...
                    GUIUtils.log_actions(self.actionloggerpath,
                                         ...
                                         f"Normalization ({biaxial.norm}), Segmentation Iteration ({biaxial.imageindex}), "
                                         f"Coloring ({biaxial.colorbyindivclusters}, {biaxial.colorbycombclusters}, {biaxial.colorbygroups})")
                    startindex = self.segmentationclusteringrounds[biaxial.imageindex][0]
                clusternums = []
                for i in range(self.numimgs):
                    curclusternums = self.cellclustervals[startindex * self.numimgs + i]
                    for n in curclusternums:
                        clusternums.append(n - 1)
                numclusters = len(np.unique(self.labeledobjects[startindex])) - 1
        ...
        count = 0
        for i in range(self.numimgs):
            coordslist.append(coords[count:count + len(self.segmentationdata[segindex * self.numimgs + i])].astype(np.float))
            count += len(self.segmentationdata[segindex * self.numimgs + i])
        self.plotcoordinates.append(coordslist)
        self.plotisumap.append(False)
        self.biaxialcount += 1

    def calculate_table_cell_color(self, analysismode):
        color = QColor(0, 0, 0)
    ...
    def change_folder(self):
        ...
        if outputfolder != "":
            ...
            self.pixelclusterdirectories = [path.replace(self.outputfolder, outputfolder) for path in
                                            self.pixelclusterdirectories]
            self.segmentationzarrpaths = [path.replace(self.outputfolder, outputfolder) for path in self.segmentationzarrpaths]
    ...
    def colormap_group(self):
        ...
        if self.analysismode == "Pixel":
            nc = len(np.unique(self.labeledpixels[ind]))
        elif self.analysismode == "Object":
            nc = len(np.unique(self.labeledobjects[ind])) - 1
        ...
        if self.analysismode == "Pixel":
            self.pixelclustercolors[ind] = cmapwidget.newcolorlist
            data = self.pixelclusterdata[tabindex]
            index = [f"RP-{i + 1}" for i in range(nc)]
            cols = self.fulltab.columns[3:]
        else:
            self.objectclustercolors[ind] = cmapwidget.newcolorlist
            data = self.objectclusterdata[tabindex][:, 1:-4]
            ...
            for i, item in enumerate(data[key]):
                ...
                if i == 0:
                    ...
                    if self.analysismode == "Object" and numrows > 4:
                        minv = self.minvalsobject[self.analysisindex][j - 2]
                        maxv = self.maxvalsobject[self.analysisindex][j - 2]
                        trainindex, numtabs = GUIUtils.find_analysis_round(self.analysisindex, self.numimgs)
                        ...
                        adj = (data[key][i] - minv) / (maxv - minv) * maxsegment[j - 2] / np.max(np.asarray(maxsegment[:-4]))
                    elif self.analysismode == "Segmentation" and numrows > 4:
                        minv = self.minvalssegment[self.analysisindex][j - 1]
                        maxv = self.maxvalssegment[self.analysisindex][j - 1]
                        adj = (data[key][i] - minv) / (maxv - minv)
                    elif self.analysismode == "Pixel":
                        minv = self.minvalspixel[self.analysisindex][j - 2]
                        maxv = self.maxvalspixel[self.analysisindex][j - 2]
                        adj = ((data[key][i] - minv) / (maxv - minv) * self.maxpixelclustervals[j - 2]) / 255
                    else:
                        adj = 0.5
                    if math.isnan(adj):
                        adj = 0.5
    ...
    def display_selected_cells(self):
        ...
        segindex = self.plotsegmentationindices[it]
        for shape in range(len(self.viewer.layers[ind].data)):
            ...
            verts[:, 0] = ((self.plotxmaxs[it] - verts[:, 0]) / (self.plotxmaxs[it] - self.plotxmins[it])) * 1.1 - 0.05
            verts[:, 1] = ((verts[:, 1] - self.plotymins[it]) / (self.plotymaxs[it] - self.plotymins[it])) * 1.1 - 0.05
            verts[:, [0, 1]] = verts[:, [1, 0]]
            ...
            for i in range(self.numimgs):
                ...
                totalcells += len(self.plotcoordinates[it][i])
                mask = p.contains_points(self.plotcoordinates[it][i])
                rows = list(self.segmentationdata[segindex + i][mask, 0].astype(int))
                inclrows.append([x - 1 for x in rows])
                selectedcells += len(rows)

                # Mask the image for the selected cells.
                arr = copy.deepcopy(self.labeledsegmentation[segindex + i])
                arr[np.isin(self.labeledsegmentation[segindex + i], rows, invert=True)] = 0
                arr = self.method_searchsort(np.unique(arr), np.array([j for j in range(len(np.unique(arr)))]), arr)
                self.labeledsegmentation.append(arr)
                masks[i, :len(arr), :arr.shape[1]][arr > 0] = 1
            ...
            if selectedcells == 0:
                ...
                for i in range(self.numimgs):
                    ...
                    newentry = self.segmentationdata[segindex + i][inclrows[i], :]
                    for j in range(len(newentry)):
                        newentry[j, 0] = j + 1
                    self.segmentationdata.append(newentry)
                    ...
                    minvals = []
                    maxvals = []
                    for j in range(self.segmentationdata[segindex + i].shape[1] - 1):
                        try:
                            minvals.append(np.min(self.segmentationdata[segindex + i][inclrows[i], j + 1]))
                            maxvals.append(np.max(self.segmentationdata[segindex + i][inclrows[i], j + 1]))
                        except:
                            minvals.append(0)
                            maxvals.append(0)
                    ...
                    self.cellidorders.append(inclrows[i])
                    self.currenttablecellorder.append(inclrows[i])
    ...
    def filter_table(self):
        ...
        if tablefilters.reset:
            ...
            if self.analysismode == "Segmentation":
                ...
                self.currentlowerbounds = copy.deepcopy(self.minvalssegment[self.analysisindex])
                self.currentupperbounds = copy.deepcopy(self.maxvalssegment[self.analysisindex])
                #ind = self.segmentationindices.index(self.tableindex)
                #startind = (ind // self.numimgs) * self.numimgs
                self.lowerboundslist[self.tableindex] = copy.deepcopy(self.minvalssegment[self.analysisindex])
                self.upperboundslist[self.tableindex] = copy.deepcopy(self.maxvalssegment[self.analysisindex])
                self.currenttablecellorder[self.analysisindex] = copy.deepcopy(self.cellidorders[self.analysisindex])
                self.update_table(self.segmentationdata[self.analysisindex][self.cellidorders[self.analysisindex], 1:], self.currentlowerbounds,
                                  self.currentupperbounds, len(self.segmentationdata[self.analysisindex]), self.currenttablecellorder[self.analysisindex])

            # If the current table corresponds to object-based clustering.
            elif self.analysismode == "Object":
                # Reset lower/upper bounds and use the full dataset to display in the table.
                self.currentlowerbounds = copy.deepcopy(self.minvalsobject[self.analysisindex])
                self.currentupperbounds = copy.deepcopy(self.maxvalsobject[self.analysisindex])
                self.lowerboundslist[self.tableindex] = copy.deepcopy(self.minvalsobject[self.analysisindex])
                self.upperboundslist[self.tableindex] = copy.deepcopy(self.maxvalsobject[self.analysisindex])
                self.currentobjectclusterorder = copy.deepcopy(self.currenttableorderfull)
                self.update_table(self.objectclusterdata[self.analysisindex][self.currenttableorderfull, :], self.currentlowerbounds,
                                  self.currentupperbounds, len(self.currenttableorderfull), self.currenttableorderfull)
            ...
            # If the current table corresponds to pixel-based clustering.
                ...
                self.currentlowerbounds = copy.deepcopy(self.minvalspixel[self.analysisindex])
                self.currentupperbounds = copy.deepcopy(self.maxvalspixel[self.analysisindex])
                self.lowerboundslist[self.tableindex] = copy.deepcopy(self.minvalspixel[self.analysisindex])
                self.upperboundslist[self.tableindex] = copy.deepcopy(self.maxvalspixel[self.analysisindex])
                self.currentpixelclusterorder = copy.deepcopy(self.currenttableorderfull)
                self.update_table(self.pixelclusterdata[self.analysisindex][self.currenttableorderfull, :], self.currentlowerbounds, self.currentupperbounds,
                                  len(self.pixelclusterdata[self.analysisindex]), self.currenttableorderfull)
        ...
        # If applying a new filter, add to the existing filters and update the table accordingly.
            ...
            if row == 0 and c >= 0:
                self.currentlowerbounds[c] = tablefilters.val
                # Lower bound must be smaller than upper bound.
                if self.currentlowerbounds[c] > self.currentupperbounds[c]:
                    self.currentlowerbounds[c] = self.currentupperbounds[c]
                #if self.analysismode == "Segmentation":
                #    ind = self.segmentationindices.index(self.tableindex)
                #    startind = (ind // self.numimgs) * self.numimgs
                #    for i in range(self.numimgs):
                #        self.lowerboundslist[startind + i] = copy.deepcopy(self.currentlowerbounds)
                #else:
                #    self.lowerboundslist[self.tableindex] = copy.deepcopy(self.currentlowerbounds)
                self.lowerboundslist[self.tableindex] = copy.deepcopy(self.currentlowerbounds)

            # If user adjusts upper bound, store that for future reference.
            elif row == 1 and c >= 0:
                self.currentupperbounds[c] = tablefilters.val
                # Lower bound must be smaller than upper bound.
                if self.currentupperbounds[c] < self.currentlowerbounds[c]:
                    self.currentupperbounds[c] = self.currentlowerbounds[c]
                #if self.analysismode == "Segmentation":
                #    ind = self.segmentationindices.index(self.tableindex)
                #    startind = (ind // self.numimgs) * self.numimgs
                #    for i in range(self.numimgs):
                #        self.upperboundslist[startind + i] = copy.deepcopy(self.currentupperbounds)
                #else:
                #    self.upperboundslist[self.tableindex] = copy.deepcopy(self.currentupperbounds)
                self.upperboundslist[self.tableindex] = copy.deepcopy(self.currentupperbounds)
            ...
            if self.analysismode == "Segmentation":
                ...
                filteredmarkers = []
                for i in range(len(self.currentlowerbounds)):
                    if self.currentlowerbounds[i] > self.minvalssegment[self.analysisindex][i] or self.currentupperbounds[i] < \
                            self.maxvalssegment[self.analysisindex][i]:
                ...
                currentdata = self.segmentationdata[self.analysisindex][self.cellidorders[self.analysisindex], 1:]

                # Store segmentation data table, and append index values at the end to log sort order.
                filtereddata = np.append(self.segmentationdata[self.analysisindex][self.cellidorders[self.analysisindex], 1:], np.expand_dims(np.arange(len(self.segmentationdata[self.analysisindex])), 1), 1)

                # Filter cells one marker at a time according to current lower- and upper-bounds.
                for marker in filteredmarkers:
                    filtermask = (np.round(filtereddata[:, marker], 3) <= np.round(self.currentupperbounds[marker], 3))
                    filtereddata = filtereddata[filtermask]
                    filtermask = (np.round(filtereddata[:, marker], 3) >= np.round(self.currentlowerbounds[marker], 3))
                    filtereddata = filtereddata[filtermask]

                # Update the list of cell IDs included in the table for each image.
                self.currenttablecellorder[self.analysisindex] = [self.cellidorders[self.analysisindex][j] for j in filtereddata[:, -1].astype(np.int).tolist()]
                ...
                self.update_table(currentdata, self.currentlowerbounds, self.currentupperbounds,
                                  len(self.segmentationdata[self.analysisindex]), self.currenttablecellorder[self.analysisindex])

                # If any cells are included, and at least one cell is filtered out, add an image to the
                # viewer containing the included cells.
                self.set_invisible(self.viewer)
                if len(self.segmentationdata[self.analysisindex]) > len(currentdata) > 0:
                    for i in range(self.numimgs):
                        if i == self.analysisindex % self.numimgs:
                            mask = np.in1d(self.labeledsegmentation[self.analysisindex],
                                           self.segmentationdata[self.analysisindex][np.asarray(self.currenttablecellorder[self.analysisindex]).astype(np.int), 0])
                            mask = mask.reshape((1, self.maximageshape[0], self.maximageshape[1]))
                        else:
                            mask = np.zeros((1, self.maximageshape[0], self.maximageshape[1]), dtype=np.bool)
                        if i == 0:
                ...
                for i in range(len(self.segmentationdata[self.analysisindex])):
                    if i in self.currentlyselectedcells[self.analysisindex] and self.cellidorders[self.analysisindex][i] not in \
                            self.currenttablecellorder[self.analysisindex]:
                        self.currentlyselectedcells[self.analysisindex].remove(i)
            ...
            # If filtering an object clustering table.
                ...
                filteredmarkers = []
                for i in range(len(self.currentlowerbounds)):
                    if (self.currentlowerbounds[i] > self.minvalsobject[self.analysisindex][i] or self.currentupperbounds[i] <
                            self.maxvalsobject[self.analysisindex][i]):
                        filteredmarkers.append(i)

                # Find object clustering data table, and append index values at the end to log sort order.
                cData = np.append(self.objectclusterdata[self.analysisindex][self.currenttableorderfull, 1:],
                                  np.expand_dims(np.arange(len(self.objectclusterdata[self.analysisindex])), 1), 1)

                # Filter the table one marker at a time according to current lower- and upper-bounds.
                for marker in filteredmarkers:
                    mask = (cData[:, marker] <= self.currentupperbounds[marker])
                    cData = cData[mask]
                    mask = (cData[:, marker] >= self.currentlowerbounds[marker])
                    cData = cData[mask]

                # Store cluster IDs that will be included in the table, and in the proper order.
                self.currentobjectclusterorder = [self.currenttableorderfull[i] for i in cData[:, -1].astype(np.int).tolist()]

                # Update the table with quantified values for the included clusters.
                currentdata = self.objectclusterdata[self.analysisindex][self.currenttableorderfull, :]
                currentdata = currentdata[cData[:, -1].astype(np.int).tolist(), :]
                self.update_table(currentdata, self.currentlowerbounds, self.currentupperbounds,
                                  len(self.objectclusterdata[self.analysisindex]), self.currentobjectclusterorder)
                ...
                trainnum, _ = GUIUtils.find_analysis_round(self.analysisindex, self.numimgs)
                if len(self.objectclusterdata[self.analysisindex]) > len(currentdata) > 0:
                    currentclusters = np.zeros_like(self.labeledobjects[trainnum])
                    for index in self.currentobjectclusterorder:
                        currentclusters[self.labeledobjects[trainnum] == index+1] = 1
                ...
                for i in range(len(currentdata)):
                    if self.currenttableorderfull[i] in self.currentlyselectedobjectclusters[trainnum] and self.currenttableorderfull[i] not in self.currentobjectclusterorder:
                        self.currentlyselectedobjectclusters[trainnum].remove(self.currenttableorderfull[i])
            ...
            # If filtering a pixel clustering table.
                ...
                filteredmarkers = []
                for i in range(len(self.currentlowerbounds)):
                    if (self.currentlowerbounds[i] > self.minvalspixel[self.analysisindex][i] or self.currentupperbounds[i] <
                            self.maxvalspixel[self.analysisindex][i]):
                        filteredmarkers.append(i)

                # Find pixel clustering data table, and append index values at the end to log sort order.
                cData = np.append(self.pixelclusterdata[self.analysisindex][self.currenttableorderfull, 1:],
                                  np.expand_dims(np.arange(len(self.pixelclusterdata[self.analysisindex])), 1), 1)

                # Filter the table one marker at a time according to current lower- and upper-bounds.
                for marker in filteredmarkers:
                    filtermask = (cData[:, marker] <= self.currentupperbounds[marker])
                    cData = cData[filtermask]
                    filtermask = (cData[:, marker] >= self.currentlowerbounds[marker])
                    cData = cData[filtermask]

                # Store cluster IDs that will be included in the table, and in the proper order.
                self.currentpixelclusterorder = [self.currenttableorderfull[i] for i in cData[:, -1].astype(np.int).tolist()]

                # Update the table with quantified values for the included clusters.
                currentdata = self.pixelclusterdata[self.analysisindex][self.currenttableorderfull, :]
                currentdata = currentdata[cData[:, -1].astype(np.int).tolist(), :]
                self.update_table(currentdata, self.currentlowerbounds, self.currentupperbounds,
                                  len(self.pixelclusterdata[self.analysisindex]), self.currentpixelclusterorder)

                # If any clusters are included, and at least one cluster is filtered out, add an image to the
                # viewer containing the cells in the included clusters.
                trainnum, _ = GUIUtils.find_analysis_round(self.analysisindex, self.numimgs)
                if len(self.pixelclusterdata[self.analysisindex]) > len(currentdata) > 0:
                    currentclusters = np.zeros_like(self.labeledpixels[trainnum])
                    for index in self.currentpixelclusterorder:
                        currentclusters[self.labeledpixels[trainnum] == index] = 1
                    self.viewer.add_image(currentclusters, name="Filter", blending="additive",
                                          visible=True)

                # Un-check clusters that are not included in the filter.
                for i in range(len(currentdata)):
                    if self.currenttableorderfull[i] in self.currentlyselectedpixelclusters[trainnum] and self.currenttableorderfull[i] not in self.currentpixelclusterorder:
                        self.currentlyselectedpixelclusters[trainnum].remove(self.currenttableorderfull[i])
    ...
    def generate_RAPID_data(self, markerindices, markernames, outfolder, denoise, normalizeeach, normalizeall, normtype,
        ...
        ### TODO: Shouldn't overwrite every time we do pixel clustering.
        self.maxpixelclustervals = []
        for i in range(self.nummarkers):
            self.maxpixelclustervals.append(np.amax(self.viewer.layers[i].data))
        fh.create_dataset('minmax', data=self.maxpixelclustervals, dtype='f8')
    ...
    def load_environment(self):
        ...
        self.cellidorders = ast.literal_eval(config.get("Variables", 'cellidorders'))
        ...
        self.currentlowerbounds = ast.literal_eval(config.get("Variables", 'currentlowerbounds'))
        self.currentlyselectedcells = ast.literal_eval(config.get("Variables", 'currentlyselectedcells'))
        self.currentlyselectedobjectclusters = ast.literal_eval(config.get("Variables", 'currentlyselectedobjectclusters'))
        self.currentlyselectedpixelclusters = ast.literal_eval(config.get("Variables", 'currentlyselectedpixelclusters'))
        self.currentobjectclusterorder = ast.literal_eval(config.get("Variables", 'currentobjectclusterorder'))
        self.currentpixelclusterorder = ast.literal_eval(config.get("Variables", 'currentpixelclusterorder'))
        self.currenttablecellorder = ast.literal_eval(config.get("Variables", 'currenttablecellorder'))
        self.currentupperbounds = ast.literal_eval(config.get("Variables", 'currentupperbounds'))
        self.currentverticalheaderlabels = np.array(ast.literal_eval(config.get("Variables", 'currentverticalheaderlabels')))
        ...
        self.imageisflipped = pd.DataFrame(ast.literal_eval(config.get("Variables", 'imageisflipped')))
        ...
        self.labeledobjects = [np.array(l) for l in ast.literal_eval(config.get("Variables", 'labeledobjects'))]
        self.labeledpixels = [np.array(l) for l in ast.literal_eval(config.get("Variables", 'labeledpixels'))]
        self.labeledsegmentation = [np.array(l) for l in ast.literal_eval(config.get("Variables", 'labeledsegmentation'))]
        ...
        self.objectclustercolors = [np.array(l) for l in ast.literal_eval(config.get("Variables", 'objectclustercolors'))]
        self.objectclusterdata = [np.array(l) for l in ast.literal_eval(config.get("Variables", 'objectclusterdata'))]
        ...
        self.pixelclusterdata = [np.array(l) for l in ast.literal_eval(config.get("Variables", 'pixelclusterdata'))]
        ...
        self.segmentationdata = [np.array(l) for l in ast.literal_eval(config.get("Variables", 'segmentationdata'))]
        ...
        if self.hasaddedtable:
            ...
            self.update_table(self.currenttabdata, self.currentlowerbounds, self.currentupperbounds, self.totalnumrows, self.tableorder)
    ...
    def load_object_clusters(self):
        ...
        for tab_len in range(self.numimgs):
            complete_tab.append(self.segmentationdata[tab_len + startind][:, markerinds])
        ...
        relabledgreyimages = np.zeros((self.numimgs, len(self.labeledsegmentation[0]), self.labeledsegmentation[0].shape[1]),
                                      dtype=np.uint8)
        ...
        for i in range(self.numimgs):
            ...
            from_values = complete_tab_DF['Label'].values[startindex:startindex + len(self.segmentationdata[i + startind])]
            tmp_to_values = to_values[startindex:startindex + len(self.segmentationdata[i + startind])].values
            self.cellclustervals.append(copy.deepcopy(tmp_to_values))
            relabeled = self.method_searchsort(from_values, tmp_to_values,
                                               self.labeledsegmentation[i + startind].flatten().astype(int))
            relabledgreyimages[i, :, :] = (relabeled.reshape(self.labeledsegmentation[i + startind].shape)).astype(np.uint8)
            relabledgreyimages[i, :, :][self.labeledsegmentation[i + startind] == 0] = 0
            cv.imwrite(os.path.join(dir, f"RELABELED_Grey{i}.png"), relabledgreyimages[i, :self.imageshapelist[i][0], :self.imageshapelist[i][1]] + 1)

            # Apply the colormap to the relabeled image and save it to the output folder.
            relabledimages = np.zeros((len(self.labeledsegmentation[0]), self.labeledsegmentation[0].shape[1], 3), dtype=np.uint8)
            ...
            if i == 0:
                ...
                relab[0, :len(relabledimages), :relabledimages.shape[1], :] = relabledimages
                if selectimagesadded.grey:
            ...
            tmp_tab = relabeled_table[startindex:startindex + len(self.segmentationdata[i])].values
            tmp_tab_df = pd.DataFrame(tmp_tab)
            startindex += len(self.segmentationdata[i])
            ...
            self.objectclusterdata.append(clusteravgs[:, 2:].astype(np.float))
        ...
        if self.numimgs > 1:
            ...
            for i in range(len(fullclusterdata)):
                ...
                weighted_average[currcluster, 1:] += fullclusterdata[i, 3:] * fullclusterdata[i, 2] / weighted_average[
                    currcluster, 0]
            self.objectclusterdata.append(weighted_average)
            minvals = []
            maxvals = []
        ...
        self.labeledobjects.append(relabledgreyimages)
    ...
    def load_pixel_results(self):
        ...
        tab = np.vstack([tab[i * args.ncluster:(i + 1) * args.ncluster, :] for i in range(len(imgnames)) if i in imagenums])
        ...
        self.maxpixelclustervals = list(inputzarr['minmax'][:])
        if not max(self.maxpixelclustervals) > 1.0:
            self.maxpixelclustervals = [a * 255. for a in self.maxpixelclustervals]
        self.markernums = [i for i in range(len(self.markers))]
        self.analysismode = "Pixel"
        self.labeledpixels.append(greyimgs)
    ...
    def load_segmentation_results(self):
        ...
        if os.path.exists(os.path.join(os.path.split(filenames[0])[0], "RawImages")):
            ...
            for path in filenames:
                ...
                labelimages[:len(loadedimg), :loadedimg.shape[1]] = loadedimg
        ...
        for path in filenames:
            ...
            # all_labelscombined[i,:len(all_labels),:all_labels.shape[1]] = all_labels
            ...
            self.set_invisible(self.viewer)
            self.viewer.layers[-1].visible = True
            self.segmentationdata.append(imgmean)
            ...
            self.minvalssegment.append(copy.deepcopy(minvals))
            self.maxvalssegment.append(copy.deepcopy(maxvals))
            self.labeledsegmentation.append(labelimages)
        ...
        self.currenttablecellorder = []
        for i in range(len(self.segmentationdata)):
            order = []
            for j in range(len(self.segmentationdata[i])):
                order.append(int(self.segmentationdata[i][j, 0] - 1))
            self.cellidorders.append(order)
            self.currenttablecellorder.append(order)
        self.segmentationclusteringrounds.append([])
        for i in range(len(self.segmentationdata)):
            self.currentlyselectedcells.append([])
        self.sorttableimages.data.choices = tuple(self.tableimagenames)
        ...
        if not self.hasaddedtable:
            self.currentlowerbounds = copy.deepcopy(self.minvalssegment[0])
            self.currentupperbounds = copy.deepcopy(self.maxvalssegment[0])
            self.update_table(self.segmentationdata[0][:, 1:], self.currentlowerbounds, self.currentupperbounds, len(self.segmentationdata[0]),
                              self.segmentationdata[0][:, 0].astype(np.uint8).tolist())
        return filenames
    ...
    def manual_annotation(self):
        ...
        to_values = [np.zeros(len(v)) for v in self.plotcoordinates[it]]
        ...
        for shape in range(len(verts)):
            ...
            tupverts[:, 0] = ((self.plotxmaxs[it] - tupverts[:, 0]) / (self.plotxmaxs[it] - self.plotxmins[it])) * 1.1 - 0.05
            tupverts[:, 1] = ((tupverts[:, 1] - self.plotymins[it]) / (self.plotymaxs[it] - self.plotymins[it])) * 1.1 - 0.05
        ...
        fulltab = []
        for tab_len in range(self.numimgs):
            fulltab.append(self.segmentationdata[tab_len + segindex][:, :len(self.markers) + 1])
        ...
        for i in range(self.numimgs):
            ...
            from_values = fulltabDF['Label'].values[startindex:startindex + len(self.segmentationdata[i + segindex])]
            tmp_to_values = to_values[startindex:startindex + len(self.segmentationdata[i + segindex])]
            self.cellclustervals.append(copy.deepcopy(tmp_to_values))
            relabeled = self.method_searchsort(from_values, tmp_to_values, self.labeledsegmentation[i + segindex].astype(int))
            relabeledgreyimages[i, :len(relabeled), :relabeled.shape[1]] = relabeled.astype(np.uint8)
            relabeledgreyimages[i, :len(relabeled), :relabeled.shape[1]][self.labeledsegmentation[i + segindex] == 0] = 0
            ...
            if i == 0:
                ...
                self.viewer.add_image(relab, name=f"Object Clusters {self.objectclustercount + 1}", blending="additive")
            ...
            currenttab = fulltab[startindex:startindex + len(self.segmentationdata[i + segindex])]
            ...
            self.objectclusterdata.append(clustertab[:, 2:].astype(np.float))
            ...
            self.lowerboundslist.append(copy.deepcopy(minvals))
            self.upperboundslist.append(copy.deepcopy(maxvals))
            startindex += len(self.segmentationdata[i + segindex])
        ...
        self.labeledobjects.append(relabeledgreyimages)
        ...
        imgid = [np.repeat(i, len(self.segmentationdata[i + self.segmentationindicesinumap[umapit] * self.numimgs])) for i in
                 range(self.numimgs)]
        fulltabDF.insert(1, "ImgID", np.hstack(imgid))
        cord = np.vstack(self.cellcoordinates[self.segmentationindicesinumap[umapit]])
        fulltabDF.insert(1, "Y", cord[:, 1])
        fulltabDF.insert(1, "X", cord[:, 0])
        self.objectclusterdfs.append(fulltabDF)
        fulltabDF.to_csv(os.path.join(dir, "FullObject_segmentation_dataCluster.csv"))

        # Update any necessary variables.
        self.clustersarepixelbased.append(False)
        self.segmentationclusteringrounds[self.segmentationindicesinumap[umapit]].append(self.objectclustercount)
    ...
    def merge_clusters(self, checked=[]):
        ...
        n, numtabs = GUIUtils.find_analysis_round(self.analysisindex, self.numimgs)
        ...
        if self.analysismode == "Pixel":
            ...
            numclusters = len(self.pixelclusterdata[self.analysisindex]) - len(checked) + 1
            numcols = self.pixelclusterdata[self.analysisindex].shape[1] + 2
            ...
            for i in range(numtabs):
                data = copy.deepcopy(self.pixelclusterdata[n * numtabs + i])
                ...
                self.pixelclusterdata[n * numtabs + i] = table[notremoved, :]
                if i < self.numimgs:
            ...
            grey = copy.deepcopy(self.labeledpixels[n])
            newclusterval = int(np.max(grey) + 1)

            ind = [i for i,m in enumerate(self.clustersarepixelbased) if m][n]
            if not self.clusternames[ind] == []:
                name = self.clusternames[ind][checked[-1]]
                for cluster in checked:
                    self.clusternames[ind].pop(cluster)
                self.clusternames[ind].append(name)

            for cluster in checked:
                grey[grey == cluster] = newclusterval
                grey[grey > cluster] = grey[grey > cluster] - 1
                newclusterval -= 1
                self.currentpixelclusterorder.remove(cluster)
                ### TODO: Is this necessary?
                self.currentpixelclusterorder = np.array(self.currentpixelclusterorder)
                self.currentpixelclusterorder[self.currentpixelclusterorder > cluster - 1] -= 1
                self.currentpixelclusterorder = list(self.currentpixelclusterorder)
                index = f"Cluster {cluster + 1} (Pixel [{n}])"
                for i in reversed(range(len(self.viewer.layers))):
                    if self.viewer.layers[i].name == index:
                        self.viewer.layers.pop(i)
                        break
            self.currentpixelclusterorder.append(newclusterval)
            self.labeledpixels[n] = grey

            # Create a colored image from the newly-labeled image, and add it to the viewer.
            newrgb = np.zeros((len(grey), grey.shape[1], grey.shape[2], 3)).astype(np.uint8)

            color = self.pixelclustercolors[n]
            newcolor = color[[checked[-1]], :]
            color = np.delete(color, checked, 0)
            color = np.append(color, newcolor, 0)
            self.pixelclustercolors[n] = color
            ...
            self.currenttableorderfull = self.currentpixelclusterorder
            newdisplaydata = self.pixelclusterdata[self.analysisindex][self.currentpixelclusterorder, :]
            self.update_table(newdisplaydata, self.currentlowerbounds, self.currentupperbounds, len(self.currentpixelclusterorder),
                              self.currentpixelclusterorder)
        ...
        # If the uer is merging object-based clusters.
            ...
            numclusters = len(self.objectclusterdata[self.analysisindex]) - len(checked) + 1
            numcols = self.objectclusterdata[self.analysisindex].shape[1] + 2
            fulltab = np.zeros((self.numimgs * numclusters, numcols))
            # Remove the checked clusters from the table and merge them together as a new entry at the end.
            count = 0
            for i in range(numtabs):
                data = self.objectclusterdata[n * numtabs + i]
                table = np.zeros((len(data) + 1, data.shape[1]))
                table[:len(data), :] = data
                table[len(data), 0] = np.sum(data[checked, 0])
                if table[len(data), 0] > 0:
                    ...
                    table[len(data), 1:] = np.zeros_like(table[len(data), 1:])
                notremoved = [j for j in range(len(table)) if j not in checked]
                self.objectclusterdata[n * numtabs + i] = table[notremoved, :]
                if i < self.numimgs:
                    fulltab[count:count + len(notremoved), 0] = i + 1
                    fulltab[count:count + len(notremoved), 1] = np.arange(1, len(notremoved) + 1)
                    fulltab[count:count + len(notremoved), 2:] = table[notremoved, :]
            ...
            grey = copy.deepcopy(self.labeledobjects[n])

            # Update the labeled image such that the clusters are merged at the end, removed, and relabeled.
            tabdata = self.objectclusterdfs[n]
            ind = [i for i, m in enumerate(self.clustersarepixelbased) if not m][n]
            newclusterval = int(np.max(grey) + 1)
            ...
            if not self.clusternames[ind] == []:
                ...
                for cluster in checked:
                    ...
                    print(self.clusternames[ind])
                    self.objectclusterdfs[n]['Cluster'][self.objectclusterdfs[n]['Cluster'] == oldname] = newname
                    cluster += 1
                    grey[grey == cluster] = newclusterval
                    grey[grey > cluster] = grey[grey > cluster] - 1
                    newclusterval -= 1
                    self.currentobjectclusterorder.remove(cluster - 1)
                    ### TODO: Is this necessary?
                    self.currentobjectclusterorder = np.array(self.currentobjectclusterorder)
                    self.currentobjectclusterorder[self.currentobjectclusterorder > cluster - 1] -= 1
                    self.currentobjectclusterorder = list(self.currentobjectclusterorder)
                    for i in range(self.numimgs):
                        self.cellclustervals[n * self.numimgs + i][self.cellclustervals[n * self.numimgs + i] == cluster] = newclusterval + 1
                        self.cellclustervals[n * self.numimgs + i][self.cellclustervals[n * self.numimgs + i] > cluster] -= 1
                ...
                self.objectclusterdfs[n].to_csv(os.path.join(self.objectclusterdirectories[n], "FullObject_segmentation_dataCluster.csv"))
                ...
                for cluster in checked:
                    ...
                    for i in reversed(range(len(self.viewer.layers))):
                        if self.viewer.layers[i].name == index:
                            ...
                            break
                clusterids += 1
                self.objectclusterdfs[n]['Cluster'] = [str(id) for id in clusterids]
                self.objectclusterdfs[n].to_csv(os.path.join(self.objectclusterdirectories[n], "FullObject_segmentation_dataCluster.csv"))

            self.labeledobjects[n] = grey

            # Create a colored image from the newly-labeled image, and add it to the viewer.
            newrgb = np.zeros((len(grey), grey.shape[1], grey.shape[2], 3)).astype(np.uint8)

            color = self.objectclustercolors[n]
            newcolor = color[[checked[-1]], :]
            color = np.delete(color, checked, 0)
            color = np.append(color, newcolor, 0)
            self.objectclustercolors[n] = color
            ...
            self.currentobjectclusterorder.append(newclusterval - 1)
            self.currenttableorderfull = self.currentobjectclusterorder
            self.currentlyselectedobjectclusters[n] = []
            newdisplaydata = self.objectclusterdata[self.analysisindex][self.currenttableorderfull, :]
            self.update_table(newdisplaydata, self.currentlowerbounds, self.currentupperbounds, newclusterval, self.currenttableorderfull,
    ...
    def minimum_spanning_tree(self):
        ...
        if ispixelcluster:
            if self.numimgs == 1:
                currentdata = np.expand_dims(self.pixelclusterdata[clusteringnum], axis=0)
            else:
                startindex = clusteringnum * (self.numimgs + 1)
                s = self.pixelclusterdata[startindex].shape
                currentdata = np.zeros((self.numimgs + 1, s[0], s[1]))
                for i in range(self.numimgs + 1):
                    currentdata[i, :, :] = self.pixelclusterdata[i + startindex]
            dir = GUIUtils.create_new_folder("PixelMST", self.outputfolder)
        else:
            if self.numimgs == 1:
                currentdata = np.expand_dims(self.objectclusterdata[clusteringnum], axis=0)
            else:
                startindex = clusteringnum * (self.numimgs + 1)
                s = self.objectclusterdata[startindex].shape
                currentdata = np.zeros((self.numimgs + 1, s[0], s[1]))
                for i in range(self.numimgs + 1):
                    currentdata[i, :, :] = self.objectclusterdata[i + startindex]
    ...
    ### TODO: Wrong segmentation image? (Cluster 5x, last on display selected, NN cells don't correspond)
    def nearest_neighbours(self):
        ...
        clusteredsegresults = [self.objectimgnames[i] for i, l in enumerate(self.segmentationclusteringrounds) if len(l) > 0]
        ...
        if len(self.segmentationclusteringrounds[imgindex]) > 1:
            iteration = GUIUtils.ObjectTrainIteration(self.segmentationclusteringrounds[imgindex])
            iteration.exec()
            if not iteration.OK:
                return
            it = iteration.iteration
        else:
            it = self.segmentationclusteringrounds[imgindex][0]

        # Get all the cluster IDs in the selected clustered image, and prompt the user to define source and
        # target clusters.
        tabdata = self.objectclusterdfs[it]
        numtabs = 1
        if len(self.filenames) > 1:
            numtabs += len(self.filenames)
        clusteringround = it * numtabs + selectedimgindex
        data = self.objectclusterdata[clusteringround]

        # Find current names of clusters.
        annotatedobjectclusters = [self.clusternames[i] for i in range(len(self.clusternames)) if not self.clustersarepixelbased[i]]
        ...
        for i in range(self.numimgs):
            ...
            mask = np.in1d(self.labeledsegmentation[i], cellind)
            mask = mask.reshape((1, self.maximageshape[0], self.maximageshape[1]))
            if i == 0:
    ...
    def object_clustering(self):
        ...
        for i in range(self.numimgs):
            ind = i + imagenum
            numcells += len(self.segmentationdata[ind])
        ...
        if args.normalize == "zscale":
            ...
            count = 0
            for i in range(self.numimgs):
                img = copy.deepcopy(self.segmentationdata[ind][:, objectmarkernums])
                scaler.fit(img)
                expressionavgs[count:count + len(self.segmentationdata[ind]), :] = scaler.transform(img)
                count += len(self.segmentationdata[ind])
        else:
            count = 0
            for i in range(self.numimgs):
                ind = i + imagenum
                expressionavgs[count:count + len(self.segmentationdata[ind]), :] = self.segmentationdata[ind][:, objectmarkernums]
                count += len(self.segmentationdata[ind])
        ...
        if self.numimgs > 1:
            ...
            self.objectclusterindices.append(self.tableimgcount)
            self.tableimgcount += 1
        self.segmentationclusteringrounds[alg.imageindex].append(self.objectclustercount)
        self.objectclustercount += 1
        self.clustersarepixelbased.append(False)
        self.clusternames.append([])
        self.currentlyselectedobjectclusters.append([])
    ...
    def open_docs(self):
        ...
        if not os.path.exists(rootfold+"/../docs"):
            gdown.download("https://drive.google.com/uc?id=1JhpIjMd_Rq-i1_laivxzavwZDJPOa4b1",rootfold+"/../docs.zip", verify=False)
    ...
    ### TODO: Asks for channel order for every image.
    def open_images(self, segmentedimgpaths=[]):
        ...
        if not self.hasloadedimage:
            ...
            for path in filenames:
                ...
                ### TODO: Account for (z,c,x,y) images... won't necessarily be the same (move down)
                ...
                if len(img.shape) == 3:
                    ...
                    nummarkers = max(nummarkers, len(img))
        ...
        GUIUtils.log_actions(self.actionloggerpath, f"Opened Images: Paths ({filenames})")
        print(self.imageshapelist)
        print(self.maximageshape)
        return True
        ...
        if self.addwhenchecked and ((column == 0 and row > 2) or row == 2):
            ...
            if currentState == QtCore.Qt.Checked:
                if column > 0:
                    ...
                    if self.analysismode == "Segmentation":
                        r = int(self.currenttablecellorder[self.analysisindex][r])
                        mask = np.in1d(self.labeledsegmentation[self.analysisindex], self.segmentationdata[self.analysisindex][r, 0])
                        ...
                        outputimg = np.zeros((self.numimgs, self.maximageshape[0], self.maximageshape[1]),
                                             dtype=np.bool)
                        outputimg[self.analysisindex % self.numimgs, :, :] = mask
                        self.viewer.add_image(outputimg, name=f"Cell {r + 1}", blending="additive", visible=True)
                        self.currentlyselectedcells[self.analysisindex].append(r)
                    elif self.analysismode == "Object":
                        r = int(self.currentobjectclusterorder[r])
                        currentcluster = np.zeros((self.numimgs, self.maximageshape[0], self.maximageshape[1])).astype(
                            np.bool)
                        ind = self.analysisindex
                        if self.numimgs > 1:
                            ind = int(self.analysisindex / (self.numimgs + 1))
                        color = self.objectclustercolors[ind][r, :] / 255

                        trainnum, _ = GUIUtils.find_analysis_round(self.analysisindex, self.numimgs)
                        currentcluster[self.labeledobjects[trainnum] == r + 1] = 1
                        ...
                        r = self.currentpixelclusterorder[r]
                        currentcluster = np.zeros((self.numimgs, self.maximageshape[0], self.maximageshape[1])).astype(
                            np.bool)
                        ind = self.analysisindex
                        if self.numimgs > 1:
                        ...
                        color = self.pixelclustercolors[ind][r, :] / 255

                        trainnum, _ = GUIUtils.find_analysis_round(self.analysisindex, self.numimgs)
                        currentcluster[self.labeledpixels[trainnum] == r] = 1
                        cmap = Colormap(ColorArray([(0, 0, 0), (color[0], color[1], color[2])]))
                ...
                if column > 0:
                    ...
                    if self.analysismode == "Segmentation":
                        r = int(self.currenttablecellorder[self.analysisindex][r])
                        self.currentlyselectedcells[self.analysisindex].remove(r)
                        index = f"Cell {r + 1}"
                    elif self.analysismode == "Object":
                        r = self.currentobjectclusterorder[r]
                        trainnum, _ = GUIUtils.find_analysis_round(self.analysisindex, self.numimgs)
    ...
    def parse_img(self, imgpath, islabel=False):
        ...
        self.imageisflipped.append(False)
        if img.shape[-2] > img.shape[-1]:
            img = np.moveaxis(img, -1, -2)
            self.imageisflipped[-1] = True
    ...
    def pixel_clustering(self):
        ...
        if not randompatchgeneration:
            ...
            def finish_pixelgui() -> Image:
                ...
                labeledpixels = np.zeros((self.numimgs, self.maximageshape[0], self.maximageshape[1]))
                count = 0
                for i in range(self.numimgs):
                    xdim = hf["imageshapelist"][i, 0]
                    ydim = hf["imageshapelist"][i, 1]
                    labeledpixels[i, :xdim, :ydim] = grey[count: count + xdim * ydim].reshape(xdim, ydim)
                    count += xdim * ydim
                ...
                labeledpixels[:, -args.ncluster:, -1] = np.unique(args.ncluster)
                for i in range(self.numimgs):
                    labeledpixels[i, :, self.imageshapelist[i][1]:] = -1
                    labeledpixels[i, self.imageshapelist[i][0]:, :] = -1

                # Update any relevant variables and close the window.
                self.labeledpixels.append(labeledpixels)
                self.currentlyselectedpixelclusters.append([])
        ...
        # If randomly generating patches.
            ...
            for i in range(self.numimgs):
                ...
                labeledpixels[i, :xdim, :ydim] = grey[count:count + xdim * ydim].reshape(xdim, ydim)
                count += xdim * ydim
            labeledpixels[:, -args.ncluster:, -1] = np.arange(args.ncluster)
            for i in range(self.numimgs):
                labeledpixels[i, :, self.imageshapelist[i][1]:] = -1
                labeledpixels[i, self.imageshapelist[i][0]:, :] = -1
    ...
    def quantify_object_cluster_region(self, imgindex, shapelayerindex, shapetypes, clusteriteration):
        ...
        greysegment = self.labeledsegmentation[segmentimgindex + imgindex]
        qtab = self.segmentationdata[segmentimgindex * self.numimgs + imgindex]
        currentimg = np.zeros((len(greysegment), greysegment.shape[1], 3))
        ...
        for shape in range(len(self.viewer.layers[shapelayerindex].data)):
            ...
            currentimgavgs = []
            for i in range(int(np.max(self.labeledobjects[clusteriteration]))):
        ...
        return avgs, int(np.max(self.labeledobjects[clusteriteration])), numcells, celldata
    ...
    def quantify_pixel_cluster_region(self, imgindex, shapelayerindex, shapetype, clusteriteration):
        ...
        """
        numpixels = []
        grey = self.labeledpixels[clusteriteration]
    ...
    def quantify_raw_img_region(self, imgindex, shapelayerindex, shapetype, verts):
        ...
        currentimg = np.reshape(currentimg, (len(currentimg), dim1 * dim2, currentimg.shape[3]))
        avgs = []
        numpixels = []
    ...
    ### TODO: Cluster names are not accounted for here.
    ...
    ### TODO: Doesn't update the table headers.
    def rename_clusters(self):
        ...
        # Find current names of clusters.
        currentnames = self.clusternames[overallclusterindex]

        # If list is empty, find number of clusters and use those for the names.
        if len(currentnames) == 0:
            if ispixelcluster:
                numclusters = len(np.unique(self.labeledpixels[clusteringnum]))
            else:
                numclusters = len(np.unique(self.labeledobjects[clusteringnum])) - 1
        ...
        self.clusternames[overallclusterindex] = renameclusters.newclusternames

        if not ispixelcluster:
            self.objectclusterdfs[clusteringnum]['Cluster'] = [renameclusters.newclusternames[oldnames.index(name)] for name in self.objectclusterdfs[clusteringnum]['Cluster']]
            self.objectclusterdfs[clusteringnum].to_csv(os.path.join(self.objectclusterdirectories[clusteringnum], "FullObject_segmentation_dataCluster.csv"))
        ...
        if index == clusteringnum and ((self.analysismode == "Pixel" and ispixelcluster) or (self.analysismode == "Object" and not ispixelcluster)):
            self.currentverticalheaderlabels[3:] = [renameclusters.newclusternames[clusternum] for clusternum in self.currentobjectclusterorder]
    ...
    ### TODO: Save full visible images for merged markers.
    def save_data(self):
        """
        ...
        GUIUtils.SaveData(self.viewer, self.outputfolder, self.fulltab, self.clustersarepixelbased, self.labeledpixels,
                          self.labeledobjects, self.imageshapelist, self.filenames, self.imageisflipped).exec()
    ...
    def save_environment(self):
        ...
        config.set("Variables", 'cellidorders', unicode(self.cellidorders))
        ...
        config.set("Variables", 'currentlowerbounds', unicode(self.currentlowerbounds))
        ...
        config.set("Variables", 'currentlyselectedpixelclusters', unicode(self.currentlyselectedpixelclusters))
        config.set("Variables", 'currentobjectclusterorder', unicode(self.currentobjectclusterorder))
        config.set("Variables", 'currentpixelclusterorder', unicode(self.currentpixelclusterorder))
        config.set("Variables", 'currenttablecellorder', unicode(self.currenttablecellorder))
        config.set("Variables", 'currenttableorderfull', unicode(self.currenttableorderfull))
        config.set("Variables", 'currentupperbounds', unicode(self.currentupperbounds))
        ...
        config.set("Variables", 'labeledobjects', unicode([arr.tolist() for arr in self.labeledobjects]))
        config.set("Variables", 'labeledpixels', unicode([arr.tolist() for arr in self.labeledpixels]))
        config.set("Variables", 'labeledsegmentation', unicode([arr.tolist() for arr in self.labeledsegmentation]))
        ...
        config.set("Variables", 'objectclusterdata', unicode([arr.tolist() for arr in self.objectclusterdata]))
        ...
        config.set("Variables", 'pixelclusterdata', unicode([arr.tolist() for arr in self.pixelclusterdata]))
        ...
        config.set("Variables", 'segmentationdata', unicode([arr.tolist() for arr in self.segmentationdata]))
    ...
    ### TODO: Add option for histogram normalization.
    ### TODO: Saved results don't account for which images are flipped.
    def segment(self):
        """
        ...
        if not alreadysegmented:
            ...
            self.segmentationzarrpaths.append(dir)
            self.viewer.window._status_bar._toggle_activity_dock(True)
        ...
        fh = zarr.open(self.segmentationzarrpaths[self.segmentcounts[iteration][segmentationmodel.modelindex]], mode='r')
        ...
        def finish_segmentgui() -> Image:
            ...
            for i in range(self.numimgs):
                ...
                blobs = morphology.remove_small_objects(blobs, min_size=int(self.minsize))
                blobs = self.remove_large_objects(blobs, maxsize=int(self.maxsize))
                expandobject = objectmodels.expand_objects(objectimg=blobs, numiterations=round(0.284 / self.resolution))
                ...
                self.labeledsegmentation.append(expandobject)

                # Store quantified expression values for each cell in an array.
                proptab = np.zeros((self.nummarkers + 4, objectcount))
                for ch in range(self.nummarkers):
                    data = self.viewer.layers[ch].data[i, :len(expandobject), :expandobject.shape[1]]
                ...
                intensityimg = copy.deepcopy(
                    self.viewer.layers[0].data[i, :len(expandobject), :expandobject.shape[1]])
                proptab[self.nummarkers, :] = [prop.area for prop in
                ...
                imgmean = np.c_[np.asarray(labtab), proptab.T]
                numCellsList.append(len(imgmean))
                self.segmentationdata.append(imgmean)
                ...
                if i == 0:
                    ...
                    expobj[0, :len(expandobject), :expandobject.shape[1]] = expandobject
                    rgbimg = np.zeros((1, self.maximageshape[0], self.maximageshape[1], 3), dtype=rgbimage.dtype)
                    rgbimg[0, :len(rgbimage), :rgbimage.shape[1], :] = rgbimage
                    if selectimagesadded.grey:
            ...
            startindex = len(self.segmentationdata) - self.numimgs
            for i in range(self.numimgs):
                segmentedtable = pd.DataFrame(np.hstack([np.vstack(self.segmentationdata[i + startindex]), cortabs[i]]))
            ...
            if not self.hasaddedtable:
                ...
                self.currentupperbounds = copy.deepcopy(self.maxvalssegment[0])
                self.update_table(self.segmentationdata[0][:, 1:], self.currentlowerbounds, self.currentupperbounds, len(self.segmentationdata[0]),
                                  self.segmentationdata[0][:, 0].astype(np.int).tolist())

            GUIUtils.log_actions(self.actionloggerpath,
                                 f"Segmented: RAPID+ ({segmentationmodel.modelindex}), Iteration ({iteration}), "
                                 f"Resolution ({self.resolution}), Prob ({self.probthreshold}), Min ({self.minsize}), "
    ...
    def sort_table_image(self):
        ...
        if (self.segmentcount > 0 or self.pixelclustercount > 0) and not self.isloadingenv:
            ...
            self.currentlowerbounds = self.lowerboundslist[index]
            self.currentupperbounds = self.upperboundslist[index]
            ...
            if index in self.segmentationindices:
                ...
                currentData = copy.deepcopy(self.segmentationdata[self.analysisindex])
                ...
                order = np.argsort(self.segmentationdata[self.analysisindex][:, m]).astype(np.int).tolist()
                if sort == "▼":
                    order.reverse()
                self.cellidorders[self.analysisindex] = order

                # Filter out cells that don't fall within the user-defined lower/upper bounds.
                filtereddata = np.append(self.segmentationdata[self.analysisindex][order, 1:],
                                         np.expand_dims(np.arange(len(self.segmentationdata[self.analysisindex])), 1), 1)
                for chan in range(len(self.currentlowerbounds)):
                    filtermask = (np.round(filtereddata[:, chan], 3) <= np.round(self.currentupperbounds[chan], 3))
                    filtereddata = filtereddata[filtermask]
                    filtermask = (np.round(filtereddata[:, chan], 3) >= np.round(self.currentlowerbounds[chan], 3))
                    filtereddata = filtereddata[filtermask]
                self.currenttablecellorder[self.analysisindex] = [order[j] for j in filtereddata[:, -1].astype(np.int).tolist()]

                # Get the cell indices to be used as the vertical header labels.
                cellnumlabels = [int(self.segmentationdata[self.analysisindex][order[j], 0]) for j in filtereddata[:, -1].astype(np.int).tolist()]

                # Sort the cells according to user selection and update the table accordingly.
                displayData = currentData[self.currenttablecellorder[self.analysisindex], 1:]
                self.update_table(displayData, self.currentlowerbounds, self.currentupperbounds, len(currentData),
                                  cellnumlabels)
            ...
            # If displaying object-based clustering results.
                ...
                currentdata = copy.deepcopy(self.objectclusterdata[self.analysisindex])
                ...
                for chan in range(len(self.currentlowerbounds)):
                    ...
                    filtermask = (np.round(filtereddata[:, chan], 3) >= np.round(self.currentlowerbounds[chan], 3))
                    filtereddata = filtereddata[filtermask]
                self.currentobjectclusterorder = [self.currenttableorderfull[i] for i in filtereddata[:, -1].astype(np.int).tolist()]

                # Sort the clusters according to user selection and update the table accordingly.
                displaydata = currentdata[self.currentobjectclusterorder, :]
                ...
                self.update_table(displaydata, self.currentlowerbounds, self.currentupperbounds, len(clusters),
                                  self.currentobjectclusterorder, headernames=self.clusternames[annotationindex])
            ...
            # If displaying pixel-based clustering results.
                ...
                currentdata = copy.deepcopy(self.pixelclusterdata[self.analysisindex])
                ...
                for chan in range(len(self.currentlowerbounds)):
                    ...
                    filtermask = (np.round(filtereddata[:, chan], 3) >= np.round(self.currentlowerbounds[chan], 3))
                    filtereddata = filtereddata[filtermask]
                self.currentpixelclusterorder = [self.currenttableorderfull[i] for i in filtereddata[:, -1].astype(np.int).tolist()]

                # Sort the clusters according to user selection and update the table accordingly.
                displaydata = currentdata[self.currentpixelclusterorder, :]
                ...
                # Update the display table in the GUI.
                self.update_table(displaydata, self.currentlowerbounds, self.currentupperbounds, len(currentdata),
                                  self.currentpixelclusterorder, headernames=self.clusternames[annotationindex])
    ...
    def spatial_analysis(self):
        ...
        if not ispixelcluster:
            grey = self.labeledobjects[clusteringnum]
        else:
            grey = self.labeledpixels[clusteringnum]
    ...
    ### TODO: Subclustering doesn't remove old cluster
    def subcluster(self):
        """
        ...
        if len(self.segmentationclusteringrounds[imgindex]) > 1:
            ...
            if not iteration.OK:
                return
            it = iteration.iteration
        elif len(self.segmentationclusteringrounds[imgindex]) == 1:
            it = 0
        else:
        ...
        it = self.segmentationclusteringrounds[imgindex][it]
        startindex = it * self.numimgs
        currentgreyimg = self.labeledobjects[it]
        ...
        for i in range(self.numimgs):
            count = 0
            for j in range(len(self.segmentationdata[i + imgindex * self.numimgs])):
                if int(self.cellclustervals[startindex + i][j]) == int(clusternum):
                    currentimage.append(self.segmentationdata[i + imgindex * self.numimgs][j, :])
                    count += 1
    ...
    def test_object(self, model, quantifiedvals, args, markerindices, createcolorimg, creategreyimg, alg, startind=0,
        ...
        for i in range(self.numimgs):
            complete_tab.append(self.segmentationdata[i + startind][:, qtabindices])
        complete_tab = np.vstack(complete_tab)
        complete_tab_DF = pd.DataFrame(complete_tab)
        ...
        imgid = []
        for i in range(self.numimgs):
            imgid.append(np.repeat(i, len(self.segmentationdata[i + startind])))
        if alg == "Phenograph":
            os.chdir(self.outputfolder)
            if args.normalize:
                numcells = 0
                for i in range(self.numimgs):
                    numcells += len(self.segmentationdata[i + startind])
                currentImage = np.zeros((numcells, len(markerindices)))
                count = 0
                for i in range(self.numimgs):
                    currentImage[count:count + len(self.segmentationdata[i + startind]), :] = self.segmentationdata[i + startind][:, markerindices]
                    count += len(self.segmentationdata[i + startind])
            ...
            with torch.no_grad():
                ...
                to_values = np.zeros(len(testdata))
        ...
        cord = np.vstack(self.cellcoordinates[int(startind / self.numimgs)])
        ...
        for i in range(self.numimgs):
            ...
            from_values = complete_tab_DF['Label'].values[startindex:startindex + len(self.segmentationdata[i + startind])]
            tmp_to_values = to_values[startindex:startindex + len(self.segmentationdata[i + startind])]
            self.cellclustervals.append(copy.deepcopy(tmp_to_values))
            relabeled = self.method_searchsort(from_values, tmp_to_values, self.labeledsegmentation[i + startind].flatten().astype(int))
            relabledgreyimages[i, :len(self.labeledsegmentation[i + startind]), :self.labeledsegmentation[i + startind].shape[1]] = (relabeled.reshape(self.labeledsegmentation[i + startind].shape)).astype(np.uint8)
            relabledgreyimages[i, :len(self.labeledsegmentation[i + startind]), :self.labeledsegmentation[i + startind].shape[1]][self.labeledsegmentation[i + startind] == 0] = 0
            ...
            del relabledimages
            gc.collect()
            tmp_tab = relabeled_table[startindex:startindex + len(self.segmentationdata[i + startind])]
            tmp_tab_df = pd.DataFrame(tmp_tab)
            startindex += len(self.segmentationdata[i + startind])
            grouped = tmp_tab_df.groupby(0)
            tabres = grouped.apply(np.mean)
            tabres.insert(0, "Sample", i)
            _, counts = np.unique(tmp_tab[:, 0], return_counts=True)
            ...
            self.objectclusterdata.append(emptytab[:, 2:].astype(np.float))
            minvals = []
            maxvals = []
    ...
    def test_object_subcluster(self, model, quantifiedvals, args, numcellsperimage, iteration, clusternum, startind,
        ...
        for i in range(numtabs):
            indices = [j for j in range(len(self.objectclusterdata[it + i]))]
            ...
            newtable = np.zeros(
                (len(self.objectclusterdata[it + i]) - 1 + len(np.unique(to_values)), len(self.markers) + 5))
            newtable[:len(self.objectclusterdata[it + i]) - 1, :] = copy.deepcopy(self.objectclusterdata[it + i])[indices, :]
            newtable[len(self.objectclusterdata[it + i]) - 1:, :] = weighted_average
            self.objectclusterdata[it + i] = newtable
            minvals = []
            maxvals = []
            ...
            if self.analysismode == "Object" and self.analysisindex == it + i:
                ...
                self.currentobjectclusterorder = [j for j in range(len(newtable))]
        self.currentlyselectedobjectclusters[iteration] = []
        self.labeledobjects[iteration] = copy.deepcopy(self.labeledobjects[iteration])
        self.labeledobjects[iteration][self.labeledobjects[iteration] == clusternum] = 0
        self.labeledobjects[iteration][self.labeledobjects[iteration] > clusternum] = self.labeledobjects[iteration][self.labeledobjects[iteration] > clusternum] - 1
        newstart = copy.deepcopy(np.max(self.labeledobjects[iteration])) + 1
        count = 0
        counter = 0
        ...
        for i in range(self.numimgs):
            ...
            segmentedimg = self.labeledsegmentation[i + startind]
            currentgreyimg = self.labeledobjects[iteration][i, :, :]
            ...
            self.labeledobjects[iteration][i, :, :] = currentgreyimg
        ...
        # self.labeledobjects[iteration] = grey
        # self.viewer.add_image(self.labeledobjects[iteration], name='Re-Clustered nums', blending="additive", contrast_limits=(0, np.max(grey)))
        ...
        newcolors = generate_colormap(len(np.unique(self.labeledobjects[iteration])))
        while len(colors) < len(np.unique(self.labeledobjects[iteration])):
        ...
        for i in range(len(np.unique(self.labeledobjects[iteration]))):
            colorimg[:, :, :, 0][self.labeledobjects[iteration] == i + 1] = colors[i][0]
            colorimg[:, :, :, 1][self.labeledobjects[iteration] == i + 1] = colors[i][1]
            colorimg[:, :, :, 2][self.labeledobjects[iteration] == i + 1] = colors[i][2]
    ...
    def umap_plot(self):
        ...
        setumapParams = GUIUtils.UMAPParameters(self.objectclustercount > 0, self.objectimgnames, self.groupsnames[1:])
        setumapParams.exec()
        ...
        currentimage2 = np.zeros((numcells, self.segmentationdata[segindex * self.numimgs].shape[1]))
        count = 0
        ...
        for i in range(self.numimgs):
            cellsincurrimg = []
            currentimage[count:count + len(self.segmentationdata[segindex * self.numimgs + i]), :] = self.segmentationdata[segindex * self.numimgs + i][:, umapmarkers.markernums]
            currentimage2[count:count + len(self.segmentationdata[segindex * self.numimgs + i]), :] = self.segmentationdata[segindex * self.numimgs + i]
            for j in range(count, count + len(self.segmentationdata[segindex * self.numimgs + i])):
                cellsincurrimg.append(j)
        ...
        if len(self.segmentationclusteringrounds[segindex]) > 0:
            ...
            if setumapParams.colorbyindivclusters or setumapParams.colorbycombclusters:
                if len(self.segmentationclusteringrounds[segindex]) > 1:
                    iteration = GUIUtils.ObjectTrainIteration(self.segmentationclusteringrounds[segindex])
                    iteration.exec()
                    if not iteration.OK:
                        return
                    startindex = self.segmentationclusteringrounds[segindex][iteration.iteration]
                    GUIUtils.log_actions(self.actionloggerpath,
                                         f"UMAP: Markers ({umapmarkers.objecttrainmarkers}), Segmentation Iteration ({segindex}), "
        ...
        self.segmentationindicesinumap.append(segindex)
        self.umapcount += 1

    ### TODO: Disappears when filtering down to one.
    def update_table(self, datavals, lowerbounds, upperbounds, totalnumrows, order=[], headernames=[]):
        """
        Apply both lower- and upper-bound thresholds to an image array.
        ...
        if self.analysismode == "Segmentation":
            if len(order) > 0:
                self.currentverticalheaderlabels = np.asarray([f"{numrows}/{totalnumrows}", "", ""] + [f"Cell {int(i)}" for i in order]).astype(np.str)
            else:
                self.currentverticalheaderlabels = np.asarray([f"{numrows}/{totalnumrows}", "", ""] + [f"Cell {int(i) + 1}" for i in range(numrows)]).astype(np.str)
        else:
            if headernames != []:
                labels = [headernames[i] for i in order]
                self.currentverticalheaderlabels = np.asarray([f"{numrows}/{totalnumrows}", "", ""] + labels).astype(np.str)
            elif len(order) > 0:
                self.currentverticalheaderlabels = np.asarray([f"{numrows}/{totalnumrows}", "", ""] + [f"Cluster {int(i) + 1}" for i in order]).astype(np.str)
            else:
                self.currentverticalheaderlabels = np.asarray([f"{numrows}/{totalnumrows}", "", ""] + [f"Cluster {int(i) + 1}" for i in range(numrows)]).astype(np.str)
                self.currenttableorderfull = []
        ...
        if len(order) > 0:
            ...
            for a in order:
                if self.analysismode == "Segmentation":
                    if a in self.currentlyselectedcells[self.analysisindex]:
                        self.tablewidget.item(counter, 0).setCheckState(QtCore.Qt.Checked)
                else:
                    trainnum, _ = GUIUtils.find_analysis_round(self.analysisindex, self.numimgs)
                    if self.analysismode == "Object":
