...
def prep_for_mst(clustertable=None, minnumpixels=100, outfolder=None, includedmarkers=None, clustermin=0.0):
    """
    ...
    Args:
        ...
        minnumpixels (int, optional): Minimum number of pixels for clusters used in MST analysis (Default: 100).
    ...
    # Filter out clusters with less than minnumpixels
    selectedcol = clustertable["Pixels"] > minnumpixels
    ...
    normdata.to_csv(outfolder + '/oldClusteredReordered.csv')
    if len(normdata) <= 1:
...
### TODO: Error if clusterheatmap is false.
def generate_mst(distancematrix=None, normalizeddf=None, colors=None, randomseed=0, outfolder=None,
    ...
    if clusterheatmap:
        ...
        PerSampleCount = uniqueclusters.pivot(index='Cluster', columns='Sample', values='Pixels')
        PerSampleCount.to_csv(outfolder + "/pixelperclustercount.csv")
        ...
        for tick_label in ClusterDend.ax_heatmap.axes.get_xticklabels():
            ...
            tick_label.set_color(colors[int(tick_text), :] / 255)
            if ((colors[int(tick_text), 0] == 255) & (colors[int(tick_text), 1] == 255) & (
                    colors[int(tick_text), 2] == 255)):
        ...
        ClusterDend.data2d.to_csv(outfolder + '/ClusteredReordered.csv')
        plt.savefig(outfolder + '/MeanIntensityClusterHEATMAP.png', dpi=100)
    ...
    # np.random.seed(randomseed)
    for node in T:
        color_map.append(matplotlib.colors.rgb2hex(colors[int(node), :] / 255))
    ...
    plt.savefig(outfolder + "/RAPID_MST.png", format="PNG", dpi=300)
    return my_data_dend
...
if __name__ == "__main__":
    ...
    my_data = pd.read_csv(rfold + "/RAPID_cluster_table.csv",
    ...
    tabledata, my_data_scaled, distancematrix, uniqueclusters = prep_for_mst(clustertable=my_data, minnumpixels=10000,
                                                                             outfolder="/tmp/",
