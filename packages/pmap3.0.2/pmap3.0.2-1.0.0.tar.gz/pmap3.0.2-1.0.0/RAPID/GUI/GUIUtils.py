...
def find_analysis_round(analysisindex, numimgs):
    """
    Find the round of analysis that corresponds to a given clustering analysis table entry.

    Args:
        analysisindex (int): Index, among its respective analysis mode, of currently selected table
...
def get_nn_in_radius(data=None, clusterid1=None, clusterid2=None, radius=100, nn=0):
    ...
    if nn == 0:
        ...
        retttt = data4cluster2.iloc[np.unique(np.hstack(ckdtree4c2.query(data4cluster1[["X", "Y"]], nn, distance_upper_bound=radius)[1]))[:-1]]
