...
class BaseMiddleware(RoutesMixin):
    ...
    def __init__(self):
        ...
        # This MUST happen after the initialization calls for Service and/or TeamServer messaging
        # (sending server start and application start) to ensure that TeamServer will accept the
        # messages sent by our background reporting threads
    ...
    def postfilter_defend(self, context):
        ...
        if self.settings.config.is_agent_lib_enabled:
            ...
            sqli_rule = rules.get(SqlInjection.RULE_NAME)
            input_analysis.get_input_analysis([sqli_rule.RULE_NAME])

            for input_row in context.user_input_analysis:
                if input_row[0].score >= 90:
                    logger.debug(
                        f"Input analysis found a value '{input_row[0].value}' that violated {sqli_rule.RULE_NAME} rule!"
                    )
                    sqli_rule.build_or_append_attack(input_row[0])
        for rule in rules.values():
