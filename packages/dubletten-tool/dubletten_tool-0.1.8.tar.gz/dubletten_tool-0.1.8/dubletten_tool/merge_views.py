...
# view to serve relations template
@method_decorator(login_required, name="dispatch")
class getMergePage(TemplateView):
    template_name = "merge/merge_page.html"
    ...
    def get(self, request, *args, **kwargs):
        ...
        return HttpResponse(template.render(
            request=request, context={
                ...
                'next_url': instance.get_next_url().replace("apis/entities", "dubletten"),
                'prev_url': instance.get_prev_url().replace("apis/entities", "dubletten") 
                }
            ))
...
class GenericListViewNew(UserPassesTestMixin, ExportMixin, SingleTableView):
    ...
    def get_queryset(self, **kwargs):
        ...
        def create_ampel(el):
            if not Ampel.objects.filter(person=el).exists():
                Ampel.objects.create(person=el, status="red")
                print("created ampel for obj", el)
        print("queryset is", qs)

        [create_ampel(el) for el in qs]
        self.filter = get_list_filter_of_entity(self.entity.title())(
            self.request.GET, queryset=qs
        )
        print("filter in get queryset of listview ist: ", self.filter)

        self.filter.form.helper = self.formhelper_class()
        return self.filter.qs

    def get_table(self, **kwargs):
        # todo: create custom table for listing vorfinale Einträge
...
"""
class SaveChanges(View):

    def post(self, request):
        success = True
        msg = "success"
        inst = Person.objects.get(id = request.POST["id"])
        data = json.loads(request.POST.get("rels"))
        ...
        try:
            for el in data: 
                print(el["id"])
                if el["id"]:
                    print("el id was true")
                    temp = {k:el[k] for k in fields}

                    # todo: check that this save mechanism works on relations with new id
                    PersonInstitution.objects.filter(id=el["id"]).update(**temp)
                else: 
                    # todo: dirty version, refactor this and check that its working reliably
                    temp = {k:el[k] for k in fields}
                    print("el id was false")
                    temp.update({"related_person":inst})
                    temp.update({"related_institution":Institution.objects.get(id=el["related_institution"])})
                    temp.update({"relation_type":PersonInstitutionRelation.objects.get(id=el["relation_type"])})
                    print("temp is: ", temp)
                    new_rel = PersonInstitution.objects.create(**temp)
                    new_rel.save()
                    print(f"created new relation from: {temp}, rel is: {new_rel}")
        except Exception as e:
            success = False
            msg = f"Something went wrong: {e}"
        ...
        return JsonResponse({"success": success, "msg":msg, "rels":rels})
"""
...
class UpdateFieldView(View):
    def post(self, request):
        ...
        msg = "success"
        #per = get_object_or_404(Person, pk=request.POST.get("id", "-111")) # todo: check if this is best solution
        #rel = get_object_or_404(PersonInstitution, pk=get("id"))
        data = json.loads(request.POST.get("relation"))
...
class UpdateAmpelStatus(View):
    def post(self, request):
        ...
        try:
            ampel = Ampel.objects.get(pk=ampel_pk)
            ampel.status = status
            ampel.save()
        except Exception as e:
            success = False
...
class HandleNote(View):
    ...
    def get(self, request, **kwargs):
        ...
        try:
            ...
            note = Ampel.objects.get(pk=ampel_pk).note
            if not note:
                note = ""
        except Exception as e:
            success = False
    ...
    def post(self, request):
        ...
        ampel_pk = request.POST.get("id")
        try:
            a = Ampel.objects.get(pk=ampel_pk)
            a.note = request.POST.get("note_text")
            a.save()
        except Exception as e:
            success = False
            msg = f"{e}"
