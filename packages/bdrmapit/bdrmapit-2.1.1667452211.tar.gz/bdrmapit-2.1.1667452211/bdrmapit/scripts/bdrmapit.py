...
from bdrmapit.version import __version__
...
def set_bdrmapit_parser(parser: ArgumentParser):
    ...
    parser.add_argument('--no-rel-peer', nargs='*', help='Cloud or CDN ASNs whose connections might not be properly represented in BGP AS paths.')
    parser.add_argument('--ixp-asns', help='Mappings from IXP addresses to ASes.')
...
def run_from_config(args):
    ...
    if 'graph' in config:
        ...
        args.poolsize = config['processes']
        if args.graph_only:
...
def main(args=None):
    ...
    norelpeer = set(map(int, args.no_rel_peer)) if args.no_rel_peer else None

    ixpasns = None
    if args.ixp_asns:
        ixpasns = {}
        with fopen(args.ixp_asns) as f:
            for line in f:
                if line.startswith('#'):
                    continue
                addr, asn = line.split()
                ixpasns[addr] = int(asn)

    bdrmapit = Bdrmapit(graph, as2org, bgp, strict=False, ixpasns=ixpasns, norelpeer=norelpeer)
    if args.peeringdb:
