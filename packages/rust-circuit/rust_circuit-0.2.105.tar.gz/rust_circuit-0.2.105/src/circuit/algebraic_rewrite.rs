...
use crate::{
    ...
    util::{
        ...
        is_unique, unique_to_appearance, AxisInt, BitMask128,
    },
};
...
pub fn einsum_permute_to_rearrange(einsum: &Einsum) -> Option<Rearrange> {
        ...
        && is_unique(&einsum.args[0].1)
    {
        return Some(Rearrange::new(
            ...
            RearrangeSpec {
                input_ints: einsum.args[0].1.iter().map(|z| sv![*z]).collect(),
                output_ints: einsum.out_axes.iter().map(|z| sv![*z]).collect(),
                int_sizes: sv![OpSize::NONE;(*einsum.out_axes.iter().max().unwrap()+1) as usize],
            }
            .canonicalize(false),
            einsum.name_cloned(),
        ));
