...
use super::{
    Add, CachedCircuitInfo, CircuitNode, CircuitNodeAutoName, CircuitRc, ConstructError, Rearrange,
    ScalarConstant, TensorEvalError,
};
use crate::{
    all_imports::{OpSize, RearrangeSpec, Shape},
    circuit::{Einsum, PyCircuitBase},
    circuit_node_auto_impl, circuit_node_extra_impl, new_rc,
    py_types::scalar_to_tensor,
    pyo3_prelude::*,
    sv,
    tensor_util::TorchDeviceDtype,
    util::cumsum,
};
...
impl CircuitNode for Cumulant {
    ...
    fn eval_tensors(
        ...
        if !self.info().is_explicitly_computable {
            return Err(TensorEvalError::NotExplicitlyComputable {
                circuit: self.crc(),
            }
            .into());
        }

        match (&self.nodes[..], tensors) {
            ([], []) => return Ok(scalar_to_tensor(1., sv![], device_dtype.clone())?),
            ([_], [t]) => return Ok(t.clone()),
            _ => unreachable!(),
        }
    }
}
...
impl Cumulant {
    ...
    pub fn new_canon_rearrange(nodes: Vec<CircuitRc>, name: Option<String>) -> Result<Rearrange> {
        ...
        Rearrange::try_new(
            cum.crc(),
            RearrangeSpec::new(
                perm_blocks.into_iter().map(|i| sv![i as u8]).collect(),
                (0..cum.info().rank()).map(|i| sv![i as u8]).collect(),
    ...
    //     return (
    //         Add.from_weighted_list([(centered_mom, 1.0), *[(v, -1.0) for v in sub]], name=f"{cumulant.name}_decompose"),
    //         centered_mom,
    //         sub,
    //     )
