...
use crate::{
    circuit::{self, ConstructError, HashBytes},
    filter_by_variant,
    lru_cache::TensorCacheRrfs,
    py_types::{Tensor, PY_UTILS},
    pycall,
    pyo3_prelude::*,
    rrfs::{save_tensor_rrfs, tensor_from_hash_prefix},
    sv,
    util::cumsum,
};
...
impl TensorIndex {
    ...
    pub fn validate(&self, shape: &Shape) -> Result<()> {
        let get_err = |at| {
            Err(ConstructError::IndexOutOfBounds {
                index: self.clone(),
                shape: shape.clone(),
                at,
    ...
    pub fn from_bijection_string(
        ...
        let er = circuit::ParseError::Fail {
        ...
        Ok(TensorIndex(
            string
                ...
                .filter(|x| !x.is_empty())
                .map(|x| -> Result<_> {
                    let num_er = circuit::ParseError::NumberFail {
                        string: x.to_string(),
                    };
                    ...
                    if let Some(no_t) = x.strip_prefix('t') {
                        return tensor_cache
                            ...
                            .unwrap_or_else(|| {
                                tensor_from_hash_prefix(no_t).context("tensor hash from string")
                            })
                            .map(TensorAxisIndex::Tensor);
                    }
        ...
        let out = Self {
            device: match (&self.device, &other.device) {
                (Some(l), Some(r)) if l != r => {
                    bail!(ConstructError::ChildrenMultipleDevices {
                        a: self.device.clone(),
            ...
            dtype: match (&self.dtype, &other.dtype) {
                (Some(l), Some(r)) if l != r => {
                    bail!(ConstructError::ChildrenMultipleDtypes {
                        a: self.dtype.clone(),
                        b: other.dtype.clone(),
                    });
                }
