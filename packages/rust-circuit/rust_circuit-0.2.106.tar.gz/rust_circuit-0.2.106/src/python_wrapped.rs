...
macro_rules! setup_wrap {
    (@full_inside $impl_struct_name:ident, $rc_struct_name:ident, $to_impl_struct_name:ident, $to_rc_struct_name:ident, $enum_name:ty, $from_py:ty) => {
        use crate::{
            circuit::HashBytes, eq_by_big_hash::EqByBigHash, impl_both_by_big_hash, simple_from,
            py_types::use_rust_comp, pyo3_prelude::*,
        };
        ...
        impl<'source> FromPyObject<'source> for $rc_struct_name {
            fn extract(from_py_obj: &'source PyAny) -> PyResult<Self> {
                let from: $from_py = from_py_obj.extract()?;
                Ok(from.into())
            }
        }
        impl IntoPy<PyObject> for $rc_struct_name {
