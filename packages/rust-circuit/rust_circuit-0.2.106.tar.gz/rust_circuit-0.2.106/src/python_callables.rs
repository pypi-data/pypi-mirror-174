...
use crate::{circuit::CircuitRc, pyo3_prelude::*};
...
pycallable! {
    ...
    pub fn deep_map_1<F>(circuit: CircuitRc ,f: F) -> Result<CircuitRc>
    where
...
#[apply(pycallable)]
#[pyo3(name = "deep_map_1")]
pub fn deep_map_2<F>(circuit: CircuitRc, f: F) -> Result<CircuitRc>
where
    F: Fn((circuit, CircuitRc)) -> Result<CircuitRc>,
{
    f(circuit)
