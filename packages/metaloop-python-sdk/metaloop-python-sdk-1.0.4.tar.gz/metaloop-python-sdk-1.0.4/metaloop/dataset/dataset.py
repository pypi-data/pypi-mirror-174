...
from metaloop.utils.file_helper import get_file_type, FileTypeZip
...
class Dataset(DatasetMeta):
    ...
    def import_data(
        ...
        if storage_type == "cos":
            if not os.path.isdir(file_path):
                ...
                upload_id = self._upload_file(file_path, storage_type)
            else:
                upload_id = self._upload_files(file_path, storage_type, self._id)
            task_type = "cos"

        elif os.path.isdir(file_path) and os.path.getsize(file_path) > 1 * GB:
            upload_id = self._upload_file(file_path, storage_type)
            task_type = "object_storage"

        elif os.path.isdir(file_path):
            upload_id = self._upload_files(file_path, storage_type)
            task_type = "object_storage"
    ...
    def _upload_file(
        ...
        return CloudClient.upload_file(s3_config.identifier, bucket, file_path)
    ...
    def _get_download_info(
        ...
        index = url.strip("/").index("/")
        key = url[index + 2:]

        if image_path:
            file_name = os.path.join(file_path, image_path.strip("/"))
        else:
            file_name = os.path.join(file_path, url.strip("/"))

        if not file_path:
            file_name = os.path.join("abaddon", file_name)

        if not os.path.exists(os.path.dirname(file_name)):
            os.makedirs(os.path.dirname(file_name), 0o0755)

        obj_url = file_name.strip(file_path).strip("/")
    ...
    def _get_stream_data(
        ...
        index = url.strip("http://").index("/")
        file_name = os.path.join(file_path, url[index + 8:])
