...
class ReaxFF(object):
  ...
  def set_parameters(self,opt=[],libfile=None):
      ...
      self.p,self.v = {},{}
      self.v = set_variables(self.p_, self.optword, self.cons, self.opt,
...
def set_variables(p_,optword,cons,opt,punit,unit,conf_vale,ang_v,tor_v):
    v = {}
    for k in p_:
        key = k.split('_')[0]
        ktor= ['cot1','V1','V2','V3']
...
def clip_parameters(p_,v,clip):
    ...
    for k in v:
        ...
        if key == 'n.u.':
           ...
           p[k] = tf.clip_by_value(v[k],-50.00,-0.01)
           ...
           p[k] = tf.clip_by_value(v[k],0.0,99.0*unit)
        elif key== 'lp3':
        ...
        #     p[k] = tf.clip_by_value(v[k],0.80*re[bd],1.2*re[bd]) # 0.95 1.2
           ...
           a   = bd_.split('-')
           if a[1]!='H':
              p[k] = tf.clip_by_value(v[k],1.0,200.0*unit)
           else:
              p[k] = v[k]
        elif key=='val2':
           p[k] = tf.clip_by_value(v[k],0.2,8.0)
        elif key=='val9':
           p[k] = tf.clip_by_value(v[k],0.3,6.0)
        elif key=='val':
           sp = vn[1]
           p[k] = tf.clip_by_value(v[k],0.2,8.0)
        elif key=='vale':
           sp = vn[1]
           p[k] = tf.clip_by_value(v[k],v['val_'+sp],8.0)
        elif key=='valboc':
           sp = vn[1]
