...
def single_point(atoms,id=0,xcf='VDW',xca='DRSLL',basistype='DZP',
    ...
    calc = SinglePointCalculator(atoms,energy=e,forces=f[-1],stress=p[-1])
...
def write_siesta_in(Atoms,coord='zmatrix',
    ...
    # print('MaxSCFIterations %d' %MaxSCFIterations,file=fdf)
    
    if 'MaxSCFIterations' in kwargs:
       print('MaxSCFIterations        {:d} \n'.format(kwargs['MaxSCFIterations']),file=fdf)
    ...
    if 'InitSpinAF' in kwargs:
       print('DM.InitSpinAF        {:s} \n'.format(kwargs['InitSpinAF']),file=fdf)
    if 'MixingWeight' in kwargs:
       print('DM.MixingWeight      {:f} \n'.format(kwargs['MixingWeight']),file=fdf)
    else:
    ...
    print('   ',file=fdf)
    print('   ',file=fdf)
    if 'MeshCutoff' in kwargs:
       print('MeshCutoff        {:f} Ry \n'.format(kwargs['MeshCutoff']),file=fdf)
    else:
...
def get_siesta_info(natom,spec,atom_name,val,label='siesta'):
    ...
    return e,np.array(forces),np.array(presses),np.zeros(natom) # np.array(qs)
...
def parse_fdf_species(fdf='in.fdf'):
    fin = open(fdf,'r') 
    lines= fin.readlines()
    fin.close()                     # getting informations from input file
        ...
        if len(l)>0:
           if l[0]=='%block':
              ...
              if line.find('Ang')>0 or line.find('ang')>0:
                 unit = 1.0
              elif line.find('Bohr')>0 or line.find('Bohr')>0:
                 unit = 0.529177249
              else:
                 unit = 1.0
