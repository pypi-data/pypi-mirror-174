...
def write_gulp_in(A,runword='gradient qiterative nosymmetry conv verb debu',
    ...
    if output is None:
       print('output movie {:d} xyz his.xyz'.format(movieFrequency),file=finp)
    else:
...
def xyztotraj(fxyz,inp='inp-gulp',checkMol=False,mode='w',scale=True):
...
def jstolib(nn=False,jsfile='ffield.json',libfile='reax.lib'):
    ...
    write_lib(p_,spec,bonds,offd,angs,torp,hbs,m=m_,mf_layer=mf_layer,libfile=libfile)
    ...
    if wforce:
       with open('Forces.FA','w') as fsiesta:
            ...
            for i,f in enumerate(forces):
                print('{:4d} {:12.8f} {:12.8f} {:12.8f}'.format(i+1,f[0],f[1],f[2]),file=fsiesta) ## Why?
