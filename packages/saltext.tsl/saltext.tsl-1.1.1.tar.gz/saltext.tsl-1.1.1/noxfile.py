...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/tsl/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/tsl/*")
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/tsl/config/schemas",
    )
