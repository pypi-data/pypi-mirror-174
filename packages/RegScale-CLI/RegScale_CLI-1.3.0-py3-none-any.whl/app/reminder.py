...
from app.utils import get_css, send_email, str_to_date, uncamel_case
...
def send_reminders(days):
    ...
    # check user has Enterprise license and isn't expired
    if is_licensed(app=app):
        ...
        # if error encountered, exit the application
            ...
            logger.error(
                "ERROR: Unable to retrieve active users from RegScale: \n%s", ex
            )
            sys.exit()
        ...
        progress_bar(
            ...
            args=(api, users, base_url, str(days)),
            thread_count=len(users),
            max_threads=config["max_threads"],
            description=f"[#f8b737]Fetching pipeline for {len(users)} user(s)...",
        )
        ...
        progress_bar(
            ...
            max_threads=config["max_threads"],
            description=f"[#ef5d23]Analyzing pipeline for {len(tenant_pipeline)} user(s)...",
        )

        # start a console progress bar and threads for the given task
        progress_bar(
            function=format_and_email,
            args=(api, config),
            thread_count=len(final_pipeline),
            max_threads=config["max_threads"],
            description=f"[#21a5bb]Sending an email to {len(final_pipeline)} user(s)...",
        )
...
def progress_bar(
    function, args: Tuple, thread_count: int, max_threads: int, description: str
):
    """
    function to create a dynamic progress bar for x amount of threads
    with the given function and arguments
    """
    # create the progress bar and its elements
    with Progress(
        "[progress.description]{task.description}",
        BarColumn(),
        "[progress.percentage]{task.percentage:>3.0f}%",
        TimeElapsedColumn(),
        refresh_per_second=0.02,
    ) as progress:
        # start the progress bar in the console
        task = progress.add_task(description, total=thread_count)
...
def get_pipeline(args: Tuple, thread: int):
    ...
    api, all_users, base_url, days = args
    ...
    tasks = api.get(url=f"{base_url}tasks/userOpenItemsDays/{user['id']}/{days}").json()
    data_calls = api.get(
...
def analyze_pipeline(args: Tuple, thread: int):
    ...
    if not tenant_pipeline[thread].analyzed:
            ...
            for item in pipelines[pipe]["Pipeline"]:
                ...
                for key in item.keys():
                    ...
                    if key == "uuid":
                        ...
                        href = f'{args[0]["domain"]}/{pipe.lower().replace(" ", "")}'
                        href += f'/form/{item["id"]}'
        ...
        final_pipeline.append(
            Pipeline(
                email=tenant_pipeline[thread].email,
                fullName=tenant_pipeline[thread].fullName,
                pipelines=user_pipeline,
                totalTasks=tenant_pipeline[thread].totalTasks,
                analyzed=True,
            )
        )
...
def format_and_email(args: Tuple, thread: int):
    ...
    if not final_pipeline[thread].emailed:
        ...
        data = {
            ...
            "User Name": final_pipeline[thread].fullName,
