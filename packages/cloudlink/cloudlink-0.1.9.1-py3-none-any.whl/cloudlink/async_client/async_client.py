...
class client:
    ...
    async def send_packet(self, cmd: str, val: any = None, listener: str = None, room_id: str = None,
    ...
    async def set_username(self, username):
        ...
        await self.send_packet(cmd="setid", val=username, listener=self.setid_listener,
    ...
    async def ping(self, listener: any = None):
        ...
        if listener:
            await self.send_packet(cmd="ping", listener=listener)
        else:
            await self.send_packet(cmd="ping", listener=self.ping_listener)
    ...
    async def send_gmsg(self, value, listener: any = None):
        ...
        await self.send_packet(cmd="gmsg", val=value, listener=listener, quirk=self.supporter.quirk_embed_val)
    ...
    async def send_gvar(self, name, value, listener: any = None):
        ...
        await self.send_packet(cmd="gvar", val=payload, listener=listener, quirk=self.supporter.quirk_update_msg)

    # Sends private message packets.
    async def send_pmsg(self, recipient, value, listener: any = None):
        if not self.client:
            return
        ...
        await self.send_packet(cmd="pmsg", val=payload, listener=listener, quirk=self.supporter.quirk_update_msg)

    # Sends private variable packets. This does not sync with Scratch.
    async def send_pvar(self, name, recipient, value, listener: any = None):
        if not self.client:
            return
        ...
        await self.send_packet(cmd="pvar", val=payload, listener=listener, quirk=self.supporter.quirk_update_msg)

    # Sends direct variable packets.
    async def send_direct(self, recipient: any = None, value: any = None, listener: any = None):
        if not self.client:
            return
        ...
        await self.send_packet(cmd="direct", val=payload, listener=listener, quirk=self.supporter.quirk_update_msg)
    ...
    def __fire_method_callbacks__(self, callback_method, message, listener):
        if callback_method.__name__ in self.method_callbacks:
            for _method in self.method_callbacks[callback_method.__name__]:
                self.asyncio.create_task(_method(self, message, listener))
...
class cl_methods:
    ...
    async def gmsg(self, message, listener):
        room_data = None
        if "rooms" in message:
    ...
    async def pmsg(self, message, listener):
        room_data = None
        if "rooms" in message:
            # Automatically create room and update the global data value
            self.rooms.create(message["rooms"])
    ...
    async def gvar(self, message, listener):
        room_data = None
        if "rooms" in message:
            # Automatically create room and update the global data value
            self.rooms.create(message["rooms"])
    ...
    async def pvar(self, message, listener):
        room_data = None
        if "rooms" in message:
            # Automatically create room and update the global data value
            self.rooms.create(message["rooms"])
    ...
    async def direct(self, message, listener):
        # Fire callbacks for method and it's listener
        self.parent.__fire_method_callbacks__(self.direct, message, listener)
        if listener:
            self.parent.__fire_method_listeners__(listener)

    async def ulist(self, message, listener):
        room_data = None
        if "rooms" in message:
            # Automatically create room and update the global data value
            self.rooms.create(message["rooms"])
    ...
    async def server_version(self, message, listener):
    ...
    async def motd(self, message, listener):
    ...
    async def client_ip(self, message, listener):
