...
from aiovkapi.types.responses.stats import (
...
class StatsCategory(BaseCategory):
    async def get(
        ...
        **kwargs
        ...
        response = await self.request("stats.get", params)
        model = GetResponse
    ...
    async def get_post_reach(
        ...
        params = self.get_set_params(locals())
        response = await self.request("stats.getPostReach", params)
    ...
    async def track_visitor(self, **kwargs) -> int:
        ...
        params = self.get_set_params(locals())
        response = await self.request("stats.trackVisitor", params)
        model = OkResponse
        return model(**response).response
