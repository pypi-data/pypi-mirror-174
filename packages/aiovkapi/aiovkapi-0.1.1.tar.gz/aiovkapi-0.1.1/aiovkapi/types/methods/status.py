...
from aiovkapi.types.responses.status import GetResponse, StatusStatus
...
class StatusCategory(BaseCategory):
    async def get(
        ...
        **kwargs
        ...
        response = await self.request("status.get", params)
        model = GetResponse
    ...
    async def set(
        ...
        **kwargs
        ...
        params = self.get_set_params(locals())
        response = await self.request("status.set", params)
        model = OkResponse
        return model(**response).response
