...
def update_infodb(valor_atualizar, just_append=True, path_dir_=None, antiblock=False):
    ...
    if client_use:
        ...
        if just_append == False:
            ...
            if antiblock == False:
                editando = False
        elif just_append == True and type(valor_atualizar) == dict:
            for k, v in valor_atualizar.items():
                temp_list = path_dir
                temp_list.append(k)
                atualiza_infodb(v, antiblock=True, path_dir_=temp_list)
            editando = False
...
def cdadd(path_to_update, replace_all_path=False, path_dir_=None):
    if client_use:
        ...
        if type(path_to_update) == list:
            ...
            if len(path_to_update) > 1 and '/' in path_to_update:
