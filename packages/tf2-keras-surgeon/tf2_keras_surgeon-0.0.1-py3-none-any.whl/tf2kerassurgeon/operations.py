...
def delete_layer(model, layer, *, node_indexes=None, copy=True):
    ...
    Args:
        ...
        node_indexes: The indices of the inbound_node to the layer instances to
                      be deleted.
    ...
    surgeon.add_job('delete_layer', layer, node_indexes=node_indexes)
    return surgeon.operate()


def insert_layer(model, layer, new_layer, *, node_indexes=None, copy=True):
    """Insert new_layer before instances of layer.

    If node_indexes is not specified. The layer will be inserted before all
    ...
    Args:
        ...
        node_indexes: The indices of the inbound_node to layer where the
    ...
    surgeon.add_job('insert_layer', layer,
                    node_indexes=node_indexes, new_layer=new_layer)
    return surgeon.operate()


def replace_layer(model, layer, new_layer, *,  node_indexes=None, copy=True):
    """Replace instances of layer with new_layer.

        If node_indexes is not specified, all instances of layer will be
...
def delete_channels(model, layer, channels, *, node_indexes=None, copy=None):
    ...
    If the layer is shared and node_indexes is set, channels will be deleted
    ...
    Args:
        ...
        node_indexes: Indices of the nodes where channels are to be deleted.
        copy: If True, the model will be copied before and after
              manipulation. This keeps both the old and new models' layers
              clean of each-others data-streams.
    ...
    """
    surgeon = Surgeon(model, copy)
    surgeon.add_job('delete_channels', layer, node_indexes=node_indexes, channels=channels)
    return surgeon.operate()
