...
class Token(APIObject):
    ...
    """
    token = StringDescr(doc="unique 32-bit token created by Ted and used to access a shared dataset")
    servicer = StringDescr(doc="the servicer of the dataset shared by the token")
    dataset = StringDescr(doc="the dataset shared by the token")
    project = StringDescr(doc="the project of the dataset shared by the token")
    ttl = IntDescr(doc="the remaining time in seconds until the token expires")
