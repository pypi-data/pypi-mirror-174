...
class Response(requests.Response):
    ...
    def iter_events(self, chunk_size: int = 1) -> t.Iterator["Event"]:
        ...
        with self:
            ...
            for line in iter_sse_lines():
                if line:
                    ...
                    retry = event.retry if event.retry is not None else retry
                elif event.data:
                    if event.data.endswith("\n"):
                        event.data = event.data[:-1]
                    yield event
                    last_id = event.id if event.id is not None else last_id
                    event = Event()
