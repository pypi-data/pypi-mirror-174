...
class client():
    ...
    def __init__(self,uri="",token="",services=None,token_key=None,logger:dict=None):
        if logger:
        ...
        if services:
            uri=uri.rstrip("/")
            uri+="?key="+str.split(services,",")
        self.__context__=self.__get_conf__(uri,header)
        if self.__context__:
            self.__dict__["__srv_struct__"]={}
            for k,v in self.__context__.items():
