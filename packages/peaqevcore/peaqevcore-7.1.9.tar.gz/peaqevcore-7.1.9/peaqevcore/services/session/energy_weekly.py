...
class EnergyWeekly:
    ...
    def set_init_model(self):
        ...
        for i in range(0,6):
            ret[i] = Day(0,0)
        _LOGGER.error(f"This is the resulting init_model: {ret}")
        self.model = ret
    ...
    def unpack(self, incoming: dict) -> dict[int, Day]:
        if isinstance(incoming, dict) and len(incoming) > 0:
            _LOGGER.error(f"This data was correct: {incoming}")
            model = {}
            for idx, m in incoming.items():
                m_ret = Day(m["sessions"], m["total_charge"])
                model[idx] = m_ret
            _LOGGER.error(f"This is the resulting unpack: {model}")
            self.model = model
        else: 
            _LOGGER.error(f"This data was not correct: {incoming}")
