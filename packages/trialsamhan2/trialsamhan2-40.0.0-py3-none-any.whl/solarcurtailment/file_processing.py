...
class FileProcessing():
    ...
    def check_data_size(self, data):
        ...
        if len(data) > MIN_LEN:
            if (data['power'].abs() > 16).any(): #also check whether it contains power more than 16.
                if ((data.index.hour >= 7) & (data.index.hour <= 17)).sum() > 0: #there must be datapoint between 7 am and 6 pm
                    size_is_ok = True
                else:
                    size_is_ok = False
            else:
